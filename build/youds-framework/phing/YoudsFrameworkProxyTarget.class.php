 ...
 * @version    $Id$
 */
class YoudsFrameworkProxyTarget extends Target
{
	/**
	...
	public function main()
	{
		...
		foreach($thisProject->getProperties() as $name => $property) {
			if(!YoudsFrameworkProxyProject::isPropertyProtected($name) && $project->getProperty($name) === null) {
		...
		foreach($project->getProperties() as $name => $property) {
			if(!YoudsFrameworkProxyProject::isPropertyProtected($name) && $thisProject->getProperty($name) === null) {
