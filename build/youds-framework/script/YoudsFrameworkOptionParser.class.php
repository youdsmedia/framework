...
require_once(__DIR__ . '/YoudsFrameworkOptionException.class.php');

/**
 * Parses input options and arguments to the YoudsFramework script.
 *
 * @package    youds
 * @subpackage build
 *
 ...
 * @version    $Id$
 */
class YoudsFrameworkOptionParser
{
	/**
	 ...
	 *                   the source.
	 *
	 * @see        YoudsFrameworkOptionParser::$source
	 */
		...
		for($i = 0; $i < $size; $i++) {
			...
			foreach($options as $optionName => $option) {
				if(in_array($source[$i], array_map(array($this, 'getSourceNameForShortName'), $option['short_names'])) ||
					...
					if(count($arguments) !== $option['arguments']) {
						throw new YoudsFrameworkOptionException(
			...
			if($handler === null) {
				if(strpos($source[$i], $this->shortNamePrefix) === 0 ||
					...
					throw new YoudsFrameworkOptionException(sprintf('Unexpected option %s', $source[$i]));
				} else {
					/* Accept arguments. */
					break;
				}
	 ...
	 *
	 * @see        YoudsFrameworkOptionParser::parse()
	 *
	 * @author     Noah Fontes <noah.fontes@bitextender.com>
	 * @since      1.0.0
	 */
