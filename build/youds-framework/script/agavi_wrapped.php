...
require(__DIR__ . '/YoudsFrameworkOptionParser.class.php');
...
function input_help_display()
{
	...
	$GLOBALS['OUTPUT']->write('  --youds-source-directory <path>  Sets the YoudsFramework source directory to <path>' . PHP_EOL);
...
function input_help(YoudsFrameworkOptionParser $parser, $name, $arguments, $scriptArguments)
{
	input_help_display();
	exit(0);
}

function input_version(YoudsFrameworkOptionParser $parser, $name, $arguments, $scriptArguments)
{
	$GLOBALS['OUTPUT']->write('YoudsFramework project configuration system, script version $Id$' . PHP_EOL);
	$GLOBALS['OUTPUT']->write(Phing::getPhingVersion() . PHP_EOL);
	exit(0);
}

function input_list(YoudsFrameworkOptionParser $parser, $name, $arguments, $scriptArguments)
{
	$GLOBALS['SHOW_LIST'] = true;
}

function input_define(YoudsFrameworkOptionParser $parser, $name, $arguments, $scriptArguments)
{
	$name = $arguments[0];
	$value = $arguments[1];
...
function input_verbose(YoudsFrameworkOptionParser $parser, $name, $arguments, $scriptArguments)
{
	$GLOBALS['VERBOSE'] = true;
}

function input_youds_source_directory(YoudsFrameworkOptionParser $parser, $name, $arguments, $scriptArguments)
{
	$path = new PhingFile($arguments[0]);
	$path = $path->isAbsolute() ? $path : new PhingFile(START_DIRECTORY, (string)$path);
	
	$GLOBALS['PROPERTIES']['youds.directory.src'] = $path;
}

function input_include_path(YoudsFrameworkOptionParser $parser, $name, $arguments, $scriptArguments)
{
	$path = new PhingFile($arguments[0]);
	$path = $path->isAbsolute() ? $path : new PhingFile(START_DIRECTORY, (string)$path);
...
function input_logger(YoudsFrameworkOptionParser $parser, $name, $arguments, $scriptArguments)
{
	$logger = $arguments[0];
...
$parser = new YoudsFrameworkOptionParser(array_slice($_SERVER['argv'], 1));
...
$parser->addOption('youds_source_directory', array(), array('youds-source-directory'), 'input_youds_source_directory', 1);
...
try {
	$parser->parse();
} catch(YoudsFrameworkOptionException $aae) {
...
if(!isset($GLOBALS['PROPERTIES']['youds.directory.src'])) {
	$GLOBALS['PROPERTIES']['youds.directory.src'] = new PhingFile(realpath(__DIR__ . '/../../..'));
}
if(!is_dir($GLOBALS['PROPERTIES']['youds.directory.src']) || !is_file($GLOBALS['PROPERTIES']['youds.directory.src'] . DIRECTORY_SEPARATOR . 'youds.php')) {
	$GLOBALS['ERROR']->write(sprintf('Error: YoudsFramework source directory expected at %s, but is not present', $GLOBALS['PROPERTIES']['youds.directory.src']) . PHP_EOL);
	$GLOBALS['ERROR']->write(PHP_EOL);
	input_help_display();
	exit(1);
}
...
try {
	...
	$project->addTaskDefinition('youds.import', 'org.youds.build.tasks.YoudsFrameworkImportTask', 'phing');
	$project->addTaskDefinition('youds.locate-project', 'org.youds.build.tasks.YoudsFrameworkLocateprojectTask', 'phing');
	$project->addTaskDefinition('youds.check-project', 'org.youds.build.tasks.YoudsFrameworkCheckprojectTask', 'phing');
	...
	try {
		...
		$task = $project->createTask('youds.import');
		$task->setFile(new PhingFile($GLOBALS['BUILD']->getAbsolutePath()));
		$task->init();
		$task->perform();
		
		$task = $project->createTask('youds.locate-project');
		...
		if($project->getProperty('project.directory') !== null) {
			$task = $project->createTask('youds.check-project');
...
try {
	...
	if(
		...
		Phing::setProperty('phing.logger.defaults', new PhingFile(BUILD_DIRECTORY . '/youds/phing/ansicolorlogger_osxleopard.properties'));
	}
	...
	$logger = new YoudsFrameworkProxyBuildLogger(new $GLOBALS['LOGGER']());
