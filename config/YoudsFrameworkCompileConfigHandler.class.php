...
/**
 * YoudsFrameworkCompileConfigHandler gathers multiple files and puts them into a single
 ...
 * @version    $Id$
 */
class YoudsFrameworkCompileConfigHandler extends YoudsFrameworkXmlConfigHandler
{
	const XML_NAMESPACE = 'http://youds.com/youds/config/parts/compile/1.1';
	...
	public function execute(YoudsFrameworkXmlConfigDomDocument $document)
	{
		...
		foreach($document->getConfigurationElements() as $configuration) {
			...
			foreach($configuration->get('compiles') as $compileFile) {
				...
				$file = YoudsFrameworkToolkit::expandDirectives($file);
