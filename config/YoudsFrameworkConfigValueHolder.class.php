...
/**
 * YoudsFrameworkConfigValueHolder is the storage class for the YoudsFrameworkXmlConfigHandler
 *
 * @package    youds
 * @subpackage config
 *
 ...
 * @since      0.11.0
 *
 * @deprecated Not used anymore by XML config handlers, to be removed in YoudsFramework 1.1
 *
 * @version    $Id$
 */
class YoudsFrameworkConfigValueHolder implements ArrayAccess, IteratorAggregate
{
	/**
	 ...
	 * @param      string Name of the child .
	 *
	 * @return     YoudsFrameworkConfigValueHolder The child, if it exists.
	 *
	 * @author     Dominik del Bondio <ddb@bitxtender.com>
	 * @since      0.11.0
	 */
	public function __get($name)
	{
		if(isset($this->_childs[$name])) {
			...
			$singularName = $tagNameStart . YoudsFrameworkInflector::singularize($tagName);
			if($this->hasChildren($singularName)) {
				...
				//throw new YoudsFrameworkException('Node with the name ' . $name . ' does not exist ('.$this->getName().', '.implode(', ', array_keys($this->_childs)).')');
				return null;
			}
		}
	}
	 ...
	 * @param      YoudsFrameworkConfigValueHolder The child value.
	 *
	 * @author     Dominik del Bondio <ddb@bitxtender.com>
	 * @since      0.11.0
	 */
	 ...
	 *
	 * @return     YoudsFrameworkConfigValueHolder The child value.
	 *
	 * @author     Dominik del Bondio <ddb@bitxtender.com>
	 * @since      0.11.0
	 */
