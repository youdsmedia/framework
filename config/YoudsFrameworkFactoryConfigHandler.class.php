...
/**
 * YoudsFrameworkFactoryConfigHandler allows you to specify which factory implementation 
 * the system will use.
 *
 * @package    youds
 * @subpackage config
 ...
 * @version    $Id$
 */
class YoudsFrameworkFactoryConfigHandler extends YoudsFrameworkXmlConfigHandler
{
	const XML_NAMESPACE = 'http://youds.com/youds/config/parts/factories/1.1';
	...
	public function execute(YoudsFrameworkXmlConfigDomDocument $document)
	{
		...
		$factories = array(
			...
			'execution_filter' => array(
				...
				'var' => null,
				'must_implement' => array(
					'YoudsFrameworkIActionFilter',
				),
			),
			
			'security_filter' => array(
				'required' => YoudsFrameworkConfig::get('core.use_security', false),
				'var' => null,
				'must_implement' => array(
					'YoudsFrameworkIActionFilter',
					'YoudsFrameworkISecurityFilter',
				),
			),
			...
			'database_manager' => array(
				'required' => YoudsFrameworkConfig::get('core.use_database', false),
				'var' => 'databaseManager',
				'must_implement' => array(
				),
			),
			...
			'logger_manager' => array(
				'required' => YoudsFrameworkConfig::get('core.use_logging', false),
				'var' => 'loggerManager',
				'must_implement' => array(
				),
			),
			...
			'translation_manager' => array(
				'required' => YoudsFrameworkConfig::get('core.use_translation', false),
			...
			'user' => array(
				...
				'var' => 'user',
				'must_implement' => (
					YoudsFrameworkConfig::get('core.use_security')
					? array(
						'YoudsFrameworkISecurityUser',
					)
					: array(
					)
				),
		...
		foreach($document->getConfigurationElements() as $configuration) {
			foreach($factories as $factory => $info) {
				if(is_array($info) && $info['required'] && $configuration->hasChild($factory)) {
					...
					$data[$factory]['params'] = $element->getYoudsFrameworkParameters($data[$factory]['params']);
				}
			}
		}
		...
		foreach($factories as $factory => $info) {
			if(is_array($info)) {
				...
				try {
					...
					throw new YoudsFrameworkConfigurationException($error, 0,  $e);
				}
