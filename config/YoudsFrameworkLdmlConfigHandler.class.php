...
/**
 * YoudsFrameworkLdmlConfigHandler allows you to parse ldml files into an array.
 *
 * @package    youds
 * @subpackage config
 *
 ...
 * @version    $Id$
 */
class YoudsFrameworkLdmlConfigHandler extends YoudsFrameworkConfigHandler
{
	...
	public function execute($config, $context = null)
	{
		...
		$lookupPaths = YoudsFrameworkLocale::getLookupPath($pathParts['filename']);
		$lookupPaths[] = 'root';
		...
		foreach(array_reverse($lookupPaths) as $basename) {
			...
			if(is_readable($filePath)) {
				$ldmlTree = YoudsFrameworkConfigCache::parseConfig($filePath, false, $this->getValidationFile(), $this->parser);
		...
		$dayMap = array(
										'sun' => YoudsFrameworkDateDefinitions::SUNDAY,
										'mon' => YoudsFrameworkDateDefinitions::MONDAY,
										'tue' => YoudsFrameworkDateDefinitions::TUESDAY,
										'wed' => YoudsFrameworkDateDefinitions::WEDNESDAY,
										'thu' => YoudsFrameworkDateDefinitions::THURSDAY,
										'fri' => YoudsFrameworkDateDefinitions::FRIDAY,
										'sat' => YoudsFrameworkDateDefinitions::SATURDAY,
		);
	 ...
	 *
	 * @param      YoudsFrameworkConfigValueHolder The ldml tree.
	 *
	 * @author     Dominik del Bondio <ddb@bitxtender.com>
	 * @since      0.11.0
	 */
	protected function prepareParentInformation($ldmlTree)
	{
		$this->nodeRefs = array();
		$i = 0;
		$ldmlTree->setAttribute('__youds_node_id', $i);
		$ldmlTree->setAttribute('__youds_parent_id', null);
	 ...
	 *
	 * @param      YoudsFrameworkConfigValueHolder The ldml node.
	 *
	 * @author     Dominik del Bondio <ddb@bitxtender.com>
	 * @since      0.11.0
	 */
	protected function generateParentInformation($childList, &$nextId, $parentId)
	{
		foreach($childList as $child) {
			$child->setAttribute('__youds_node_id', $nextId);
			$child->setAttribute('__youds_parent_id', $parentId);
			$this->nodeRefs[$nextId] = $child;
			++$nextId;
			if($child->hasChildren()) {
				$this->generateParentInformation($child->getChildren(), $nextId, $child->getAttribute('__youds_node_id'));
			}
		}
	}
...
*/
	...
	/**
	 * Generates the array used by YoudsFrameworkLocale from an LDML tree.
	 *
	 * @param      YoudsFrameworkConfigValueHolder The ldml tree.
							...
							foreach($dtfItems as $item) {
								if($item->getName() == 'dateTimeFormatLength') {
									if(isset($item->dateTimeFormat->pattern)) {
										...
										throw new YoudsFrameworkException('unknown child content in dateTimeFormatLength tag');
									}
								} elseif($item->getName() == 'availableFormats') {
									foreach($item as $dateFormatItem) {
										if($dateFormatItem->getName() != 'dateFormatItem') {
											throw new YoudsFrameworkException('unknown childtag "' . $dateFormatItem->getName() . '" in availableFormats tag');
										}
									...
									foreach($item as $appendItem) {
										if($appendItem->getName() != 'appendItem') {
											throw new YoudsFrameworkException('unknown childtag "' . $appendItem->getName() . '" in appendItems tag');
										}
									...
									throw new YoudsFrameworkException('unknown childtag "' . $item->getName() . '" in dateTimeFormats tag');
								}
							}
						}
	 ...
	 *
	 * @param      array List of YoudsFrameworkConfigValueHolder items.
	 * @param      array The array to store the parsed data to.
	 *
	 * @return     array The array with the data.
	 *
	...
	protected function getTypeList($list, &$data)
	{
		...
		if($lc != count($list->getChildren())) {
			throw new YoudsFrameworkException('wrong tagcount');
		}
	 ...
	 *
	 * @param      YoudsFrameworkConfigValueHolder The item.
	...
	protected function getChildsOrAlias($item)
	{
		if(isset($item->alias)) {
			...
			if($alias->getAttribute('source') != 'locale') {
				throw new YoudsFrameworkException('The alias handling doesn\'t support any source except locale (' . $alias->getAttribute('source') . ' was given)');
			}

			$pathParts = explode('/', $alias->getAttribute('path'));

			$currentNodeId = $item->getAttribute('__youds_node_id');
			
			foreach($pathParts as $part) {
				// select the parent node
				if($part == '..') {
					$currentNodeId = $this->nodeRefs[$currentNodeId]->getAttribute('__youds_parent_id');
				} else {
					$predicates = array();
					if(preg_match('#([^\[]+)\[([^\]]+)\]#', $part, $match)) {
						if(!preg_match('#@([^=]+)=\'([^\']+)\'#', $match[2], $predMatch)) {
							throw new YoudsFrameworkException('Unknown predicate ' . $match[2] . ' in alias xpath spec');
						}
						$tagName = $match[1];
					...
					foreach($this->nodeRefs[$currentNodeId]->getChildren() as $childNode) {
						...
						if($isSearchedNode) {
							$currentNodeId = $childNode->getAttribute('__youds_node_id');
						}
					}
				}
			}
