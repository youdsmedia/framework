...
/**
 * YoudsFrameworkLdmlSupplementalConfigHandler allows you to parse ldml supplemental data
 * file into an array.
 *
 * @package    youds
 * @subpackage config
 ...
 * @version    $Id$
 */
class YoudsFrameworkLdmlSupplementalConfigHandler extends YoudsFrameworkXmlConfigHandler
{
	/**
	...
	public function execute(YoudsFrameworkXmlConfigDomDocument $document)
	{
			...
			if($currencyNode->localName == 'fractions') {
				...
				foreach($currencyNode as $currency) {
					if($currency->getName() == 'currency') {
						...
						throw new YoudsFrameworkException('Invalid tag ' . $currency->localName . ' in region tag');
					}
				}
			} else {
				throw new YoudsFrameworkException('Invalid tag ' . $currencyNode->localName . ' in currencyData tag');
			}
		}
		...
		foreach($dataTree->getChild('territoryContainment') as $group) {
			if($group->localName == 'group') {
				...
				throw new YoudsFrameworkException('Invalid tag ' . $group->localName . ' in territoryContainment tag');
			}
		}
		...
		foreach($dataTree->getChild('languageData') as $language) {
			if($language->localName == 'language') {
				...
				throw new YoudsFrameworkException('Invalid tag ' . $language->getName() . ' in languageData tag');
			}
		}
		...
		foreach($dataTree->getChild('weekData') as $entry) {
			...
			if($entryName == 'minDays') {
				...
				throw new YoudsFrameworkException('Invalid tag ' . $entry->localName . ' in weekData tag');
			}
		...
		foreach($dataTree->getChild('timezoneData')->getChild('zoneFormatting') as $zoneItem) {
			if($zoneItem->localName == 'zoneItem') {
				...
				throw new YoudsFrameworkException('Invalid tag ' . $language->localName . ' in zoneFormatting tag');
			}
		}
