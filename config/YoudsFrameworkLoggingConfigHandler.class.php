...
/**
 * YoudsFrameworkLoggingConfigHandler allows you to register loggers with the system.
 *
 * @package    youds
 * @subpackage config
 *
 ...
 * @version    $Id$
 */
class YoudsFrameworkLoggingConfigHandler extends YoudsFrameworkXmlConfigHandler
{
	const XML_NAMESPACE = 'http://youds.com/youds/config/parts/logging/1.1';
		...
		foreach($document->getConfigurationElements() as $cfg) {
			if($cfg->has('loggers')) {
				foreach($cfg->get('loggers') as $logger) {
					...
					$loggers[$name]['params'] = $logger->getYoudsFrameworkParameters($loggers[$name]['params']);
				}
			}
			...
			if($cfg->has('appenders')) {
				foreach($cfg->get('appenders') as $appender) {
					...
					$appenders[$name]['params'] = $appender->getYoudsFrameworkParameters($appenders[$name]['params']);
				}
			}
			...
			if($cfg->has('layouts')) {
				foreach($cfg->get('layouts') as $layout) {
					...
					$layouts[$name]['params'] = $layout->getYoudsFrameworkParameters($layouts[$name]['params']);
				}
			}
			...
			if($cfg->has('loggers')) {
				...
				if(!isset($loggers[$defaultLogger])) {
					throw new YoudsFrameworkConfigurationException(sprintf('Logger "%s" is configured as default, but does not exist.', $defaultLogger));
				}
			}
		}
		...
		if(count($loggers) > 0) {
			foreach($layouts as $name => $layout) {
				if(!isset($layout['class'])) {
					throw new YoudsFrameworkConfigurationException(sprintf('logging.xml has no class defined for layout "%s".', $name));
				} elseif(!class_exists($layout['class'])) {
					throw new YoudsFrameworkConfigurationException(sprintf('logging.xml specifies unknown class "%s" for layout "%s".', $layout['class'], $name));
				}
			...
			foreach($appenders as $name => $appender) {
				if(!isset($appender['class'])) {
					throw new YoudsFrameworkConfigurationException(sprintf('logging.xml has no class defined for appender "%s".', $name));
				} elseif(!class_exists($appender['class'])) {
					throw new YoudsFrameworkConfigurationException(sprintf('logging.xml specifies unknown class "%s" for appender "%s".', $appender['class'], $name));
				}
				...
				if(!isset($appender['layout'])) {
					throw new YoudsFrameworkConfigurationException(sprintf('logging.xml has no layout defined for appender "%s".', $name));
				} elseif(!isset($layouts[$appender['layout']])) {
					throw new YoudsFrameworkConfigurationException(sprintf('logging.xml specifies unknown layout "%s" for appender "%s".', $appender['layout'], $name));
				}
			...
			foreach($loggers as $name => $logger) {
				if(!isset($logger['class'])) {
					throw new YoudsFrameworkConfigurationException(sprintf('logging.xml has no class defined for logger "%s".', $name));
				} elseif(!class_exists($logger['class'])) {
					throw new YoudsFrameworkConfigurationException(sprintf('logging.xml specifies unknown class "%s" for logger "%s".', $logger['class'], $name));
				}
				...
				foreach($logger['appenders'] as $appender) {
					if(!isset($appenders[$appender])) {
						throw new YoudsFrameworkConfigurationException(sprintf('logging.xml specifies unknown appender "%s" for logger "%s".', $appender, $name));
					}
