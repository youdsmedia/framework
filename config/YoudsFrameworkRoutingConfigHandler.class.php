...
/**
 * YoudsFrameworkRoutingConfigHandler allows you to specify a list of routes that will
 ...
 * @version    $Id$
 */
class YoudsFrameworkRoutingConfigHandler extends YoudsFrameworkXmlConfigHandler
{
	const XML_NAMESPACE = 'http://youds.com/youds/config/parts/routing/1.1';
	...
	public function execute(YoudsFrameworkXmlConfigDomDocument $document)
	{
		...
		$routing = YoudsFrameworkContext::getInstance($this->context)->getRouting();
	...
	/**
	 * Takes a nested array of YoudsFrameworkConfigValueHolder containing the routing
	 * information and creates the routes in the given routing.
	 *
	 * @param      YoudsFrameworkRouting The routing instance to create the routes in.
	 ...
	 * @since      0.11.0
	 */
	protected function parseRoutes(YoudsFrameworkRouting $routing, $routes, $parent = null)
	{
		foreach($routes as $route) {
			$pattern = YoudsFrameworkToolkit::expandDirectives($route->getAttribute('pattern'));
			$opts = array();
			if($route->hasAttribute('imply'))					$opts['imply']				= YoudsFrameworkToolkit::literalize($route->getAttribute('imply'));
			if($route->hasAttribute('cut'))						$opts['cut']					= YoudsFrameworkToolkit::literalize($route->getAttribute('cut'));
			if($route->hasAttribute('stop'))					$opts['stop']					= YoudsFrameworkToolkit::literalize($route->getAttribute('stop'));
			if($route->hasAttribute('name'))					$opts['name']					= YoudsFrameworkToolkit::expandDirectives($route->getAttribute('name'));
			if($route->hasAttribute('source'))				$opts['source']				= YoudsFrameworkToolkit::expandDirectives($route->getAttribute('source'));
			if($route->hasAttribute('constraint'))		$opts['constraint']		= array_map('trim', explode(' ', trim(YoudsFrameworkToolkit::expandDirectives($route->getAttribute('constraint')))));
			// values which will be set when the route matched
			if($route->hasAttribute('action'))				$opts['action']				= YoudsFrameworkToolkit::expandDirectives($route->getAttribute('action'));
			if($route->hasAttribute('locale'))				$opts['locale']				= YoudsFrameworkToolkit::expandDirectives($route->getAttribute('locale'));
			if($route->hasAttribute('method'))				$opts['method']				= YoudsFrameworkToolkit::expandDirectives($route->getAttribute('method'));
			if($route->hasAttribute('module'))				$opts['module']				= YoudsFrameworkToolkit::expandDirectives($route->getAttribute('module'));
			if($route->hasAttribute('output_type'))		$opts['output_type']	= YoudsFrameworkToolkit::expandDirectives($route->getAttribute('output_type'));
			...
			if($route->has('callbacks')) {
				...
				foreach($route->get('callbacks') as $callback) {
					$opts['callbacks'][] = array(
						...
						'parameters' => $callback->getYoudsFrameworkParameters(),
					);
				}
			}

			$opts['parameters'] = $route->getYoudsFrameworkParameters();
