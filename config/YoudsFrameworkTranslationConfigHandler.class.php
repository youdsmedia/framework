...
/**
 * YoudsFrameworkTranslationConfigHandler allows you to define translator implementations
 ...
 * @version    $Id$
 */
class YoudsFrameworkTranslationConfigHandler extends YoudsFrameworkXmlConfigHandler
{
	const XML_NAMESPACE = 'http://youds.com/youds/config/parts/translation/1.1';
	...
	public function execute(YoudsFrameworkXmlConfigDomDocument $document)
	{
		...
		foreach($document->getConfigurationElements() as $cfg) {
			...
			if($cfg->hasChild('available_locales')) {
				...
				foreach($availableLocales as $locale) {
					...
					$localeData[$name]['params'] = $locale->getYoudsFrameworkParameters($localeData[$name]['params']);
		...
		foreach($localeData as $locale) {
			...
			$data[] = sprintf('$this->availableConfigLocales[%s] = array(\'identifier\' => %s, \'identifierData\' => %s, \'parameters\' => %s);', var_export($locale['name'], true), var_export($locale['name'], true), var_export(YoudsFrameworkLocale::parseLocaleIdentifier($locale['name']), true), var_export($locale['params'], true));
		}
			...
			foreach(array('msg', 'num', 'cur', 'date') as $type) {
				if(isset($translator[$type]['class'])) {
					if(!class_exists($translator[$type]['class'])) {
						throw new YoudsFrameworkConfigurationException(sprintf('The Translator or Formatter class "%s" for domain "%s" could not be found.', $translator[$type]['class'], $domain));
					}
					$data[] = join("\n", array(
	 ...
	 *
	 * @param      YoudsFrameworkConfigValueHolder The Translator node.
	 *
	...
	protected function getFilters($translator)
	{
		...
		if($translator->has('filters')) {
			foreach($translator->get('filters') as $filter) {
				...
				if(!is_callable($func)) {
					throw new YoudsFrameworkConfigurationException('Non-existant or uncallable filter function "' . $filter->getValue() .  '" specified.');
				}
				$filters[] = $func;
			}
		}
	 ...
	 *
	 * @param      YoudsFrameworkConfigValueHolder The translators container.
	...
	protected function getTranslators($translators, &$data, $parent = null)
	{
		static $defaultData = array(
			...
			'num'  => array('class' => 'YoudsFrameworkNumberFormatter', 'filters' => array(), 'params' => array()),
			'cur'  => array('class' => 'YoudsFrameworkCurrencyFormatter', 'filters' => array(), 'params' => array()),
			'date' => array('class' => 'YoudsFrameworkDateFormatter', 'filters' => array(), 'params' => array()),
		);
		...
		foreach($translators as $translator) {
			...
			foreach(array('msg' => 'message_translator', 'num' => 'number_formatter', 'cur' => 'currency_formatter', 'date' => 'date_formatter') as $type => $nodeName) {
				if($translator->hasChild($nodeName)) {
					...
					$domainData[$type]['params'] = $node->getYoudsFrameworkParameters($domainData[$type]['params']);
