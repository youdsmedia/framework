...
/**
 * YoudsFrameworkValidatorConfigHandler allows you to register validators with the
 * system.
 *
 * @package    youds
 * @subpackage config
 ...
 * @version    $Id$
 */
class YoudsFrameworkValidatorConfigHandler extends YoudsFrameworkXmlConfigHandler
{
	const XML_NAMESPACE = 'http://youds.com/youds/config/parts/validators/1.1';
		...
		foreach($document->getConfigurationElements() as $cfg) {
			if($cfg->has('validator_definitions')) {
				foreach($cfg->get('validator_definitions') as $def) {
					...
					$this->classMap[$name]['parameters'] = $def->getYoudsFrameworkParameters($this->classMap[$name]['parameters']);
					$this->classMap[$name]['errors'] = $this->getYoudsFrameworkErrors($def, $this->classMap[$name]['errors']);
				}
			}
	 ...
	 *
	 * @param      YoudsFrameworkXmlConfigDomElement The value holder of this validator.
	...
	protected function getValidatorArray($validator, $code, $parent, $stdSeverity, $stdMethod, $stdRequired = true, $stdTranslationDomain = null)
	{
		if(!isset($this->classMap[$validator->getAttribute('class')])) {
			...
			if(!class_exists($class)) {
				throw new YoudsFrameworkValidatorException('unknown validator found: ' . $class);
			}
		...
		if($validator->hasAttribute('name')) {
			...
			$name = YoudsFrameworkToolkit::uniqid();
		...
		$parameters = $validator->getYoudsFrameworkParameters($parameters);
		...
		$errors = $this->getYoudsFrameworkErrors($validator, $this->classMap[$validator->getAttribute('class')]['errors']);
		
		if($validator->hasAttribute('required')) {
			$stdRequired = $parameters['required'] = YoudsFrameworkToolkit::literalize($validator->getAttribute('required'));
		}
	 ...
	 *
	 * @see        YoudsFrameworkValidatorConfigHandler::getValidatorArray()
	 *
	 * @param      YoudsFrameworkXmlConfigDomElement The value holder of this validator.
	 * @param      array                    The code of old validators (we simply
	 *                                      overwrite "old" validators here).
	...
	/**
	 * Retrieve all of the YoudsFramework error elements associated with this
	 * element.
	 *
	 * @param      YoudsFrameworkXmlConfigDomElement The value holder of this validator.
	 ...
	 * @since      1.0.8
	 */
	public function getYoudsFrameworkErrors(YoudsFrameworkXmlConfigDomElement $node, array $existing = array())
	{
		$result = $existing;
