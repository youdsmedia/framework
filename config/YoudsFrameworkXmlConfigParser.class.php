...
/**
 * YoudsFrameworkXmlConfigParser handles both YoudsFramework and foreign XML configuration files,
 ...
 * @version    $Id$
 */
class YoudsFrameworkXmlConfigParser
{
	const NAMESPACE_AGAVI_ENVELOPE_0_11 = 'http://youds.com/youds/1.0/config';
	
	const NAMESPACE_AGAVI_ENVELOPE_1_0 = 'http://youds.com/youds/config/global/envelope/1.0';
	
	const NAMESPACE_AGAVI_ENVELOPE_1_1 = 'http://youds.com/youds/config/global/envelope/1.1';
	
	const NAMESPACE_AGAVI_ENVELOPE_LATEST = self::NAMESPACE_AGAVI_ENVELOPE_1_1;
	
	const NAMESPACE_AGAVI_ANNOTATIONS_1_0 = 'http://youds.com/youds/config/global/annotations/1.0';
	...
	/**
	 * @var        array A list of XML namespaces for YoudsFramework configuration files as
	 *                   keys and their associated XPath namespace prefix (value).
	 */
	public static $youdsEnvelopeNamespaces = array(
		self::NAMESPACE_AGAVI_ENVELOPE_0_11 => 'youds_envelope_0_11',
		self::NAMESPACE_AGAVI_ENVELOPE_1_0 => 'youds_envelope_1_0',
		self::NAMESPACE_AGAVI_ENVELOPE_1_1 => 'youds_envelope_1_1',
	);
	 ...
	 */
	public static $youdsNamespaces = array(
		self::NAMESPACE_AGAVI_ENVELOPE_0_11 => 'youds_envelope_0_11',
		self::NAMESPACE_AGAVI_ENVELOPE_1_0 => 'youds_envelope_1_0',
		self::NAMESPACE_AGAVI_ENVELOPE_1_1 => 'youds_envelope_1_1',
		self::NAMESPACE_AGAVI_ANNOTATIONS_1_0 => 'youds_annotations_1_0',
	);
	...
	/**
	 * Test if the given document looks like an YoudsFramework config file.
	 *
	 * @param      DOMDocument The document to test.
	 *
	 * @return     bool True, if it is an YoudsFramework config document, false otherwise.
	 *
	 * @author     David Zülke <david.zuelke@bitextender.com>
	 * @since      1.0.0
	 */
	public static function isYoudsFrameworkConfigurationDocument(DOMDocument $doc)
	{
		return $doc->documentElement && $doc->documentElement->localName == 'configurations' && self::isYoudsFrameworkEnvelopeNamespace($doc->documentElement->namespaceURI);
	}
	
	/**
	 * Check if the given namespace URI is a valid YoudsFramework envelope namespace.
	 *
	 ...
	 * @since      1.0.0
	 */
	public static function isYoudsFrameworkEnvelopeNamespace($namespaceUri)
	{
		return isset(self::$youdsEnvelopeNamespaces[$namespaceUri]);
	}
	
	/**
	 * Check if a given namespace URI is a valid YoudsFramework namespace.
	 *
	 * @param      string The namespace URI.
	 *
	 ...
	 * @since      1.0.0
	 */
	public static function isYoudsFrameworkNamespace($namespaceUri)
	{
		return isset(self::$youdsNamespaces[$namespaceUri]);
	}
	 ...
	 * @since      1.0.0
	 */
	public static function getYoudsFrameworkNamespacePrefix($namespaceUri)
	{
		if(self::isYoudsFrameworkNamespace($namespaceUri)) {
			return self::$youdsNamespaces[$namespaceUri];
		}
		return null;
	}
	
	/**
	 * Register YoudsFramework namespace prefixes in a given document.
	 *
	 * @param      YoudsFrameworkXmlConfigDomDocument The document.
	 *
	 * @author     Noah Fontes <noah.fontes@bitextender.com>
	 * @since      1.0.0
	 */
	public static function registerYoudsFrameworkNamespaces(YoudsFrameworkXmlConfigDomDocument $document)
	{
		$xpath = $document->getXpath();
		
		foreach(self::$youdsNamespaces as $namespaceUri => $prefix) {
			$xpath->registerNamespace($prefix, $namespaceUri);
		}
		
		/* Register the latest namespaces. */
		$xpath->registerNamespace('youds_envelope_latest', self::NAMESPACE_AGAVI_ENVELOPE_LATEST);
		$xpath->registerNamespace('youds_annotations_latest', self::NAMESPACE_AGAVI_ANNOTATIONS_LATEST);
	}
	...
	{
		$isYoudsFrameworkConfigFormat = true;
		...
		while($nextPath !== null) {
			...
			$parser = new YoudsFrameworkXmlConfigParser($nextPath, $environment, $context);
			$doc = $parser->execute($transformationInfo[self::STAGE_SINGLE], $validationInfo[self::STAGE_SINGLE]);
			
			// put the new document in the list
			$docs[] = $doc;
			
			// make sure it (still) is a <configurations> file with the proper YoudsFramework namespace
			if($isYoudsFrameworkConfigFormat) {
				$isYoudsFrameworkConfigFormat = self::isYoudsFrameworkConfigurationDocument($doc);
			}
			
			// is it an YoudsFramework <configurations> element? does it have a parent attribute? yes? good. parse that next
			// TODO: support future namespaces
			if($isYoudsFrameworkConfigFormat && $doc->documentElement->hasAttribute('parent')) {
				$theNextPath = YoudsFrameworkToolkit::literalize($doc->documentElement->getAttribute('parent'));
				
				// no infinite loop plz, kthx
				if($nextPath === $theNextPath) {
					throw new YoudsFrameworkParseException(sprintf("YoudsFramework detected an infinite loop while processing parent configuration files of \n%s\n\nFile\n%s\nincludes itself as a parent.", $path, $theNextPath));
		...
		$retval = new YoudsFrameworkXmlConfigDomDocument();
		foreach(self::$youdsEnvelopeNamespaces as $envelopeNamespaceUri => $envelopeNamespacePrefix) {
			$retval->getXpath()->registerNamespace($envelopeNamespacePrefix, $envelopeNamespaceUri);
		}
		
		if($isYoudsFrameworkConfigFormat) {
			// if it is an YoudsFramework config, we'll create a new document with all files' <configuration> blocks inside
			$retval->appendChild(new YoudsFrameworkXmlConfigDomElement('configurations', null, self::NAMESPACE_AGAVI_ENVELOPE_LATEST));
			...
			foreach($docs as $doc) {
				...
				foreach($doc->documentElement->childNodes as $node) {
					if($node->nodeType == XML_ELEMENT_NODE && $node->localName == 'configuration' && self::isYoudsFrameworkEnvelopeNamespace($node->namespaceURI)) {
				...
				if($doc->isYoudsFrameworkConfiguration()) {
			...
			$configurationOrder = array(
				'count(self::node()[@youds_annotations_latest:matched and not(@environment) and not(@context)])',
				'count(self::node()[@youds_annotations_latest:matched and @environment and not(@context)])',
				'count(self::node()[@youds_annotations_latest:matched and not(@environment) and @context])',
				'count(self::node()[@youds_annotations_latest:matched and @environment and @context])',
			);
			...
			// it's not an youds config file. just pass it through then
	...
	public function __construct($path, $environment = null, $context = null)
	{
		...
		if(!is_readable($path)) {
			...
			throw new YoudsFrameworkUnreadableException($error);
		}
		
		// store path to the config file
		$this->path = $path;
		
		// YoudsFrameworkXmlConfigDomDocument has convenience methods!
		try {
			$this->doc = new YoudsFrameworkXmlConfigDomDocument();
			$this->doc->substituteEntities = true;
			$this->doc->load($path);
		} catch(DOMException $dome) {
			throw new YoudsFrameworkParseException(sprintf('Configuration file "%s" could not be parsed: %s', $path, $dome->getMessage()), 0, $dome);
		}
	}
	...
	public function execute(array $transformationInfo = array(), array $validationInfo = array())
	{
		...
		self::validate($this->doc, $this->environment, $this->context, $validationInfo[YoudsFrameworkXmlConfigParser::STEP_TRANSFORMATIONS_BEFORE]);
		...
		self::validate($this->doc, $this->environment, $this->context, $validationInfo[YoudsFrameworkXmlConfigParser::STEP_TRANSFORMATIONS_AFTER]);
	 ...
	 *
	 * @param      YoudsFrameworkXmlConfigDomDocument The document to act upon.
	 * @param      string The environment name.
	 ...
	 * @since      1.0.0
	 */
	public static function executeCompilation(YoudsFrameworkXmlConfigDomDocument $document, $environment, $context, array $transformationInfo = array(), array $validationInfo = array())
	{
		// resolve xincludes
		self::xinclude($document);
		
		// validate pre-transformation
		self::validate($document, $environment, $context, $validationInfo[YoudsFrameworkXmlConfigParser::STEP_TRANSFORMATIONS_BEFORE]);
		...
		// validate post-transformation
		self::validate($document, $environment, $context, $validationInfo[YoudsFrameworkXmlConfigParser::STEP_TRANSFORMATIONS_AFTER]);
	 ...
	 * @since      1.0.0
	 */
	public static function xinclude(YoudsFrameworkXmlConfigDomDocument $document)
	{
		...
		for($i = 0; $i < $length; $i++) {
			...
			if($element->hasAttribute('href')) {
				...
				$parts[0] = str_replace('\\', '/', YoudsFrameworkToolkit::expandDirectives($parts[0]));
		...
		try {
			...
			throw new YoudsFrameworkParseException(sprintf('Configuration file "%s" could not be parsed: %s', $document->documentURI, $dome->getMessage()), 0, $dome);
		}
	 ...
	 * @since      1.0.0
	 */
	public static function match(YoudsFrameworkXmlConfigDomDocument $document, $environment, $context)
	{
		if($document->isYoudsFrameworkConfiguration()) {
			// it's an youds config, so we need to set "matched" flags on all <configuration> elements where "context" and "environment" attributes match the values below
			...
			foreach($document->getConfigurationElements() as $configuration) {
				...
				if($matched) {
					...
					$configuration->setAttributeNS(self::NAMESPACE_AGAVI_ANNOTATIONS_LATEST, 'youds_annotations_latest:matched', 'true');
				}
			}
		}
	}
	 ...
	 *
	 * @return     YoudsFrameworkXmlConfigDomDocument The transformed document.
	 *
	 * @author     David Zülke <dz@bitxtender.com>
	 * @author     Noah Fontes <noah.fontes@bitextender.com>
	 * @since      0.11.0
	 */
	public static function transform(YoudsFrameworkXmlConfigDomDocument $document, $environment, $context, array $transformationInfo = array(), $transformations = array())
	{
		// loop over all the paths we found and load the files
		foreach($transformationInfo as $href) {
			try {
				$xsl = new YoudsFrameworkXmlConfigDomDocument();
				$xsl->load($href);
			} catch(DOMException $dome) {
				throw new YoudsFrameworkParseException(sprintf('Configuration file "%s" could not be parsed: Could not load XSL stylesheet "%s": %s', $document->documentURI, $href, $dome->getMessage()), 0, $dome);
			}
		...
		foreach($transformations as $xsl) {
			...
			try {
				$proc = new YoudsFrameworkXsltProcessor();
				$proc->registerPHPFunctions();
				$proc->importStylesheet($xsl);
			} catch(Exception $e) {
				throw new YoudsFrameworkParseException(sprintf('Configuration file "%s" could not be parsed: Could not import XSL stylesheet "%s": %s', $document->documentURI, $xsl->documentURI, $e->getMessage()), 0, $e);
			}
			...
			$proc->setParameter('', array(
				'youds.config_path' => (string)$document->documentURI,
				'youds.environment' => (string)$environment,
				'youds.context' => (string)$context,
			));
			...
			try {
				...
				throw new YoudsFrameworkParseException(sprintf('Configuration file "%s" could not be parsed: Could not transform the document using the XSL stylesheet "%s": %s', $document->documentURI, $xsl->documentURI, $e->getMessage()), 0, $e);
			}
	 ...
	 * @since      1.0.0
	 */
	public static function transformProcessingInstructions(YoudsFrameworkXmlConfigDomDocument $document, $environment, $context)
	{
			...
			if(in_array($type, array('text/xml', 'text/xsl', 'application/xml', 'application/xsl+xml'))) {
				...
				if(strpos($href, '#') === 0) {
					...
					if($stylesheets->length) {
						...
						throw new YoudsFrameworkParseException(sprintf('Configuration file "%s" could not be parsed because the inline stylesheet "%s" referenced in the "xml-stylesheet" processing instruction could not be found in the document.', $document->documentURI, $href));
					}
				} else {
					// href references an xsl file, remember the path
					$transformationInfo[] = YoudsFrameworkToolkit::expandDirectives($href);
				}
	 ...
	 * @since      0.11.0
	 */
	public static function validate(YoudsFrameworkXmlConfigDomDocument $document, $environment, $context, array $validationInfo = array())
	{
		// bail out right away if validation is disabled
		if(YoudsFrameworkConfig::get('core.skip_config_validation', false)) {
			return;
		}
		...
		foreach($validationInfo as $type => $files) {
			...
			} catch(YoudsFrameworkParseException $e) {
				$errors[] = $e->getMessage();
			}
		}
		
		if($errors) {
			throw new YoudsFrameworkParseException(sprintf('Validation of configuration file "%s" failed:' . "\n\n%s", $document->documentURI, implode("\n\n", $errors)));
		}
	}
	 ...
	 *
	 * @param      YoudsFrameworkXmlConfigDomDocument The document to clean up.
	 *
	 * @author     David Zülke <dz@bitxtender.com>
	 * @since      0.11.0
	 */
	public static function cleanup(YoudsFrameworkXmlConfigDomDocument $document)
	{
	 ...
	 * @since      1.0.0
	 */
	public static function validateXsi(YoudsFrameworkXmlConfigDomDocument $document)
	{
		...
		if($sources) {
			...
			foreach($sources as &$source) {
				...
				$source = YoudsFrameworkToolkit::expandDirectives($source);
				if(parse_url($source, PHP_URL_SCHEME) === null && !YoudsFrameworkToolkit::isPathAbsolute($source)) {
				...
				if($schema === false) {
					throw new YoudsFrameworkUnreadableException(sprintf('XML Schema validation file "%s" for configuration file "%s" does not exist or is unreadable', $source, $document->documentURI));
				}
				$schemas[] = $schema;
			}
	 ...
	 * @since      0.11.0
	 */
	public static function validateXmlschema(YoudsFrameworkXmlConfigDomDocument $document, array $validationFiles = array())
	{
		foreach($validationFiles as $validationFile) {
			if(!is_resource($validationFile) && !is_readable($validationFile)) {
				throw new YoudsFrameworkUnreadableException(sprintf('XML Schema validation file "%s" for configuration file "%s" does not exist or is unreadable', $validationFile, $document->documentURI));
			}
			
			try {
				$document->schemaValidate($validationFile);
			} catch(DOMException $dome) {
				throw new YoudsFrameworkParseException(sprintf('XML Schema validation of configuration file "%s" failed:' . "\n\n%s", $document->documentURI, $dome->getMessage()), 0, $dome);
			}
		}
	}
	 ...
	 * @since      1.0.0
	 */
	public static function validateXmlschemaSource(YoudsFrameworkXmlConfigDomDocument $document, array $validationSources = array())
	{
	 ...
	 * @since      0.11.0
	 */
	public static function validateRelaxng(YoudsFrameworkXmlConfigDomDocument $document, array $validationFiles = array())
	{
		foreach($validationFiles as $validationFile) {
			if(!is_readable($validationFile)) {
				throw new YoudsFrameworkUnreadableException(sprintf('RELAX NG validation file "%s" for configuration file "%s" does not exist or is unreadable', $validationFile, $document->documentURI));
			}
			
			try {
				$document->relaxNGValidate($validationFile);
			} catch(DOMException $dome) {
				throw new YoudsFrameworkParseException(sprintf('RELAX NG validation of configuration file "%s" failed:' . "\n\n%s", $document->documentURI, $dome->getMessage()), 0, $dome);
			}
		}
	}
	 ...
	 * @since      0.11.0
	 */
	public static function validateSchematron(YoudsFrameworkXmlConfigDomDocument $document, $environment, $context, array $validationFiles = array())
	{
		if(YoudsFrameworkConfig::get('core.skip_config_transformations', false)) {
			return;
		}
		...
		$schematron = new YoudsFrameworkSchematronProcessor();
		$schematron->setNode($document);
		// set some info (config file path, context name, environment name) as params
		// first arg is the namespace URI, which PHP doesn't support. awesome. see http://bugs.php.net/bug.php?id=30622 for the sad details
		// we could use "youds:context" etc, that does work even without such a prefix being declared in the stylesheet, but that would be completely non-XML-ish, confusing, and against the spec. so we use dots instead.
		$schematron->setParameters(array(
			'youds.config_path' => $document->documentURI,
			'youds.environment' => $environment,
			'youds.context' => $context,
		));
		...
		foreach($validationFiles as $href) {
			if(!is_readable($href)) {
				throw new YoudsFrameworkUnreadableException(sprintf('Schematron validation file "%s" for configuration file "%s" does not exist or is unreadable', $href, $document->documentURI));
			}
			
			// load the .sch file
			try {
				$sch = new YoudsFrameworkXmlConfigDomDocument();
				$sch->load($href);
			} catch(DOMException $dome) {
				throw new YoudsFrameworkParseException(sprintf('Schematron validation of configuration file "%s" failed: Could not load schema file "%s": %s', $document->documentURI, $href, $dome->getMessage()), 0, $dome);
			}
			...
			try {
				...
				throw new YoudsFrameworkParseException(sprintf('Schematron validation of configuration file "%s" failed: Transformation failed: %s', $document->documentURI, $e->getMessage()), 0, $e);
			}
			...
			if($results->length) {
				...
				throw new YoudsFrameworkParseException(sprintf('Schematron validation of configuration file "%s" failed:' . "\n\n%s", $document->documentURI, implode("\n", $errors)));
			}
		}
	}
}
