 ...
 * @version    $Id$
 */
class YoudsFrameworkXmlConfigDomDocument extends DOMDocument
{
	/**
	...
	/**
	 * @var        array A map of DOM classes and extended YoudsFramework implementations.
	 */
	protected $nodeClassMap = array(
		'DOMAttr'                  => 'YoudsFrameworkXmlConfigDomAttr',
		'DOMCharacterData'         => 'YoudsFrameworkXmlConfigDomCharacterData',
		'DOMComment'               => 'YoudsFrameworkXmlConfigDomComment',
		// yes, even DOMDocument, so we don't get back a vanilla DOMDocument when doing $doc->documentElement etc
		'DOMDocument'              => 'YoudsFrameworkXmlConfigDomDocument',
		'DOMDocumentFragment'      => 'YoudsFrameworkXmlConfigDomDocumentFragment',
		'DOMDocumentType'          => 'YoudsFrameworkXmlConfigDomDocumentType',
		'DOMElement'               => 'YoudsFrameworkXmlConfigDomElement',
		'DOMEntity'                => 'YoudsFrameworkXmlConfigDomEntity',
		'DOMEntityReference'       => 'YoudsFrameworkXmlConfigDomEntityReference',
		'DOMNode'                  => 'YoudsFrameworkXmlConfigDomNode',
		// 'DOMNotation'              => 'YoudsFrameworkXmlConfigDomNotation',
		'DOMProcessingInstruction' => 'YoudsFrameworkXmlConfigDomProcessingInstruction',
		'DOMText'                  => 'YoudsFrameworkXmlConfigDomText',
	);
	
	/**
	 * The constructor.
	 * Will auto-register YoudsFramework DOM node classes and create an XPath instance.
	 *
	...
	public function __construct($version = "1.0", $encoding = "UTF-8")
	{
		...
		foreach($this->nodeClassMap as $domClass => $youdsClass) {
			$this->registerNodeClass($domClass, $youdsClass);
		}
	...
	public function load($filename, $options = 0)
	{
		...
		if($this->isYoudsFrameworkConfiguration()) {
			YoudsFrameworkXmlConfigParser::registerYoudsFrameworkNamespaces($this);
		}
	...
	/**
	 * Check whether or not this is a standard YoudsFramework configuration file, i.e. with
	 * a <configurations> and <configuration> envelope.
	 *
	 * @return     bool true, if it is an YoudsFramework config structure, false otherwise.
	 *
	 * @author     David Zülke <dz@bitxtender.com>
	 * @since      1.0.0
	 */
	public function isYoudsFrameworkConfiguration()
	{
		return YoudsFrameworkXmlConfigParser::isYoudsFrameworkConfigurationDocument($this);
	}
	
	/**
	 * Retrieve the namespace of the YoudsFramework envelope.
	 *
	 * @return     string A namespace URI, or null if it's not an YoudsFramework config.
	 *
	 * @author     David Zülke <dz@bitxtender.com>
	 * @since      1.0.0
	 */
	public function getYoudsFrameworkEnvelopeNamespace()
	{
		if($this->isYoudsFrameworkConfiguration()) {
	...
	/**
	 * Method to retrieve a list of YoudsFramework <configuration> elements regardless of
	 * their namespace.
	 *
	 * @return     array A list of YoudsFrameworkXmlConfigDomElement elements.
	 *
	 * @author     David Zülke <dz@bitxtender.com>
	 * @since      1.0.0
	 */
	public function getConfigurationElements()
	{
		...
		if($this->isYoudsFrameworkConfiguration()) {
			$youdsNs = $this->getYoudsFrameworkEnvelopeNamespace();
			
			foreach($this->documentElement->childNodes as $node) {
				if($node->nodeType == XML_ELEMENT_NODE && $node->localName == 'configuration' && $node->namespaceURI == $youdsNs) {
					$retval[] = $node;
				}
			}
		}
	...
	/**
	 * Method to retrieve the YoudsFramework <sandbox> element regardless of the namespace.
	 *
	 * @return     YoudsFrameworkXmlConfigDomElement The <sandbox> element, or null.
	 *
	 * @author     David Zülke <dz@bitxtender.com>
	 * @since      1.0.0
	 */
	public function getSandbox()
	{
		if($this->isYoudsFrameworkConfiguration()) {
			...
			foreach($this->documentElement->childNodes as $node) {
				if($node->nodeType == XML_ELEMENT_NODE && $node->localName == 'sandbox' && $node->namespaceURI == $youdsNs) {
					return $node;
				}
			}
		}
