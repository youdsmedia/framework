 ...
 * @version    $Id$
 */
class YoudsFrameworkXmlConfigDomElement extends DOMElement implements IteratorAggregate
{
	/**
	 * __toString() magic method, returns the element value.
	 *
	 * @see        YoudsFrameworkXmlConfigDomElement::getValue()
	 *
	 ...
	 * {http://youds.com/youds/config/global/envelope/1.1}literalize into account
	...
	public function getLiteralValue()
	{
		...
		$preserveWhitespace = $this->getAttributeNS(YoudsFrameworkXmlConfigParser::NAMESPACE_XML_1998, 'space') == 'preserve';
		$literalize = YoudsFrameworkToolkit::literalize($this->getAttributeNS(YoudsFrameworkXmlConfigParser::NAMESPACE_AGAVI_ENVELOPE_LATEST, 'literalize')) !== false;
		
		if($literalize) {
			if($preserveWhitespace && ($trimmedValue === '' || $value != $trimmedValue)) {
				// we must preserve whitespace, and there is leading or trailing whitespace in the original value, so we won't run YoudsFrameworkToolkit::literalize(), which trims the input and then converts "true" to a boolean and so forth
				// however, we should still expand possible occurrences of config directives
				$value = YoudsFrameworkToolkit::expandDirectives($value);
			} else {
				// no need to preserve whitespace, or no leading/trailing whitespace, which means we can expand "true", "false" and so forth using YoudsFrameworkToolkit::literalize()
				$value = YoudsFrameworkToolkit::literalize($trimmedValue);
			}
	...
	protected function singularize($name)
	{
		...
		$names[count($names) - 1] = YoudsFrameworkInflector::singularize(end($names));
	...
	public function countChildren($name, $namespaceUri = null, $pluralMagic = false)
	{
		...
		// http://trac.youds.com/ticket/1039
		$marker = uniqid('', true);
		$this->setAttributeNS(YoudsFrameworkXmlConfigParser::NAMESPACE_AGAVI_ANNOTATIONS_LATEST, 'youds_annotations_latest:marker', $marker);
		...
		if($pluralMagic) {
			...
			if($namespaceUri) {
				$query = 'count(child::*[local-name() = "%2$s" and namespace-uri() = "%3$s" and ../@youds_annotations_latest:marker = "%4$s"]) + count(child::*[local-name() = "%1$s" and namespace-uri() = "%3$s" and ../@youds_annotations_latest:marker = "%4$s"]/*[local-name() = "%2$s" and namespace-uri() = "%3$s" and ../../@youds_annotations_latest:marker = "%4$s"])';
			} else {
				$query = 'count(%1$s[../@youds_annotations_latest:marker = "%4$s"]/%2$s[../../@youds_annotations_latest:marker = "%4$s"]) + count(%2$s[../@youds_annotations_latest:marker = "%4$s"])';
			}
		} else {
			if($namespaceUri) {
				$query = 'count(child::*[local-name() = "%1$s" and namespace-uri() = "%3$s" and ../@youds_annotations_latest:marker = "%4$s"])';
			} else {
				$query = 'count(%1$s[../@youds_annotations_latest:marker = "%4$s"])';
			}
		}
		
		$retval = (int)$this->ownerDocument->getXpath()->evaluate(sprintf($query, $name, $singularName, $namespaceUri, $marker), $this);
		
		$this->removeAttributeNS(YoudsFrameworkXmlConfigParser::NAMESPACE_AGAVI_ANNOTATIONS_LATEST, 'youds_annotations_latest:marker');
	...
	public function getChildren($name, $namespaceUri = null, $pluralMagic = false)
	{
		...
		if($pluralMagic) {
			...
			$singularName = $this->singularize($name);
			if($namespaceUri) {
				$query = 'child::*[local-name() = "%2$s" and namespace-uri() = "%3$s" and ../@youds_annotations_latest:marker = "%4$s"] | child::*[local-name() = "%1$s" and namespace-uri() = "%3$s" and ../@youds_annotations_latest:marker = "%4$s"]/*[local-name() = "%2$s" and namespace-uri() = "%3$s" and ../../@youds_annotations_latest:marker = "%4$s"]';
			} else {
				$query = '%1$s[../@youds_annotations_latest:marker = "%4$s"]/%2$s[../../@youds_annotations_latest:marker = "%4$s"] | %2$s[../@youds_annotations_latest:marker = "%4$s"]';
			}
		} else {
			if($namespaceUri) {
				$query = 'child::*[local-name() = "%1$s" and namespace-uri() = "%3$s" and ../@youds_annotations_latest:marker = "%4$s"]';
			} else {
				$query = '%1$s[../@youds_annotations_latest:marker = "%4$s"]';
			}
		}
	...
	public function getChild($name, $namespaceUri = null)
	{
		...
		if($namespaceUri) {
			$query = 'self::node()[count(child::*[local-name() = "%1$s" and namespace-uri() = "%2$s" and ../@youds_annotations_latest:marker = "%3$s"]) = 1]/*[local-name() = "%1$s" and namespace-uri() = "%2$s" and ../@youds_annotations_latest:marker = "%3$s"]';
		} else {
			$query = 'self::node()[count(child::%1$s[../@youds_annotations_latest:marker = "%3$s"]) = 1]/%1$s[../@youds_annotations_latest:marker = "%3$s"]';
		}
	...
	/**
	 * Check whether or not the element has YoudsFramework parameters as children.
	 *
	 ...
	 * @since      1.0.0
	 */
	public function hasYoudsFrameworkParameters()
	{
		if($this->ownerDocument->isYoudsFrameworkConfiguration()) {
			return $this->has('parameters', YoudsFrameworkXmlConfigParser::NAMESPACE_AGAVI_ENVELOPE_LATEST);
		}
	...
	/**
	 * Retrieve all of the YoudsFramework parameter elements associated with this
	 * element.
	 *
	 ...
	 * @since      1.0.0
	 */
	public function getYoudsFrameworkParameters(array $existing = array())
	{
		$result = $existing;
		$offset = 0;
		
		if($this->ownerDocument->isYoudsFrameworkConfiguration()) {
			$elements = $this->get('parameters', YoudsFrameworkXmlConfigParser::NAMESPACE_AGAVI_ENVELOPE_LATEST);
			...
			foreach($elements as $element) {
				...
				if($element->hasYoudsFrameworkParameters()) {
					$result[$key] = isset($result[$key]) && is_array($result[$key]) ? $result[$key] : array();
					$result[$key] = $element->getYoudsFrameworkParameters($result[$key]);
				} else {
