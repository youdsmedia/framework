...
<xsl:stylesheet
	...
	xmlns:config_handlers_1_1="http://youds.com/youds/config/parts/config_handlers/1.1"
>
	<xsl:output method="xml" version="1.0" encoding="utf-8" indent="yes" />
	
	<xsl:include href="_common.xsl" />
	
	<xsl:variable name="config_handlers_1_0" select="'http://youds.com/youds/config/parts/config_handlers/1.0'" />
	<xsl:variable name="config_handlers_1_1" select="'http://youds.com/youds/config/parts/config_handlers/1.1'" />
	...
	<!-- YoudsFramework 1.1 -->
	<!-- for YoudsFrameworkReturnArrayConfigHandler definitions, we need to copy the necessary transformation for YoudsFramework 1.1+ -->
	<xsl:template match="config_handlers_1_0:handler[@class = 'YoudsFrameworkReturnArrayConfigHandler' and not(.//config_handlers_1_0:transformation)]">
		<xsl:element name="{local-name()}" namespace="{$config_handlers_1_1}">
			<xsl:copy-of select="@*" />
			<config_handlers_1_1:transformation>%core.youds_dir%/config/xsl/rach.xsl</config_handlers_1_1:transformation>
			<xsl:apply-templates />
		</xsl:element>
	</xsl:template>
