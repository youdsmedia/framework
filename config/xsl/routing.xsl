...
<xsl:stylesheet
	...
	xmlns:routing_1_0="http://youds.com/youds/config/parts/routing/1.0"
	xmlns:routing_1_1="http://youds.com/youds/config/parts/routing/1.1"
>
	<xsl:output method="xml" version="1.0" encoding="utf-8" indent="yes" />
	
	<xsl:include href="_common.xsl" />
	
	<xsl:variable name="routing_1_0" select="'http://youds.com/youds/config/parts/routing/1.0'" />
	<xsl:variable name="routing_1_1" select="'http://youds.com/youds/config/parts/routing/1.1'" />
	...
	<xsl:template match="routing_1_0:*">
		<xsl:element name="{local-name()}" namespace="{$routing_1_1}">
			<xsl:copy-of select="@*" />
			<xsl:apply-templates />
		</xsl:element>
	</xsl:template>
