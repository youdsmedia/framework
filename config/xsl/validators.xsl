...
<xsl:stylesheet
	...
	xmlns:validators_1_0="http://youds.com/youds/config/parts/validators/1.0"
...
>
	...
	<xsl:variable name="validators_1_0" select="'http://youds.com/youds/config/parts/validators/1.0'" />
	<xsl:variable name="validators_1_1" select="'http://youds.com/youds/config/parts/validators/1.1'" />
	...
	<!-- we moved the default mappings ("string" => YoudsFrameworkStringValidator with "min"=1 etc) from the config handler to that file -->
	<xsl:template match="envelope_0_11:configurations[not(@parent)]">
		<xsl:call-template name="_common-migrate-envelope-element">
			<xsl:with-param name="attributes"><dummy parent="%core.youds_dir%/config/defaults/validators.xml" /></xsl:with-param>
		</xsl:call-template>
	</xsl:template>
	...
	<xsl:template match="validators_1_0:validator[validators_1_0:arguments[@base]]/@provides">
		<xsl:attribute name="provides"><xsl:value-of select="php:function('YoudsFrameworkDependencyManager::populateArgumentBaseKeyRefs', concat(../validators_1_0:arguments/@base,'[',.,']'))" /></xsl:attribute>
	</xsl:template>
	...
	<xsl:template match="validators_1_0:validator[validators_1_0:arguments[@base]]/@depends[substring(.,1,1)='[']">
		<xsl:attribute name="depends"><xsl:value-of select="php:function('YoudsFrameworkDependencyManager::populateArgumentBaseKeyRefs', concat(../validators_1_0:arguments/@base,.))" /></xsl:attribute>
	</xsl:template>
