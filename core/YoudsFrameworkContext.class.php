...
/**
 * YoudsFrameworkContext provides information about the current application context, 
...
class YoudsFrameworkContext
{
	...
	/**
	 * @var        YoudsFrameworkController A Controller instance.
	 */
	...
	/**
	 * @var        YoudsFrameworkDatabaseManager A DatabaseManager instance.
	 */
	protected $databaseManager = null;
	
	/**
	 * @var        YoudsFrameworkLoggerManager A LoggerManager instance.
	 */
	protected $loggerManager = null;
	
	/**
	 * @var        YoudsFrameworkRequest A Request instance.
	 */
	protected $request = null;
	
	/**
	 * @var        YoudsFrameworkRouting A Routing instance.
	 */
	protected $routing = null;
	
	/**
	 * @var        YoudsFrameworkStorage A Storage instance.
	 */
	protected $storage = null;
	
	/**
	 * @var        YoudsFrameworkTranslationManager A TranslationManager instance.
	 */
	protected $translationManager = null;
	
	/**
	 * @var        YoudsFrameworkUser A User instance.
	 */
	protected $user = null;
	...
	/**
	 * @var        array An array of YoudsFrameworkContext instances.
	 */
	...
	{
		trigger_error('Cloning an YoudsFrameworkContext instance is not allowed.', E_USER_ERROR);
	}
	 ...
	 *
	 * @see        YoudsFrameworkContext::getName()
	 *
	 * @author     David Zülke <dz@bitxtender.com>
	 * @since      0.11.0
	 */
	 ...
	 *
	 * @throws     YoudsFrameworkException If no such identifier exists.
	 *
	...
	public function createInstanceFor($for)
	{
		...
		if(null === $info) {
			throw new YoudsFrameworkException(sprintf('No factory info for "%s"', $for));
		}
	 ...
	 *
	 * @return     YoudsFrameworkController The current Controller implementation instance.
	 *
	 * @author     Sean Kerr <skerr@mojavi.org>
	 * @since      0.9.0
	 */
	 ...
	 *
	 * @return     YoudsFrameworkDatabaseManager The current DatabaseManager instance.
	 *
	 * @author     Sean Kerr <skerr@mojavi.org>
	 * @since      0.9.0
	 */
	...
	/**
	 * Retrieve the YoudsFrameworkContext instance.
	 *
	 ...
	 *
	 * @return     YoudsFrameworkContext An context instance initialized with the 
	...
	public static function getInstance($profile = null)
	{
		try {
			if($profile === null) {
				$profile = YoudsFrameworkConfig::get('core.default_context');
				if($profile === null) {
					throw new YoudsFrameworkException('You must supply a context name to YoudsFrameworkContext::getInstance() or set the name of the default context to be used in the configuration directive "core.default_context".');
				}
			}
			$profile = strtolower($profile);
			if(!isset(self::$instances[$profile])) {
				$class = YoudsFrameworkConfig::get('core.context_implementation', get_called_class());
	 ...
	 *
	 * @return     YoudsFrameworkLoggerManager The current LoggerManager implementation 
	 *                                instance.
	 *
	 * @author     David Zülke <dz@bitxtender.com>
	 * @since      0.11.0
	...
	/**
	 * (re)Initialize the YoudsFrameworkContext instance.
	 *
	 * @author     Dominik del Bondio <ddb@bitxtender.com>
	 * @author     David Zülke <dz@bitxtender.com>
	 * @author     Mike Vincent <mike@youds.com>
	...
	{
		try {
			include(YoudsFrameworkConfigCache::checkConfig(YoudsFrameworkConfig::get('core.config_dir') . '/factories.xml', $this->name));
		} catch(Exception $e) {
			YoudsFrameworkException::render($e, $this);
		}
	...
	/**
	 * Shut down this YoudsFrameworkContext and all related factories.
	 *
	 * @author     David Zülke <dz@bitxtender.com>
	 * @since      0.11.0
	 */
	 ...
	 *                    the constructor.
	 *
	 * @return     YoudsFrameworkModel A Model implementation instance.
	 *
	 * @throws     YoudsFrameworkAutloadException if class is ultimately not found.
	 *
	 * @author     David Zülke <dz@bitxtender.com>
	 * @since      0.11.0
	 */
	public function getModel($modelName, $moduleName = null, array $parameters = null)
	{
		...
		if($moduleName === null) {
			...
			if(!class_exists($class)) {
				...
				$file = YoudsFrameworkConfig::get('core.model_dir') . '/' . $modelName . 'Model.class.php';
			}
		} else {
			try {
		...
		if(!class_exists($class)) {
			// it's not there. 
			throw new YoudsFrameworkAutoloadException(sprintf("Couldn't find class for Model %s", $origModelName));
		}
		...
		if($rc->implementsInterface('YoudsFrameworkISingletonModel')) {
			// it's a singleton
	 ...
	 *
	 * @return     YoudsFrameworkRequest The current Request implementation instance.
	 *
	 * @author     Sean Kerr <skerr@mojavi.org>
	 * @since      0.9.0
	 */
	 ...
	 *
	 * @return     YoudsFrameworkRouting The current Routing implementation instance.
	 *
	 * @author     Dominik del Bondio <ddb@bitxtender.com>
	 * @since      0.11.0
	 */
	 ...
	 *
	 * @return     YoudsFrameworkStorage The current Storage implementation instance.
	 *
	 * @author     Sean Kerr <skerr@mojavi.org>
	 * @since      0.9.0
	 */
	 ...
	 *
	 * @return     YoudsFrameworkTranslationManager The current TranslationManager
	 ...
	 * Retrieve the user.
	 *
	 * @return     YoudsFrameworkUser The current User implementation instance.
	 *
	 * @author     Sean Kerr <skerr@mojavi.org>
	 * @since      0.9.0
	 */
