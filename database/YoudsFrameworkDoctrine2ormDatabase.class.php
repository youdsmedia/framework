 ...
 * referencing an YoudsFrameworkDoctrine2dbalDatabase connection.
 * 
 ...
 *                  the name (string) of a configured YoudsFrameworkDoctrine2dbalDatabase
 ...
 * @version    $Id$
 */
class YoudsFrameworkDoctrine2ormDatabase extends YoudsFrameworkDoctrine2Database
{
	/**
	...
	public function connect()
	{
		...
		if(is_string($connection)) {
			...
			} catch(YoudsFrameworkDatabaseException $e) {
				throw new YoudsFrameworkDatabaseException(sprintf('YoudsFrameworkDoctrine2dbalDatabase connection "%s" configured for use in YoudsFrameworkDoctrine2ormDatabase "%s" could not be found.', $connection, $this->getName()), 0, $e);
			}
			try {
				$connection = $connection->getConnection();
			} catch(YoudsFrameworkDatabaseException $e) {
				throw new YoudsFrameworkDatabaseException(sprintf("YoudsFrameworkDoctrine2dbalDatabase connection '%s' configured for use in YoudsFrameworkDoctrine2ormDatabase '%s' could not be initialized:\n\n%s", $this->getParameter('connection'), $this->getName(), $e->getMessage()), 0, $e);
			}
		} elseif(!is_array($connection)) {
			throw new YoudsFrameworkDatabaseException('YoudsFrameworkDoctrine2ormDatabase expects configuration parameter "connection" to be an array containing connection details or a string with the name of an YoudsFrameworkDoctrine2dbalDatabase to use.');
		}
		...
		try {
			...
			throw new YoudsFrameworkDatabaseException(sprintf("Failed to create Doctrine\ORM\EntityManager for connection '%s':\n\n%s", $this->getName(), $e->getMessage()), 0, $e);
		}
	}
	...
	protected function prepareConfiguration(\Doctrine\DBAL\Configuration $config)
	{
		...
		$config->setAutoGenerateProxyClasses($this->getParameter('configuration[auto_generate_proxy_classes]', YoudsFrameworkConfig::get('core.debug')));
		...
		$config->setProxyNamespace($this->getParameter('configuration[proxy_namespace]', 'YoudsFrameworkDoctrine2ormDatabase_Proxy_' . preg_replace('#\W#', '_', $this->getName())));
		$config->setProxyDir($this->getParameter('configuration[proxy_dir]', YoudsFrameworkConfig::get('core.cache_dir')));
		
		// unless configured differently, use ArrayCache in debug mode and APC (if available) otherwise
		if(YoudsFrameworkConfig::get('core.debug') || !extension_loaded('apc')) {
