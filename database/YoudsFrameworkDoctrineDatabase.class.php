 ...
 * @version    $Id$
 */
class YoudsFrameworkDoctrineDatabase extends YoudsFrameworkDatabase
{
	/**
	 ...
	 *
	 * @throws     <b>YoudsFrameworkDatabaseException</b> If no resource could be retrieved
	 *
	 * @author     David Zülke <dz@bitxtender.com>
	 * @since      0.11.0
	 */
	 ...
	 *
	 * @param      YoudsFrameworkDatabaseManager The database manager of this instance.
	 ...
	 * @since      0.11.0
	 */
	public function initialize(YoudsFrameworkDatabaseManager $databaseManager, array $parameters = array())
	{
		...
		if(!$is12) {
		  trigger_error('Support for Doctrine versions older than 1.2 is deprecated and will be removed in YoudsFramework 1.2.', E_USER_DEPRECATED);
		}
		...
		try {
			...
			$cel = $this->getParameter('connection_event_listener_class', 'YoudsFrameworkDoctrineDatabaseEventListener');
			$this->connection->setListener(new $cel($this));
			
			// set the context instance as a connection parameter
			$this->connection->setParam('context', $databaseManager->getContext(), 'org.youds');
			...
			throw new YoudsFrameworkDatabaseException($e->getMessage(), 0, $e);
		}
	}
