...
/**
 * YoudsFrameworkPdoDatabase provides connectivity for the PDO database API layer.
 *
 * @package    youds
 * @subpackage database
 *
 ...
 * @version    $Id$
 */
class YoudsFrameworkPdoDatabase extends YoudsFrameworkDatabase
{
	/**
	...
	public function initialize(YoudsFrameworkDatabaseManager $databaseManager, array $parameters = array())
	{
		...
		if($this->getParameter('warn_mysql_charset', true) && strpos($this->getParameter('dsn'), 'mysql:') === 0) {
			if($matches = preg_grep('/^\s*SET\s+NAMES\b/i', (array)$this->getParameter('init_queries'))) {
				throw new YoudsFrameworkDatabaseException(sprintf(
