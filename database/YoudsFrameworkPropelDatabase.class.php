...
/**
 * An YoudsFramework Database driver for Propel. Supports Propel 1.3 and later.
 * 
 ...
 * @version    $Id$
 */
class YoudsFrameworkPropelDatabase extends YoudsFrameworkDatabase
{
	/**
	...
	public function initialize(YoudsFrameworkDatabaseManager $databaseManager, array $parameters = array())
	{
		...
		$configPath = YoudsFrameworkToolkit::expandDirectives($this->getParameter('config'));
		...
		if($datasource === null || $datasource == 'default') {
			if(isset($config['propel']['datasources']['default'])) {
				...
				throw new YoudsFrameworkDatabaseException('No datasource given for Propel connection, and no default datasource specified in runtime configuration file.');
			}
		}
		...
		if($is13) {
			// old-style config array; PropelConfiguration was added after 1.3.0, http://trac.youds.com/ticket/1195
			...
			// array cast because "query" might be a string if just one init query was given, http://trac.youds.com/ticket/1194
