 ...
 * @version    $Id$
 */
abstract class YoudsFrameworkCalendar
{
	const GREGORIAN = 'gregorian';

	/**
	 * @var        YoudsFrameworkTranslationManager The translation manager instance.
	 * @since      0.11.0
	 */
	 ...
	 *
	 * @return     YoudsFrameworkTranslationManager The translation manager.
	 *
	 * @author     Dominik del Bondio <ddb@bitxtender.com>
	 * @since      0.11.0
	 */
	 ...
	 *
	 * @param      YoudsFrameworkTimeZone The timezone to use.
	 * @param      YoudsFrameworkLocale   The locale to use.
	 *
	 * @author     Dominik del Bondio <ddb@bitxtender.com>
	 * @author     The ICU Project
	 * @since      0.11.0
	 */
	protected function constructorOO(YoudsFrameworkTimeZone $zone, YoudsFrameworkLocale $locale)
	{
	...
	protected static $kCalendarLimits = array(
		...
		array(                 0,                  0,YoudsFrameworkDateDefinitions::MILLIS_PER_HOUR , YoudsFrameworkDateDefinitions::MILLIS_PER_HOUR ), // DST_OFFSET
	...
	{
		return time() * YoudsFrameworkDateDefinitions::MILLIS_PER_SECOND;
	}
	...
	public function getNativeDateTime()
	{
		$dateTimeString = sprintf(
			'%d-%d-%d %d:%d:%d',
			$this->get(YoudsFrameworkDateDefinitions::YEAR), $this->get(YoudsFrameworkDateDefinitions::MONTH) + 1, $this->get(YoudsFrameworkDateDefinitions::DATE),
			$this->get(YoudsFrameworkDateDefinitions::HOUR_OF_DAY), $this->get(YoudsFrameworkDateDefinitions::MINUTE), $this->get(YoudsFrameworkDateDefinitions::SECOND)
		);
	...
	{
		$unixTime = floor($this->getTimeInMillis() / YoudsFrameworkDateDefinitions::MILLIS_PER_SECOND);
		$unixTimeInt = (int) $unixTime;
		// lets check if the int can't represent the time anymore
		if($unixTime != $unixTimeInt) {
			throw new OverflowException('cannot convert the date ' . $this->get(YoudsFrameworkDateDefinitions::YEAR) . '/' . $this->get(YoudsFrameworkDateDefinitions::MONTH) . '/' . $this->get(YoudsFrameworkDateDefinitions::DATE) . ' into a unix timestamp');
		}
		return $unixTimeInt;
	}
	...
	{
		$this->setTimeInMillis($timestamp * YoudsFrameworkDateDefinitions::MILLIS_PER_SECOND);
	}

	/**
	 * @see        YoudsFrameworkCalendar::getAll()
	 *
	 ...
	 *
	 * @param      array An array using the YoudsFrameworkDateDefinitions::XXX as key 
	...
	{
		return YoudsFrameworkLocale::getTimeZoneOptionString($this, $prefix);
	}

	public function __is_equal(YoudsFrameworkCalendar $that)
	{
		return $this->isEquivalentTo($that) && $this->getTimeInMillis() == $that->getTimeInMillis();
	}

	public function __is_not_equal(YoudsFrameworkCalendar $that)
	{
		return !$this->isEquivalentTo($that) || $this->getTimeInMillis() != $that->getTimeInMillis();
	}
	 ...
	 *
	 * @param      YoudsFrameworkCalendar the Calendar to be compared with this Calendar
	 *
	 * @return     bool
	 *
	 * @author     Dominik del Bondio <ddb@bitxtender.com>
	 ...
	 * @since      0.11.0
	 */
	public function isEquivalentTo(YoudsFrameworkCalendar $other)
	{
	 ...
	 * @since      0.11.0
	 */
	public function equals(YoudsFrameworkCalendar $when)
	{
	 ...
	 * @since      0.11.0
	 */
	public function before(YoudsFrameworkCalendar $when)
	{
	 ...
	 * @since      0.11.0
	 */
	public function after(YoudsFrameworkCalendar $when)
	{
		return ($this->__is_not_equal($when) && $this->getTimeInMillis() > $when->getTimeInMillis());
	}
	 ...
	 * add(YoudsFrameworkDateDefinitions::DATE, -5). When adding on the month or
	 * YoudsFrameworkDateDefinitions::DATE field, other fields like date might conflict
	...
	public function add($field, $amount)
	{
		...
		switch($field) {
			case YoudsFrameworkDateDefinitions::ERA:
				...
				$this->pinField(YoudsFrameworkDateDefinitions::ERA);
				return;
			...
			case YoudsFrameworkDateDefinitions::MONTH:
				$this->set($field, $this->get($field) + $amount);
				$this->pinField(YoudsFrameworkDateDefinitions::DAY_OF_MONTH);
				return;
			...
			case YoudsFrameworkDateDefinitions::DAY_OF_WEEK_IN_MONTH:
				$delta *= YoudsFrameworkDateDefinitions::MILLIS_PER_WEEK;
				break;

			case YoudsFrameworkDateDefinitions::AM_PM:
				$delta *= 12 * YoudsFrameworkDateDefinitions::MILLIS_PER_HOUR;
				break;

			case YoudsFrameworkDateDefinitions::DAY_OF_MONTH:
			...
			case YoudsFrameworkDateDefinitions::JULIAN_DAY:
				$delta *= YoudsFrameworkDateDefinitions::MILLIS_PER_DAY;
				break;

			case YoudsFrameworkDateDefinitions::HOUR_OF_DAY:
			case YoudsFrameworkDateDefinitions::HOUR:
				$delta *= YoudsFrameworkDateDefinitions::MILLIS_PER_HOUR;
				$keepHourInvariant = false;
				break;

			case YoudsFrameworkDateDefinitions::MINUTE:
				$delta *= YoudsFrameworkDateDefinitions::MILLIS_PER_MINUTE;
				$keepHourInvariant = false;
				break;

			case YoudsFrameworkDateDefinitions::SECOND:
				$delta *= YoudsFrameworkDateDefinitions::MILLIS_PER_SECOND;
				$keepHourInvariant = false;
				break;
			...
			default:
				throw new InvalidArgumentException('YoudsFrameworkCalendar::add(): field ' . $field . ' is not supported');
				return;
		}
		...
		if($keepHourInvariant) {
			$dst = $this->get(YoudsFrameworkDateDefinitions::DST_OFFSET);
			$hour = $this->internalGet(YoudsFrameworkDateDefinitions::HOUR_OF_DAY);
		}
		...
		if($keepHourInvariant) {
			...
			if($dst != 0) {
				...
				if($this->get(YoudsFrameworkDateDefinitions::HOUR_OF_DAY) != $hour) {
	 ...
	 * roll(YoudsFrameworkDateDefinitions::DATE, +1). When rolling on the month or
	 * YoudsFrameworkDateDefinitions::MONTH field, other fields like date might
	 ...
	 * hour-in-day or YoudsFrameworkDateDefinitions::HOUR_OF_DAY field, it will
	...
	public function roll($field, $amount)
	{
		...
		switch($field) {
			...
			case YoudsFrameworkDateDefinitions::HOUR_OF_DAY:
				...
				// the time in millis directly.
				{
					...
					$this->setTimeInMillis($start + YoudsFrameworkDateDefinitions::MILLIS_PER_HOUR * ($newHour - $oldHour));
					return;
				}
			...
			case YoudsFrameworkDateDefinitions::MONTH:
				...
				{
						$max = $this->getActualMaximum(YoudsFrameworkDateDefinitions::MONTH);
						$mon = ($this->internalGet(YoudsFrameworkDateDefinitions::MONTH) + $amount) % ($max + 1);

						if($mon < 0) {
							$mon += ($max + 1);
						}
						$this->set(YoudsFrameworkDateDefinitions::MONTH, $mon);
			...
			case YoudsFrameworkDateDefinitions::EXTENDED_YEAR:
				...
				$this->pinField(YoudsFrameworkDateDefinitions::MONTH);
				$this->pinField(YoudsFrameworkDateDefinitions::DAY_OF_MONTH);
				return;
			...
			case YoudsFrameworkDateDefinitions::WEEK_OF_MONTH:
				{
						...
						$dow = $this->internalGet(YoudsFrameworkDateDefinitions::DAY_OF_WEEK) - $this->getFirstDayOfWeek();
						if($dow < 0) {
							$dow += 7;
						}
						...
						// of the month.
						$fdm = ($dow - $this->internalGet(YoudsFrameworkDateDefinitions::DAY_OF_MONTH) + 1) % 7;
						if($fdm < 0) {
							$fdm += 7;
						}
						...
						// day of the month.
						$monthLen = $this->getActualMaximum(YoudsFrameworkDateDefinitions::DAY_OF_MONTH);
						$ldm = ($monthLen - $this->internalGet(YoudsFrameworkDateDefinitions::DAY_OF_MONTH) + $dow) % 7;
						...
						$gap = $limit - $start;
						$day_of_month = ($this->internalGet(YoudsFrameworkDateDefinitions::DAY_OF_MONTH) + $amount * 7 - $start) % $gap;
						if($day_of_month < 0) {
							$day_of_month += $gap;
						}
						$day_of_month += $start;
						...
						// to.
						$this->set(YoudsFrameworkDateDefinitions::DAY_OF_MONTH, $day_of_month);
						return;
				}
			case YoudsFrameworkDateDefinitions::WEEK_OF_YEAR:
				{
					...
					// Find the day of the week (normalized for locale) for the first
					// of the year.
					$fdy = ($dow - $this->internalGet(YoudsFrameworkDateDefinitions::DAY_OF_YEAR) + 1) % 7;
					if($fdy < 0) {
						$fdy += 7;
					}
					...
					// Get the day of the week (normalized for locale) for the last
					// day of the year.
					$yearLen = $this->getActualMaximum(YoudsFrameworkDateDefinitions::DAY_OF_YEAR);
					$ldy = ($yearLen - $this->internalGet(YoudsFrameworkDateDefinitions::DAY_OF_YEAR) + $dow) % 7;
					...
					// Now roll between start and (limit - 1).
					$gap = $limit - $start;
					$day_of_year = ($this->internalGet(YoudsFrameworkDateDefinitions::DAY_OF_YEAR) + $amount * 7 - $start) % $gap;
					if($day_of_year < 0) {
						$day_of_year += $gap;
					}
					$day_of_year += $start;
					...
					$this->set(YoudsFrameworkDateDefinitions::DAY_OF_YEAR, $day_of_year);
					$this->clear(YoudsFrameworkDateDefinitions::MONTH);
					return;
				}
			case YoudsFrameworkDateDefinitions::DAY_OF_YEAR:
				{
					...
					$delta = (float) ($amount * YoudsFrameworkDateDefinitions::MILLIS_PER_DAY); // Scale up from days to millis
					$min2 = (float) ($this->internalGet(YoudsFrameworkDateDefinitions::DAY_OF_YEAR) - 1);
					$min2 *= YoudsFrameworkDateDefinitions::MILLIS_PER_DAY;
					$min2 = $this->internalGetTime() - $min2;

					$yearLength = (float) $this->getActualMaximum(YoudsFrameworkDateDefinitions::DAY_OF_YEAR);
					$oneYear = $yearLength;
					$oneYear *= YoudsFrameworkDateDefinitions::MILLIS_PER_DAY;
			...
			case YoudsFrameworkDateDefinitions::DOW_LOCAL:
				{
					...
					$leadDays -= ($field == YoudsFrameworkDateDefinitions::DAY_OF_WEEK) ? $this->getFirstDayOfWeek() : 1;
					if($leadDays < 0) {
						$leadDays += 7;
					}
					$min2 = (float) ($this->internalGetTime() - $leadDays * YoudsFrameworkDateDefinitions::MILLIS_PER_DAY);
					$newtime = fmod(($this->internalGetTime() + $delta - $min2), YoudsFrameworkDateDefinitions::MILLIS_PER_WEEK);
					if($newtime < 0) {
						$newtime += YoudsFrameworkDateDefinitions::MILLIS_PER_WEEK;
					}
					$this->setTimeInMillis($newtime + $min2);
					return;
				}
			case YoudsFrameworkDateDefinitions::DAY_OF_WEEK_IN_MONTH:
				{
					...
					$delta = (float) ($amount * YoudsFrameworkDateDefinitions::MILLIS_PER_WEEK); // Scale up from weeks to millis
					// Find the number of same days of the week before this one
					// in this month.
					$preWeeks = (int) (($this->internalGet(YoudsFrameworkDateDefinitions::DAY_OF_MONTH) - 1) / 7);
					// Find the number of same days of the week after this one
					// in this month.
					$postWeeks = (int) (($this->getActualMaximum(YoudsFrameworkDateDefinitions::DAY_OF_MONTH) - $this->internalGet(YoudsFrameworkDateDefinitions::DAY_OF_MONTH)) / 7);
					// From these compute the min and gap millis for rolling.
					$min2 = (float) ($this->internalGetTime() - $preWeeks * YoudsFrameworkDateDefinitions::MILLIS_PER_WEEK);
					$gap2 = (float) (YoudsFrameworkDateDefinitions::MILLIS_PER_WEEK * ($preWeeks + $postWeeks + 1)); // Must add 1!
		...
		default:
				...
				throw new InvalidArgumentException('YoudsFrameworkCalendar::roll(): field ' . $field . ' cannot be rolled with this method');
		}
	}
	 ...
	 * YoudsFrameworkDateDefinitions::MONTH)</code> returns 3, then this calendar is set to
	 ...
	 *
	 * <pre>
	 * $y = $cal->fieldDifference($when, YoudsFrameworkDateDefinitions::YEAR);
	 * $m = $cal->fieldDifference($when, YoudsFrameworkDateDefinitions::MONTH);
	 * $d = $cal->fieldDifference($when, YoudsFrameworkDateDefinitions::DATE);</pre>
	 *
	 ...
	 * <pre>
	 * $cal->setTime($date1);
	 * $m1 = $cal->fieldDifference($date2, YoudsFrameworkDateDefinitions::MONTH);
	 * $d1 = $cal->fieldDifference($date2, YoudsFrameworkDateDefinitions::DATE);
	 * $cal->setTime($date2);
	 * $m2 = $cal->fieldDifference($date1, YoudsFrameworkDateDefinitions::MONTH);
	 * $d2 = $cal->fieldDifference($date1, YoudsFrameworkDateDefinitions::DATE);</pre>
	 *
	 ...
	 *
	 * @param      float|YoudsFrameworkCalendar     When the date to compare this 
	 *                                     calendar's time to
	...
	public function fieldDifference($targetMs /* $when */, $field)
	{
		...
		if($targetMs instanceof YoudsFrameworkCalendar) {
	 ...
	 *
	 * @param      YoudsFrameworkTimeZone The given time zone. If null is passed nothing
	 *                           is done.
	 *
	 * @author     Dominik del Bondio <ddb@bitxtender.com>
	 * @author     The ICU Project
	 * @since      0.11.0
	 */
	public function setTimeZone(YoudsFrameworkTimeZone $zone = null)
	{
	 ...
	 *
	 * @return     YoudsFrameworkTimeZone The time zone object associated with this
	 *                           calendar.
	 *
	 * @author     Dominik del Bondio <ddb@bitxtender.com>
	 * @author     The ICU Project
	 ...
	 * more efficient way to accomplish this (in most cases, you can simply return
	 * getMaximum()). YoudsFrameworkGregorianCalendar overrides this function with a more
	 * efficient implementation.
	 *
	...
	public function getActualMaximum($field)
	{
		...
		switch($field) {
			case YoudsFrameworkDateDefinitions::DATE:
				...
				$result = $this->handleGetMonthLength($cal->get(YoudsFrameworkDateDefinitions::EXTENDED_YEAR), $cal->get(YoudsFrameworkDateDefinitions::MONTH));
				break;

			case YoudsFrameworkDateDefinitions::DAY_OF_YEAR:
				$cal = clone $this;
				$cal->prepareGetActual($field, false);
				$result = $this->handleGetYearLength($cal->get(YoudsFrameworkDateDefinitions::EXTENDED_YEAR));
				break;
	 ...
	 * Overloaded
	 * 
	 * @see        YoudsFrameworkCalendar::set1()
	 * @see        YoudsFrameworkCalendar::set2()
	 * @see        YoudsFrameworkCalendar::set3()
	 * @see        YoudsFrameworkCalendar::set4()
	 *
	 * @author     Dominik del Bondio <ddb@bitxtender.com>
	 * @author     The ICU Project
	 * @since      0.11.0
	...
	public function set()
	{
		...
		$fName = YoudsFrameworkToolkit::overloadHelper(array(
			array('name' => 'set1',
	...
	{
		$this->set1(YoudsFrameworkDateDefinitions::YEAR, $year);
		$this->set1(YoudsFrameworkDateDefinitions::MONTH, $month);
		$this->set1(YoudsFrameworkDateDefinitions::DATE, $date);
	}
	...
	public function set3($year, $month, $date, $hour, $minute)
	{
		...
		$this->set1(YoudsFrameworkDateDefinitions::MONTH, $month);
		$this->set1(YoudsFrameworkDateDefinitions::DATE, $date);
		$this->set1(YoudsFrameworkDateDefinitions::HOUR_OF_DAY, $hour);
		$this->set1(YoudsFrameworkDateDefinitions::MINUTE, $minute);
	}
	...
	public function set4($year, $month, $date, $hour, $minute, $second)
	{
		...
		$this->set1(YoudsFrameworkDateDefinitions::HOUR_OF_DAY, $hour);
		$this->set1(YoudsFrameworkDateDefinitions::MINUTE, $minute);
		$this->set1(YoudsFrameworkDateDefinitions::SECOND, $second);
	}

	/**
	 * Overloaded
	 *
	 * @see        YoudsFrameworkCalendar::clear1()
	 * @see        YoudsFrameworkCalendar::clear2()
	 * 
	 * @author     Dominik del Bondio <ddb@bitxtender.com>
	 * @author     The ICU Project
	 * @since      0.11.0
	...
	protected function computeTime()
	{
		...
		$millis = YoudsFrameworkCalendarGrego::julianDayToMillis($julianDay);
		...
		if($this->fStamp[YoudsFrameworkDateDefinitions::MILLISECONDS_IN_DAY] >= (self::kMinimumUserStamp) &&
				$this->newestStamp(YoudsFrameworkDateDefinitions::AM_PM, YoudsFrameworkDateDefinitions::MILLISECOND, self::kUnset) <= $this->fStamp[YoudsFrameworkDateDefinitions::MILLISECONDS_IN_DAY]) {
			$millisInDay = $this->internalGet(YoudsFrameworkDateDefinitions::MILLISECONDS_IN_DAY);
		} else {
		...
		if($this->fStamp[YoudsFrameworkDateDefinitions::ZONE_OFFSET] >= (self::kMinimumUserStamp) ||
				$this->fStamp[YoudsFrameworkDateDefinitions::DST_OFFSET] >= (self::kMinimumUserStamp)) {
			$millisInDay -= $this->internalGet(YoudsFrameworkDateDefinitions::ZONE_OFFSET) + $this->internalGet(YoudsFrameworkDateDefinitions::DST_OFFSET);
		} else {
	...
	protected function computeFields()
	{
		...
		$mask =   //fInternalSetMask;
			(1 << YoudsFrameworkDateDefinitions::ERA) |
			(1 << YoudsFrameworkDateDefinitions::YEAR) |
			(1 << YoudsFrameworkDateDefinitions::MONTH) |
			(1 << YoudsFrameworkDateDefinitions::DAY_OF_MONTH) | // = UCAL_DATE
			(1 << YoudsFrameworkDateDefinitions::DAY_OF_YEAR) |
			(1 << YoudsFrameworkDateDefinitions::EXTENDED_YEAR);
		...
		$days = floor($localMillis / YoudsFrameworkDateDefinitions::MILLIS_PER_DAY);

		$this->internalSet(YoudsFrameworkDateDefinitions::JULIAN_DAY, $days + YoudsFrameworkDateDefinitions::EPOCH_START_AS_JULIAN_DAY);

		// In some cases we will have to call this method again below to
		// adjust for DST pushing us into the next Julian day.
		$this->computeGregorianAndDOWFields($this->fFields[YoudsFrameworkDateDefinitions::JULIAN_DAY]);

		$millisInDay =  (int) ($localMillis - ($days * YoudsFrameworkDateDefinitions::MILLIS_PER_DAY));
		if($millisInDay < 0) {
			$millisInDay += (int) YoudsFrameworkDateDefinitions::MILLIS_PER_DAY;
		}
		...
		if($millisInDay >= (int) YoudsFrameworkDateDefinitions::MILLIS_PER_DAY) {
			$millisInDay -= (int) YoudsFrameworkDateDefinitions::MILLIS_PER_DAY; // ASSUME dstOffset < 24:00
		...
		$this->handleComputeFields($this->fFields[YoudsFrameworkDateDefinitions::JULIAN_DAY]);
		...
		$this->fFields[YoudsFrameworkDateDefinitions::MILLISECONDS_IN_DAY] = $millisInDay;
		$this->fFields[YoudsFrameworkDateDefinitions::MILLISECOND] = $millisInDay % 1000;
		$millisInDay /= 1000;
		$this->fFields[YoudsFrameworkDateDefinitions::SECOND] = $millisInDay % 60;
		$millisInDay /= 60;
		$this->fFields[YoudsFrameworkDateDefinitions::MINUTE] = $millisInDay % 60;
		$millisInDay /= 60;
		$this->fFields[YoudsFrameworkDateDefinitions::HOUR_OF_DAY] = (int) $millisInDay;
		$this->fFields[YoudsFrameworkDateDefinitions::AM_PM] = (int) ($millisInDay / 12); // Assume AM == 0
		$this->fFields[YoudsFrameworkDateDefinitions::HOUR] = $millisInDay % 12;
		$this->fFields[YoudsFrameworkDateDefinitions::ZONE_OFFSET] = $rawOffset;
		$this->fFields[YoudsFrameworkDateDefinitions::DST_OFFSET] = $dstOffset;
	}
	...
	{
		$this->set(YoudsFrameworkDateDefinitions::MILLISECONDS_IN_DAY, 0);

		switch($field) {
			case YoudsFrameworkDateDefinitions::YEAR:
			case YoudsFrameworkDateDefinitions::YEAR_WOY:
			case YoudsFrameworkDateDefinitions::EXTENDED_YEAR:
				$this->set(YoudsFrameworkDateDefinitions::DAY_OF_YEAR, $this->getGreatestMinimum(YoudsFrameworkDateDefinitions::DAY_OF_YEAR));
				break;

			case YoudsFrameworkDateDefinitions::MONTH:
				$this->set(YoudsFrameworkDateDefinitions::DATE, $this->getGreatestMinimum(YoudsFrameworkDateDefinitions::DATE));
				break;

			case YoudsFrameworkDateDefinitions::DAY_OF_WEEK_IN_MONTH:
				// For dowim, the maximum occurs for the DOW of the first of the
				// month.
				$this->set(YoudsFrameworkDateDefinitions::DATE, 1);
				$this->set(YoudsFrameworkDateDefinitions::DAY_OF_WEEK, $this->get(YoudsFrameworkDateDefinitions::DAY_OF_WEEK)); // Make this user set
				break;
			...
			case YoudsFrameworkDateDefinitions::WEEK_OF_YEAR:
				...
				if($isMinimum) {
					...
					if($dow < YoudsFrameworkDateDefinitions::SUNDAY) {
						$dow += 7;
					}
				}

				$this->set(YoudsFrameworkDateDefinitions::DAY_OF_WEEK, $dow);
				break;
		default:
				;
		}
	 ...
	 * following fields:
	 *
	 * <pre>YoudsFrameworkDateDefinitions::ERA
	 * YoudsFrameworkDateDefinitions::YEAR
	 * YoudsFrameworkDateDefinitions::MONTH
	 * YoudsFrameworkDateDefinitions::WEEK_OF_YEAR
	 * YoudsFrameworkDateDefinitions::WEEK_OF_MONTH
	 * YoudsFrameworkDateDefinitions::DATE
	 * YoudsFrameworkDateDefinitions::DAY_OF_YEAR
	 * YoudsFrameworkDateDefinitions::DAY_OF_WEEK_IN_MONTH
	 * YoudsFrameworkDateDefinitions::YEAR_WOY
	 * YoudsFrameworkDateDefinitions::EXTENDED_YEAR</pre>
	 *
	...
	{
		$useMonth = ($bestField == YoudsFrameworkDateDefinitions::DAY_OF_MONTH ||
									$bestField == YoudsFrameworkDateDefinitions::WEEK_OF_MONTH ||
									$bestField == YoudsFrameworkDateDefinitions::DAY_OF_WEEK_IN_MONTH);
		$year = 0;

		if($bestField == YoudsFrameworkDateDefinitions::WEEK_OF_YEAR) {
			$year = $this->internalGet(YoudsFrameworkDateDefinitions::YEAR_WOY, $this->handleGetExtendedYear());
			$this->internalSet(YoudsFrameworkDateDefinitions::EXTENDED_YEAR, $year);
		} else {
		...
		if($this->_isSet(YoudsFrameworkDateDefinitions::MONTH)) {
			$month = $this->internalGet(YoudsFrameworkDateDefinitions::MONTH);
		} else {
		...
		if($bestField == YoudsFrameworkDateDefinitions::DAY_OF_MONTH) {

			// give calendar subclass a chance to have a default 'first' dom
			$dayOfMonth = 0;
			if($this->_isSet(YoudsFrameworkDateDefinitions::DAY_OF_MONTH)) {
				$dayOfMonth = $this->internalGet(YoudsFrameworkDateDefinitions::DAY_OF_MONTH, 1);
			} else {
		...
		if($bestField == YoudsFrameworkDateDefinitions::DAY_OF_YEAR) {
			return $julianDay + $this->internalGet(YoudsFrameworkDateDefinitions::DAY_OF_YEAR);
		}
		...
		if($bestField == YoudsFrameworkDateDefinitions::DAY_OF_WEEK_IN_MONTH) {
			...
			// negative.
			$dim = $this->internalGet(YoudsFrameworkDateDefinitions::DAY_OF_WEEK_IN_MONTH, 1);
			if($dim >= 0) {
				$date += 7 * ($dim - 1);
			} else {
				...
				$m = $this->internalGet(YoudsFrameworkDateDefinitions::MONTH, YoudsFrameworkDateDefinitions::JANUARY);
				$monthLength = $this->handleGetMonthLength($year, $m);
				$date += ((int)(($monthLength - $date) / 7) + $dim + 1) * 7;
			}
		} else {
			if($bestField == YoudsFrameworkDateDefinitions::WEEK_OF_YEAR) {  // ------------------------------------- WOY -------------
				if(
					!$this->_isSet(YoudsFrameworkDateDefinitions::YEAR_WOY) ||  // YWOY not set at all or
					(
						($this->resolveFields(self::$kYearPrecedence) != YoudsFrameworkDateDefinitions::YEAR_WOY) // YWOY doesn't have precedence
						&& ($this->fStamp[YoudsFrameworkDateDefinitions::YEAR_WOY] != self::kInternallySet) // (excluding where all fields are internally set - then YWOY is used)
					)
				) {
	...
	protected function handleGetExtendedYearFromWeekFields($yearWoy, $woy)
	{
		...
		switch($bestField) {
			...
			case YoudsFrameworkDateDefinitions::DATE:
				if(($this->internalGet(YoudsFrameworkDateDefinitions::MONTH)==0) &&
						($woy >= $this->getLeastMaximum(YoudsFrameworkDateDefinitions::WEEK_OF_YEAR))) {
	...
	protected function computeJulianDay()
	{
		...
		// is used.
		if($this->fStamp[YoudsFrameworkDateDefinitions::JULIAN_DAY] >= (int)self::kMinimumUserStamp) {
			$bestStamp = $this->newestStamp(YoudsFrameworkDateDefinitions::ERA, YoudsFrameworkDateDefinitions::DAY_OF_WEEK_IN_MONTH, self::kUnset);
			$bestStamp = $this->newestStamp(YoudsFrameworkDateDefinitions::YEAR_WOY, YoudsFrameworkDateDefinitions::EXTENDED_YEAR, $bestStamp);
			if($bestStamp <= $this->fStamp[YoudsFrameworkDateDefinitions::JULIAN_DAY]) {
				return $this->internalGet(YoudsFrameworkDateDefinitions::JULIAN_DAY);
			}
		}

		$bestField = $this->resolveFields($this->getFieldResolutionTable());
		if($bestField == YoudsFrameworkDateDefinitions::FIELD_COUNT) {
			$bestField = YoudsFrameworkDateDefinitions::DAY_OF_MONTH;
		}
	...
	protected function computeMillisInDay()
	{
		...
		// AM_PM and the HOUR.
		$hourOfDayStamp = $this->fStamp[YoudsFrameworkDateDefinitions::HOUR_OF_DAY];
		$hourStamp = ($this->fStamp[YoudsFrameworkDateDefinitions::HOUR] > $this->fStamp[YoudsFrameworkDateDefinitions::AM_PM]) ? $this->fStamp[YoudsFrameworkDateDefinitions::HOUR] : $this->fStamp[YoudsFrameworkDateDefinitions::AM_PM];
		...
		if($bestStamp != self::kUnset) {
			if($bestStamp == $hourOfDayStamp) {
				...
				$millisInDay += $this->internalGet(YoudsFrameworkDateDefinitions::HOUR_OF_DAY);
			} else {
				// Don't normalize here; let overflow bump into the next period.
				// This is consistent with how we handle other fields.
				$millisInDay += $this->internalGet(YoudsFrameworkDateDefinitions::HOUR);
				$millisInDay += 12 * $this->internalGet(YoudsFrameworkDateDefinitions::AM_PM); // Default works for unset AM_PM
			}
		}
		...
		// == HOUR_OF_DAY.
		$millisInDay *= 60;
		$millisInDay += $this->internalGet(YoudsFrameworkDateDefinitions::MINUTE); // now have minutes
		$millisInDay *= 60;
		$millisInDay += $this->internalGet(YoudsFrameworkDateDefinitions::SECOND); // now have seconds
		$millisInDay *= 1000;
		$millisInDay += $this->internalGet(YoudsFrameworkDateDefinitions::MILLISECOND); // now have millis
	...
	static $kDatePrecedence = array(
		array(
			array(YoudsFrameworkDateDefinitions::DAY_OF_MONTH, self::RESOLVE_STOP),
			array(YoudsFrameworkDateDefinitions::WEEK_OF_YEAR, YoudsFrameworkDateDefinitions::DAY_OF_WEEK, self::RESOLVE_STOP),
			array(YoudsFrameworkDateDefinitions::WEEK_OF_MONTH, YoudsFrameworkDateDefinitions::DAY_OF_WEEK, self::RESOLVE_STOP),
			array(YoudsFrameworkDateDefinitions::DAY_OF_WEEK_IN_MONTH, YoudsFrameworkDateDefinitions::DAY_OF_WEEK, self::RESOLVE_STOP),
			array(YoudsFrameworkDateDefinitions::WEEK_OF_YEAR, YoudsFrameworkDateDefinitions::DOW_LOCAL, self::RESOLVE_STOP),
			array(YoudsFrameworkDateDefinitions::WEEK_OF_MONTH, YoudsFrameworkDateDefinitions::DOW_LOCAL, self::RESOLVE_STOP),
			array(YoudsFrameworkDateDefinitions::DAY_OF_WEEK_IN_MONTH, YoudsFrameworkDateDefinitions::DOW_LOCAL, self::RESOLVE_STOP),
			array(YoudsFrameworkDateDefinitions::DAY_OF_YEAR, self::RESOLVE_STOP),
			//    kResolveRemap | UCAL_DAY_OF_MONTH
			array(37, YoudsFrameworkDateDefinitions::YEAR, self::RESOLVE_STOP),  // if YEAR is set over YEAR_WOY use DAY_OF_MONTH
			//    kResolveRemap | UCAL_WEEK_OF_YEAR
			array(35, YoudsFrameworkDateDefinitions::YEAR_WOY, self::RESOLVE_STOP),  // if YEAR_WOY is set,  calc based on WEEK_OF_YEAR
			array(self::RESOLVE_STOP),
		),
		array(
			array(YoudsFrameworkDateDefinitions::WEEK_OF_YEAR, self::RESOLVE_STOP),
			array(YoudsFrameworkDateDefinitions::WEEK_OF_MONTH, self::RESOLVE_STOP),
			array(YoudsFrameworkDateDefinitions::DAY_OF_WEEK_IN_MONTH, self::RESOLVE_STOP),
			//    kResolveRemap | UCAL_DAY_OF_WEEK_IN_MONTH
			array(40, YoudsFrameworkDateDefinitions::DAY_OF_WEEK, self::RESOLVE_STOP),
			//    self::kResolveRemap | UCAL_DAY_OF_WEEK_IN_MONTH
			array(40, YoudsFrameworkDateDefinitions::DOW_LOCAL, self::RESOLVE_STOP),
			array(self::RESOLVE_STOP),
		),
		array(
			array(self::RESOLVE_STOP),
	...
	protected static $kYearPrecedence = array(
		array(
			array(YoudsFrameworkDateDefinitions::YEAR, self::RESOLVE_STOP),
			array(YoudsFrameworkDateDefinitions::EXTENDED_YEAR, self::RESOLVE_STOP),
			array(YoudsFrameworkDateDefinitions::YEAR_WOY, YoudsFrameworkDateDefinitions::WEEK_OF_YEAR, self::RESOLVE_STOP),  // YEAR_WOY is useless without WEEK_OF_YEAR
			array(self::RESOLVE_STOP),
		),
		array(
			array(self::RESOLVE_STOP),
	...
	protected static $kDOWPrecedence = array(
		array(
			array(YoudsFrameworkDateDefinitions::DAY_OF_WEEK, self::RESOLVE_STOP, self::RESOLVE_STOP),
			array(YoudsFrameworkDateDefinitions::DOW_LOCAL, self::RESOLVE_STOP, self::RESOLVE_STOP),
			array(self::RESOLVE_STOP),
		),
		array(
			array(self::RESOLVE_STOP),
	...
	{
		$bestField = YoudsFrameworkDateDefinitions::FIELD_COUNT;
		for($g = 0; $precedenceTable[$g][0][0] != -1 && ($bestField == YoudsFrameworkDateDefinitions::FIELD_COUNT); ++$g) {
...
//linesInGroup:
	 ...
	 *
	 * <ul><li>YoudsFrameworkDateDefinitions::ERA
	 * <li>YoudsFrameworkDateDefinitions::YEAR
	 * <li>YoudsFrameworkDateDefinitions::MONTH
	 * <li>YoudsFrameworkDateDefinitions::DAY_OF_MONTH
	 * <li>YoudsFrameworkDateDefinitions::DAY_OF_YEAR
	 * <li>YoudsFrameworkDateDefinitions::EXTENDED_YEAR</ul>
	 *
	...
	{
		$this->internalSet(YoudsFrameworkDateDefinitions::MONTH, $this->getGregorianMonth());
		$this->internalSet(YoudsFrameworkDateDefinitions::DAY_OF_MONTH, $this->getGregorianDayOfMonth());
		$this->internalSet(YoudsFrameworkDateDefinitions::DAY_OF_YEAR, $this->getGregorianDayOfYear());
		$eyear = $this->getGregorianYear();
		$this->internalSet(YoudsFrameworkDateDefinitions::EXTENDED_YEAR, $eyear);
		$era = GregorianCalendar::AD;
		if($eyear < 1) {
			$era = GregorianCalendar::BC;
			$eyear = 1 - $eyear;
		}
		$this->internalSet(YoudsFrameworkDateDefinitions::ERA, $era);
		$this->internalSet(YoudsFrameworkDateDefinitions::YEAR, $eyear);
	}

	/**
	 * Return the extended year on the Gregorian calendar as computed by
	 * <code>computeGregorianFields()</code>.
	 * @see        YoudsFrameworkCalendar::computeGregorianFields
	 * @internal
	 *
	 ...
	 * YoudsFrameworkDateDefinitions::DAY_OF_MONTH field for a calendar set to April 31
	...
	protected function getLocalDOW()
	{
		...
		switch($this->resolveFields(self::$kDOWPrecedence)) {
			case YoudsFrameworkDateDefinitions::DAY_OF_WEEK:
				$dowLocal = $this->internalGet(YoudsFrameworkDateDefinitions::DAY_OF_WEEK) - $this->fFirstDayOfWeek;
				break;
			case YoudsFrameworkDateDefinitions::DOW_LOCAL:
				$dowLocal = $this->internalGet(YoudsFrameworkDateDefinitions::DOW_LOCAL) - 1;
				break;
			default:
				break;
		}
	...
	/**
	 * @var        YoudsFrameworkTimeZone Time zone affects the time calculation done by
	 ...
	 * time.
	 *
	 * @param      YoudsFrameworkLocale The given locale.
	...
	private function setWeekCountData($desiredLocale, $type)
	{
		...
		$this->fFirstDayOfWeek = YoudsFrameworkDateDefinitions::SUNDAY;
	...
	private function computeGregorianAndDOWFields($julianDay)
	{
		...
		$this->internalSet(YoudsFrameworkDateDefinitions::DAY_OF_WEEK, $dow);
		...
		$this->internalSet(YoudsFrameworkDateDefinitions::DOW_LOCAL, $dowLocal);
		$this->fFields[YoudsFrameworkDateDefinitions::DOW_LOCAL] = $dowLocal;
	}
	...
	private function computeGregorianFields($julianDay)
	{
		...
		YoudsFrameworkCalendarGrego::dayToFields($julianDay - YoudsFrameworkDateDefinitions::EPOCH_START_AS_JULIAN_DAY, $this->fGregorianYear, $this->fGregorianMonth, $this->fGregorianDayOfMonth, $gregorianDayOfWeekUnused, $this->fGregorianDayOfYear);
	}
	...
	{
		$eyear = $this->fFields[YoudsFrameworkDateDefinitions::EXTENDED_YEAR];
		$year = $this->fFields[YoudsFrameworkDateDefinitions::YEAR];
		$dayOfWeek = $this->fFields[YoudsFrameworkDateDefinitions::DAY_OF_WEEK];
		$dayOfYear = $this->fFields[YoudsFrameworkDateDefinitions::DAY_OF_YEAR];
		...
		$this->fFields[YoudsFrameworkDateDefinitions::WEEK_OF_YEAR] = $woy;
		$this->fFields[YoudsFrameworkDateDefinitions::YEAR_WOY] = $yearOfWeekOfYear;
		// WEEK_OF_YEAR end

		$dayOfMonth = $this->fFields[YoudsFrameworkDateDefinitions::DAY_OF_MONTH];
		$this->fFields[YoudsFrameworkDateDefinitions::WEEK_OF_MONTH] = $this->weekNumber($dayOfMonth, $dayOfWeek);
		$this->fFields[YoudsFrameworkDateDefinitions::DAY_OF_WEEK_IN_MONTH] = (int)(($dayOfMonth-1) / 7) + 1;
	...
	{
		for($field = 0; $field < YoudsFrameworkDateDefinitions::FIELD_COUNT; ++$field) {
	...
	private function validateField($field)
	{
		switch($field) {
			case YoudsFrameworkDateDefinitions::DAY_OF_MONTH:
				...
				$this->validateField1($field, 1, $this->handleGetMonthLength($y, $this->internalGet(YoudsFrameworkDateDefinitions::MONTH)));
				break;
			case YoudsFrameworkDateDefinitions::DAY_OF_YEAR:
				$y = $this->handleGetExtendedYear();
	...
	protected function julianDayToDayOfWeek($julian)
	{
		...
		$result = ($dayOfWeek + (($dayOfWeek < 0) ? (7 + YoudsFrameworkDateDefinitions::SUNDAY) : YoudsFrameworkDateDefinitions::SUNDAY));
		return $result;
	}
