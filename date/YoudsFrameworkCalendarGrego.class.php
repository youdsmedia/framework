 ...
 *
 * Note: Unlike YoudsFrameworkGregorianCalendar, all computations performed by this
 ...
 * @version    $Id$
 */
final class YoudsFrameworkCalendarGrego
{
	/**
	...
	public static function dayToFields($day, &$year, &$month, &$dom, &$dow, &$doy = 0)
	{
		...
		// 1461 == 365*4 + 1 days.
		$n400 = (int) YoudsFrameworkToolkit::floorDivide($day, 146097, $doy); // 400-year cycle length
		$n100 = (int) YoudsFrameworkToolkit::floorDivide($doy, 36524, $doy); // 100-year cycle length
		$n4   = (int) YoudsFrameworkToolkit::floorDivide($doy, 1461, $doy); // 4-year cycle length
		$n1   = (int) YoudsFrameworkToolkit::floorDivide($doy, 365, $doy);
		...
		$dow += ($dow < 0) ? (YoudsFrameworkDateDefinitions::SUNDAY + 7) : YoudsFrameworkDateDefinitions::SUNDAY;
	...
	{
		$millisInDay = null;
		$day = YoudsFrameworkToolkit::floorDivide($time, YoudsFrameworkDateDefinitions::MILLIS_PER_DAY, $millisInDay); // 400-year cycle length
		$mid = (int) $millisInDay;
	...
	{
		$dow = null;
		YoudsFrameworkToolkit::floorDivide($day + YoudsFrameworkDateDefinitions::THURSDAY, 7, $dow);
		return ($dow == 0) ? YoudsFrameworkDateDefinitions::SATURDAY : $dow;
	}
