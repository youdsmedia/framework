 ...
 * @version    $Id$
 */
class YoudsFrameworkDateFormat
{
	/**
	 ...
	 *
	 * @param      string Format to be used for formatting.
	 * @param      YoudsFrameworkLocale The locale.
	 *
	 * @author     Dominik del Bondio <ddb@bitxtender.com>
	 * @since      0.11.0
	 */
	public function setLocalizedFormat($format, YoudsFrameworkLocale $locale)
	{
	 ...
	 *                   needed info with the YoudsFrameworkDateDefinitions constants as 
	 *                   keys or an unix timestamp (doesn't work yet!) or an
	 *                   YoudsFrameworkCalendar instance.
	 * @param      string The calendar type this date should be formatted in 
	 *                    (this will usually be gregorian)
	 * @param      YoudsFrameworkLocale|string The locale to format the date in.
	 *
	 ...
	 * @since      0.11.0
	 */
	public function format($data, $calendarType = YoudsFrameworkCalendar::GREGORIAN, $locale = null)
	{
		if($locale instanceof YoudsFrameworkLocale) {
			...
			throw new InvalidArgumentException('This YoudsFrameworkDateFormat has not been initialize()d. To be able to pass a string as locale you need to call initialize() or create this YoudsFrameworkDateFormat using YoudsFrameworkTranslationManager::createDateFormat()');
		}
		...
		// implicitly created)
		$calNeedsClone = false;
		if($data instanceof YoudsFrameworkCalendar) {
			$cal = $data;
			$calNeedsClone = true;
		...
		foreach($this->tokenList as $token) {
			...
			switch($token[0]) {
				...
				case self::T_ERA:
					$era = $data[YoudsFrameworkDateDefinitions::ERA];
					if($count == 4) {
				...
				case self::T_YEAR:
					$year = $data[YoudsFrameworkDateDefinitions::YEAR];
					if($count == 2) {
				...
				case self::T_SA_MONTH:
					$month = $data[YoudsFrameworkDateDefinitions::MONTH] + 1;
					if($count == 3) {
				...
				case self::T_DATE:
					$out .= str_pad($data[YoudsFrameworkDateDefinitions::DATE], $count, '0', STR_PAD_LEFT);
					break;

				case self::T_HOUR_1_24:
					$hour = $data[YoudsFrameworkDateDefinitions::HOUR_OF_DAY];
					$out .= str_pad($hour == 0 ? 24 : $hour, $count, '0', STR_PAD_LEFT);
					break;

				case self::T_HOUR_0_23:
					$out .= str_pad($data[YoudsFrameworkDateDefinitions::HOUR_OF_DAY], $count, '0', STR_PAD_LEFT);
					break;

				case self::T_MINUTE:
					$out .= str_pad($data[YoudsFrameworkDateDefinitions::MINUTE], $count, '0', STR_PAD_LEFT);
					break;

				case self::T_SECOND:
					$out .= str_pad($data[YoudsFrameworkDateDefinitions::SECOND], $count, '0', STR_PAD_LEFT);
					break;

				case self::T_FRACTIONAL_SECOND:
					$value = str_pad($data[YoudsFrameworkDateDefinitions::MILLISECOND], 3, '0', STR_PAD_LEFT);
					$value = substr($value, 0, $count);
					$out .= $value;
					break;

				case self::T_DAY_OF_WEEK:
					$dow = $data[YoudsFrameworkDateDefinitions::DAY_OF_WEEK];
					if($count == 4) {
				...
				case self::T_DAY_OF_YEAR:
					$out .= str_pad($data[YoudsFrameworkDateDefinitions::DAY_OF_YEAR], $count, '0', STR_PAD_LEFT);
					break;

				case self::T_DAY_OF_WEEK_IN_MONTH:
					$out .= str_pad($data[YoudsFrameworkDateDefinitions::DAY_OF_WEEK_IN_MONTH], $count, '0', STR_PAD_LEFT);
					break;

				case self::T_WEEK_OF_YEAR:
					$out .= str_pad($data[YoudsFrameworkDateDefinitions::WEEK_OF_YEAR], $count, '0', STR_PAD_LEFT);
					break;

				case self::T_WEEK_OF_MONTH:
					$out .= str_pad($data[YoudsFrameworkDateDefinitions::WEEK_OF_MONTH], $count, '0', STR_PAD_LEFT);
					break;

				case self::T_AM_PM:
					$isPm = $data[YoudsFrameworkDateDefinitions::AM_PM];
					if($isPm) {
				...
				case self::T_HOUR_1_12:
					$hour = $data[YoudsFrameworkDateDefinitions::HOUR];
					$out .= str_pad($hour == 0 ? 12 : $hour, $count, '0', STR_PAD_LEFT);
					break;

				case self::T_HOUR_0_11:
					$out .= str_pad($data[YoudsFrameworkDateDefinitions::HOUR], $count, '0', STR_PAD_LEFT);
					break;
				...
				case self::T_TIMEZONE_GENERIC:
					if(!$tzid) {
						...
						if($token[0] == self::T_TIMEZONE_GENERIC) {
							...
							if($data[YoudsFrameworkDateDefinitions::DST_OFFSET] != 0) {
								if($count < 4) {
				...
				case self::T_ISO_YEAR:
					$out .= str_pad($data[YoudsFrameworkDateDefinitions::YEAR_WOY], $count, '0', STR_PAD_LEFT);
					break;

				case self::T_LOCAL_DAY_OF_WEEK:
				case self::T_SA_LOCAL_DAY_OF_WEEK:
					$dow = $data[YoudsFrameworkDateDefinitions::DOW_LOCAL];
					if($count == 4) {
						$out .= $locale->getCalendarDayWide($calendarType, $dow);
					} elseif($count == 5) {
						$out .= $locale->getCalendarDayNarrow($calendarType, $dow);
				...
				case self::T_EXTENDED_YEAR:
					$out .= str_pad($data[YoudsFrameworkDateDefinitions::EXTENDED_YEAR], $count, '0', STR_PAD_LEFT);
					break;

				case self::T_MODIFIED_JD:
					$out .= str_pad($data[YoudsFrameworkDateDefinitions::JULIAN_DAY], $count, '0', STR_PAD_LEFT);
					break;

				case self::T_MS_IN_DAY:
					$out .= str_pad($data[YoudsFrameworkDateDefinitions::MILLISECONDS_IN_DAY], $count, '0', STR_PAD_LEFT);
					break;
				...
				case self::T_TIMEZONE_RFC:
					if($count > 3) {
						...
						$value = ($data[YoudsFrameworkDateDefinitions::ZONE_OFFSET] + $data[YoudsFrameworkDateDefinitions::DST_OFFSET]) / YoudsFrameworkDateDefinitions::MILLIS_PER_MINUTE;
						if($value < 0) {
							$value = -$value;
							$sign = '-';
						}
				...
				case self::T_SA_QUARTER:
					$quarter = (int)($data[YoudsFrameworkDateDefinitions::MONTH] / 3);
					if($count == 3) {
	...
	{
		$value = $data[YoudsFrameworkDateDefinitions::ZONE_OFFSET] + $data[YoudsFrameworkDateDefinitions::DST_OFFSET];
		...
		$str .=		str_pad((int) ($value / YoudsFrameworkDateDefinitions::MILLIS_PER_HOUR), 2, '0', STR_PAD_LEFT)
						. ':'
						. str_pad((int) (($value % YoudsFrameworkDateDefinitions::MILLIS_PER_HOUR) / YoudsFrameworkDateDefinitions::MILLIS_PER_MINUTE),  2, '0', STR_PAD_LEFT);
		return $str;
	}
				...
				if($c == '\'') {
					...
					if(preg_match('#[a-z]#i', $c)) {
						if(isset($charToTokenMap[$c])) {
							...
							throw new YoudsFrameworkException('Unknown pattern char "' . $c . '" (#'. ord($c) . ') in format string "' . $format . '" at index ' . $i);
						}
					} else {
						$this->tokenList[$tokenIdx] = array(self::T_TEXT, $c);
						++$tokenIdx;
	 ...
	 *                  non-lenient.
	 *
	 * @return     YoudsFrameworkCalendar The calendar object.
	 *
	 * @author     Dominik del Bondio <ddb@bitxtender.com>
	 * @since      0.11.0
	 */
	public function parse($dateString, $locale = null, $strict = false)
	{
		...
		$cal->clear();
		$cal->set(YoudsFrameworkDateDefinitions::ERA, $era);
		...
		$calendarType = YoudsFrameworkCalendar::GREGORIAN;
		$datePos = 0;
		...
		for($i = 0; $i < $tlCount; ++$i) {
			if($datePos >= strlen($dateString)) {
				if($strict) {
					throw new YoudsFrameworkException('Input string "' . $dateString . '" is to short');
				}
				break;
			}
			...
			if($type == 'number' && $i + 1 < $tlCount && $this->getTokenType($this->tokenList[$i + 1]) == 'number') {
				...
				if($diff > 0) {
					...
					if($diff >= $tLen - 1) {
						...
						throw new YoudsFrameworkException('Not enough digits given in "' . $dateString . '" at pos ' . $datePos . ' (expected ' . $tokenReqLen . ' digits)');
					}
				}
				...
				foreach($unprocessedTokens as $token) {
					...
					if($dateField === null) {
						throw new YoudsFrameworkException('Token type ' . $token[0] . ' claims to be numerical but has no date field');
					}

					$number = (int) substr($dateString, $datePos, $token[1]);

					$datePos += $token[1];
					if($dateField == YoudsFrameworkDateDefinitions::HOUR_OF_DAY && $token[0] == YoudsFrameworkDateFormat::T_HOUR_1_24 && $number == 24) {
						$number = 0;
					} elseif($dateField == YoudsFrameworkDateDefinitions::HOUR && $token[0] == YoudsFrameworkDateFormat::T_HOUR_1_12 && $number == 12) {
						$number = 0;
					} elseif($dateField == YoudsFrameworkDateDefinitions::MONTH && $number > 0) {
						$number -= 1;
					}
					...
					if(self::T_QUARTER == $token[0] || self::T_SA_QUARTER == $token[0]) {
						...
						if(!$cal->_isSet(YoudsFrameworkDateDefinitions::MONTH)) {
				...
				if($dateField == YoudsFrameworkDateDefinitions::MILLISECOND && $numberLen > 3) {
				...
				if($dateField == YoudsFrameworkDateDefinitions::MONTH && $number > 0) {
					$number -= 1;
				} elseif($dateField == YoudsFrameworkDateDefinitions::HOUR_OF_DAY && $token[0] == YoudsFrameworkDateFormat::T_HOUR_1_24 && $number == 24) {
					$number = 0;
				} elseif($dateField == YoudsFrameworkDateDefinitions::HOUR && $token[0] == YoudsFrameworkDateFormat::T_HOUR_1_12 && $number == 12) {
					$number = 0;
				...
				switch($token[0]) {
					case self::T_TEXT:
						if(substr_compare($dateString, $token[1], $datePos, strlen($token[1])) == 0) {
							...
							throw new YoudsFrameworkException('Unknown character in "' . $dateString . '" at pos ' . $datePos . ' (expected: "' . $token[1] . '", got: "' . substr($dateString, $datePos, strlen($token[1])) . '")');
						}
						break;
					...
					case self::T_SA_LOCAL_DAY_OF_WEEK:
						...
						switch($token[0]) {
							case self::T_ERA:
								...
								$dataField = YoudsFrameworkDateDefinitions::ERA;
								break;
							case self::T_DAY_OF_WEEK:
								$funcPrefix = 'getCalendarDays';
								$dataField = YoudsFrameworkDateDefinitions::DAY_OF_WEEK;
								break;

							case self::T_LOCAL_DAY_OF_WEEK:
							case self::T_SA_LOCAL_DAY_OF_WEEK:
								$funcPrefix = 'getCalendarDays';
								$dataField = YoudsFrameworkDateDefinitions::DOW_LOCAL;
								break;
						}
						...
						if($this->matchStringWithFallbacks($dateString, $items, $datePos, $item)) {
							...
							throw new YoudsFrameworkException('Unknown character in "' . $dateString . '" at pos ' . $datePos . ' (expected one of: ' . implode(', ', $items) . ')');
						}
						break;
					...
					case self::T_AM_PM:
						...
						$item = null;
						if($this->matchStringWithFallbacks($dateString, $items, $datePos, $item)) {
							$cal->set(YoudsFrameworkDateDefinitions::AM_PM, $item);
						} else {
							throw new YoudsFrameworkException('Unknown character in "' . $dateString . '" at pos ' . $datePos . ' (expected one of: ' . implode(', ', $items) . ')');
						}
						break;
						...
						if(preg_match('#^(GMT)?(\+|-)?(\d{1,2}:\d{1,2}|\d{1,2}\d{1,2})#i', $remainder, $match)) {
							...
							if(!($tz = $tm->createTimeZone($remainder))) {
								...
								if($this->matchStringWithFallbacks($dateString, $localizedTzMap, $datePos, $id)) {
									...
									throw new YoudsFrameworkException('Unknown character in "' . $dateString . '" at pos ' . $datePos . ' (expected one of: ' . implode(', ', $localizedTzMap) . ')');
								}
							} else {
					...
					case self::T_SA_MONTH:
						...
						if($this->matchStringWithFallbacks($dateString, $months, $datePos, $month)) {
							$cal->set(YoudsFrameworkDateDefinitions::MONTH, $month - 1);
						} else {
							throw new YoudsFrameworkException('Unknown character in "' . $dateString . '" at pos ' . $datePos . ' (expected one of: ' . implode(', ', $months) . ')');
						}
					...
					case self::T_SA_QUARTER:
						...
						if($this->matchStringWithFallbacks($dateString, $quarters, $datePos, $quarter)) {
							if(!$cal->_isSet(YoudsFrameworkDateDefinitions::MONTH)) {
								$cal->set(YoudsFrameworkDateDefinitions::MONTH, ($quarter - 1) * 3);
							}
						} else {
							throw new YoudsFrameworkException('Unknown character in "' . $dateString . '" at pos ' . $datePos . ' (expected one of: ' . implode(', ', $quarters) . ')');
						}
						break;
		...
		if($strict) {
			...
			if($datePos < strlen($dateString)) {
				throw new YoudsFrameworkException('Input string "' . $dateString . '" has characters after the date');
			}
		}
	...
	protected function getDateFieldFromTokenType($type)
	{
		static $typeMap = array(
			self::T_ERA                     => YoudsFrameworkDateDefinitions::ERA,
			self::T_YEAR                    => YoudsFrameworkDateDefinitions::YEAR,
			self::T_MONTH                   => YoudsFrameworkDateDefinitions::MONTH,
			self::T_SA_MONTH                => YoudsFrameworkDateDefinitions::MONTH,
			self::T_DATE                    => YoudsFrameworkDateDefinitions::DATE,
			self::T_HOUR_1_24               => YoudsFrameworkDateDefinitions::HOUR_OF_DAY,
			self::T_HOUR_0_23               => YoudsFrameworkDateDefinitions::HOUR_OF_DAY,
			self::T_MINUTE                  => YoudsFrameworkDateDefinitions::MINUTE,
			self::T_SECOND                  => YoudsFrameworkDateDefinitions::SECOND,
			self::T_FRACTIONAL_SECOND       => YoudsFrameworkDateDefinitions::MILLISECOND,
			self::T_DAY_OF_WEEK             => YoudsFrameworkDateDefinitions::DAY_OF_WEEK,
			self::T_DAY_OF_YEAR             => YoudsFrameworkDateDefinitions::DAY_OF_YEAR,
			self::T_DAY_OF_WEEK_IN_MONTH    => YoudsFrameworkDateDefinitions::DAY_OF_WEEK_IN_MONTH,
			self::T_WEEK_OF_YEAR            => YoudsFrameworkDateDefinitions::WEEK_OF_YEAR,
			self::T_WEEK_OF_MONTH           => YoudsFrameworkDateDefinitions::WEEK_OF_MONTH,
			self::T_AM_PM                   => YoudsFrameworkDateDefinitions::AM_PM,
			self::T_HOUR_1_12               => YoudsFrameworkDateDefinitions::HOUR,
			self::T_HOUR_0_11               => YoudsFrameworkDateDefinitions::HOUR,
			self::T_ISO_YEAR                => YoudsFrameworkDateDefinitions::YEAR_WOY,
			self::T_LOCAL_DAY_OF_WEEK       => YoudsFrameworkDateDefinitions::DOW_LOCAL,
			self::T_SA_LOCAL_DAY_OF_WEEK    => YoudsFrameworkDateDefinitions::DOW_LOCAL,
			self::T_EXTENDED_YEAR           => YoudsFrameworkDateDefinitions::EXTENDED_YEAR,
			self::T_MODIFIED_JD             => YoudsFrameworkDateDefinitions::JULIAN_DAY,
			self::T_MS_IN_DAY               => YoudsFrameworkDateDefinitions::MILLISECONDS_IN_DAY,
			self::T_QUARTER                 => YoudsFrameworkDateDefinitions::MONTH,
			self::T_SA_QUARTER              => YoudsFrameworkDateDefinitions::MONTH,
		);
