 ...
 * @version    $Id$
 */
class YoudsFrameworkGregorianCalendar extends YoudsFrameworkCalendar
{
	const CUTOVER_JULIAN_DAY = 2299161;
	//const kPapalCutover = (2299161.0 - kEpochStartAsJulianDay) * YoudsFrameworkDateDefinitions::MILLIS_PER_DAY;
	const PAPAL_CUTOVER      = -12219292800000.0;

	/**
	 * Overloaded.
	 *
	 * @see        YoudsFrameworkGregorianCalendar::constructorO()
	 * @see        YoudsFrameworkGregorianCalendar::constructorOO()
	 * @see        YoudsFrameworkGregorianCalendar::constructorOIII()
	 * @see        YoudsFrameworkGregorianCalendar::constructorOIIIII()
	 * @see        YoudsFrameworkGregorianCalendar::constructorOIIIIII()
	 * 
	 * @author     Dominik del Bondio <ddb@bitxtender.com>
	 * @author     The ICU Project
	 * @since      0.11.0
	...
	public function __construct()
	{
		...
		$fName = YoudsFrameworkToolkit::overloadHelper(
			array(
				array(
					'name' => 'constructorO',
	 ...
	 * Constructor.
	 * 
	 * @param      YoudsFrameworkTimeZone|YoudsFrameworkLocale|YoudsFrameworkTranslationManager
	 * 
	...
	protected function constructorO($zoneOrLocale)
	{
		...
		if($zoneOrLocale instanceof YoudsFrameworkTimeZone) {
			$this->translationManager = $zoneOrLocale->getTranslationManager();
			$zone = $zoneOrLocale;
			$locale = $this->translationManager->getCurrentLocale();
		} elseif($zoneOrLocale instanceof YoudsFrameworkLocale) {
			$this->translationManager = $zoneOrLocale->getTranslationManager();
			$zone = $this->translationManager->getDefaultTimeZone();
			$locale = $zoneOrLocale;
		} elseif($zoneOrLocale instanceof YoudsFrameworkTranslationManager) {
		...
		$this->set(YoudsFrameworkDateDefinitions::ERA, self::AD);
	}
	 ...
	 * Constructor.
	 * 
	 * @param      YoudsFrameworkTranslationManager
	 * @param      int
	 * @param      int
	 * @param      int
	 * 
	 ...
	 * @since      0.11.0
	 */
	protected function constructorOIII(YoudsFrameworkTranslationManager $tm, $year, $month, $date)
	{
		parent::constructorOO($tm->getDefaultTimeZone(), $tm->getCurrentLocale());
		$this->set(YoudsFrameworkDateDefinitions::ERA, self::AD);
		$this->set(YoudsFrameworkDateDefinitions::YEAR, $year);
		$this->set(YoudsFrameworkDateDefinitions::MONTH, $month);
		$this->set(YoudsFrameworkDateDefinitions::DATE, $date);
	}
	 ...
	 * @since      0.11.0
	 */
	protected function constructorOIIIII(YoudsFrameworkTranslationManager $tm, $year, $month, $date, $hour, $minute)
	{
		parent::constructorOO($tm->getDefaultTimeZone(), $tm->getCurrentLocale());
		$this->set(YoudsFrameworkDateDefinitions::ERA, self::AD);
		$this->set(YoudsFrameworkDateDefinitions::YEAR, $year);
		$this->set(YoudsFrameworkDateDefinitions::MONTH, $month);
		$this->set(YoudsFrameworkDateDefinitions::DATE, $date);
		$this->set(YoudsFrameworkDateDefinitions::HOUR_OF_DAY, $hour);
		$this->set(YoudsFrameworkDateDefinitions::MINUTE, $minute);
	}
	 ...
	 * @since      0.11.0
	 */
	protected function constructorOIIIIII(YoudsFrameworkTranslationManager $tm, $year, $month, $date, $hour, $minute, $second)
	{
		parent::constructorOO($tm->getDefaultTimeZone(), $tm->getCurrentLocale());
		$this->set(YoudsFrameworkDateDefinitions::ERA, self::AD);
		$this->set(YoudsFrameworkDateDefinitions::YEAR, $year);
		...
		$this->set(YoudsFrameworkDateDefinitions::HOUR_OF_DAY, $hour);
		$this->set(YoudsFrameworkDateDefinitions::MINUTE, $minute);
		$this->set(YoudsFrameworkDateDefinitions::SECOND, $second);
	}
	...
	public function setGregorianChange($date)
	{
		...
		// pure date values.
		$cutoverDay = (int) floor($this->fGregorianCutover / YoudsFrameworkDateDefinitions::MILLIS_PER_DAY);
		$this->fNormalizedGregorianCutover = $cutoverDay * YoudsFrameworkDateDefinitions::MILLIS_PER_DAY;
		...
		if($cutoverDay < 0 && $this->fNormalizedGregorianCutover > 0) {
			$this->fNormalizedGregorianCutover = ($cutoverDay + 1) * YoudsFrameworkDateDefinitions::MILLIS_PER_DAY;
		}

		// Normalize the year so BC values are represented as 0 and negative
		// values.
		$cal = new YoudsFrameworkGregorianCalendar($this->getTimeZone());

		$cal->setTime($date);
		$this->fGregorianCutoverYear = $cal->get(YoudsFrameworkDateDefinitions::YEAR);
		if($cal->get(YoudsFrameworkDateDefinitions::ERA) == self::BC) 
	...
	public function isLeapYear($year)
	{
		...
		return ($year >= $this->fGregorianCutoverYear
						? YoudsFrameworkCalendarGrego::isLeapYear($year) // Gregorian
						: (($year & 0x3) == 0)); // Julian
	}
	...
	{
		// Calendar override.
		return YoudsFrameworkCalendar::isEquivalentTo($other) && $this->getGregorianChange() == $other->getGregorianChange();
	}

	/**
	 * @see        YoudsFrameworkCalendar::getActualMinimum()
	 * 
	 * @author     Dominik del Bondio <ddb@bitxtender.com>
	 * @author     The ICU Project
	 * @since      0.11.0
	...
	/**
	 * @see        YoudsFrameworkCalendar::getActualMaximum()
	 * 
	 * @author     Dominik del Bondio <ddb@bitxtender.com>
	 * @author     The ICU Project
	 * @since      0.11.0
				...
				*/
				{
					...
					$era = $cal->get(YoudsFrameworkDateDefinitions::ERA);
					$d = $cal->getTime();

					/* Perform a binary search, with the invariant that lowGood is a
					* valid year, and highBad is an out of range year.
					*/
					$lowGood = self::$kGregorianCalendarLimits[YoudsFrameworkDateDefinitions::YEAR][1];
					$highBad = self::$kGregorianCalendarLimits[YoudsFrameworkDateDefinitions::YEAR][2] + 1;
					while((lowGood + 1) < highBad) {
						$y = (int)(($lowGood + $highBad) / 2);
						$cal->set(YoudsFrameworkDateDefinitions::YEAR, $y);
						if($cal->get(YoudsFrameworkDateDefinitions::YEAR) == $y && $cal->get(YoudsFrameworkDateDefinitions::ERA) == $era) {
							$lowGood = $y;
						} else {
							$highBad = $y;
	...
	/**
	 * @see        YoudsFrameworkCalendar::inDaylightTime
	 *
	 * @author     Dominik del Bondio <ddb@bitxtender.com>
	 * @author     The ICU Project
	 * @since      0.11.0
	...
	public function inDaylightTime()
	{
		...
		return ($this->internalGet(YoudsFrameworkDateDefinitions::DST_OFFSET) != 0);
	}
	...
	{
		return $this->_isSet(YoudsFrameworkDateDefinitions::ERA) ? $this->internalGet(YoudsFrameworkDateDefinitions::ERA) : self::AD;
	}

	/**
	 * @see        YoudsFrameworkCalendar::handleComputeMonthStart
	 * 
	 * @author     Dominik del Bondio <ddb@bitxtender.com>
	 * @author     The ICU Project
	 * @since      0.11.0
	...
	protected function handleComputeMonthStart($eyear, $month, $useMonth)
	{
		...
		if($month < 0 || $month > 11) {
			$eyear += YoudsFrameworkToolkit::floorDivide($month, 12, $month);
		}

		$isLeap = ($eyear % 4 == 0);
		$y = $eyear - 1;
		$julianDay = 365 * $y + floor($y / 4) + (YoudsFrameworkDateDefinitions::JAN_1_1_JULIAN_DAY - 3);
		...
		if($this->fIsGregorian) {
			...
			// Julian calendar
			$gregShift = YoudsFrameworkCalendarGrego::gregorianShift($eyear);
	...
	/**
	 * @see        YoudsFrameworkCalendar::handleComputeJulianDay
	 * 
	 * @author     Dominik del Bondio <ddb@bitxtender.com>
	 * @author     The ICU Project
	 * @since      0.11.0
	...
	protected function handleComputeJulianDay($bestField)
	{
		...
		if(($bestField == YoudsFrameworkDateDefinitions::WEEK_OF_YEAR) &&  // if we are doing WOY calculations, we are counting relative to Jan 1 *julian*
				($this->internalGet(YoudsFrameworkDateDefinitions::EXTENDED_YEAR) == $this->fGregorianCutoverYear) && $jd >= $this->fCutoverJulianDay) { 
		...
		if($this->fIsGregorian && ($this->internalGet(YoudsFrameworkDateDefinitions::EXTENDED_YEAR) == $this->fGregorianCutoverYear)) {
			$gregShift = YoudsFrameworkCalendarGrego::gregorianShift($this->internalGet(YoudsFrameworkDateDefinitions::EXTENDED_YEAR));
			if($bestField == YoudsFrameworkDateDefinitions::DAY_OF_YEAR) {
				$jd -= $gregShift;
			} elseif($bestField == YoudsFrameworkDateDefinitions::WEEK_OF_MONTH) {
				$weekShift = 14;
	...
	/**
	 * @see        YoudsFrameworkCalendar::handleGetMonthLength
	 * 
	 * @author     Dominik del Bondio <ddb@bitxtender.com>
	 * @author     The ICU Project
	 * @since      0.11.0
	...
	protected function handleGetMonthLength($extendedYear, $month)
	{
		...
		// modify the extended year value accordingly.
		if($month < 0 || $month > 11) { 
			$extendedYear += YoudsFrameworkToolkit::floorDivide($month, 12, $month); 
		}
	...
	/**
	 * @see        YoudsFrameworkCalendar::handleGetYearLength
	 * 
	 * @author     Dominik del Bondio <ddb@bitxtender.com>
	 * @author     The ICU Project
	 * @since      0.11.0
	...
	{
		$year = $this->internalGet(YoudsFrameworkDateDefinitions::EXTENDED_YEAR);
	...
	{
		return $this->isLeapYear($this->internalGet(YoudsFrameworkDateDefinitions::YEAR)) ? 366 : 365;
	}
	...
	{
		$monthLen = $this->monthLength($this->internalGet(YoudsFrameworkDateDefinitions::MONTH));
		$dom = $this->internalGet(YoudsFrameworkDateDefinitions::DATE);
		if($dom > $monthLen) 
			$this->set(YoudsFrameworkDateDefinitions::DATE, $monthLen);
	}
	...
	protected function getEpochDay()
	{
		...
		$wallSec = $this->internalGetTime() / 1000.0 + ($this->internalGet(YoudsFrameworkDateDefinitions::ZONE_OFFSET) + $this->internalGet(YoudsFrameworkDateDefinitions::DST_OFFSET)) / 1000.0;

		return floor($wallSec / (YoudsFrameworkDateDefinitions::MILLIS_PER_DAY / 1000.0));
	}

	/**
	 * @see        YoudsFrameworkCalendar::handleGetLimit
	 * 
	 * @author     Dominik del Bondio <ddb@bitxtender.com>
	 * @author     The ICU Project
	 * @since      0.11.0
	...
	/**
	 * @see        YoudsFrameworkCalendar::handleGetExtendedYear
	 * 
	 * @author     Dominik del Bondio <ddb@bitxtender.com>
	 * @author     The ICU Project
	 * @since      0.11.0
	...
	{
		$year = YoudsFrameworkDateDefinitions::EPOCH_YEAR;

		switch($this->resolveFields(self::$kYearPrecedence)) {
			case YoudsFrameworkDateDefinitions::EXTENDED_YEAR:
				$year = $this->internalGet(YoudsFrameworkDateDefinitions::EXTENDED_YEAR, YoudsFrameworkDateDefinitions::EPOCH_YEAR);
				break;

			case YoudsFrameworkDateDefinitions::YEAR:
				{
					// The year defaults to the epoch start, the era to AD
					$era = $this->internalGet(YoudsFrameworkDateDefinitions::ERA, self::AD);
					if($era == self::BC) {
						$year = 1 - $this->internalGet(YoudsFrameworkDateDefinitions::YEAR, 1); // Convert to extended year
					} else {
						$year = $this->internalGet(YoudsFrameworkDateDefinitions::YEAR, YoudsFrameworkDateDefinitions::EPOCH_YEAR);
					}
				}
				break;

			case YoudsFrameworkDateDefinitions::YEAR_WOY:
				$year = $this->handleGetExtendedYearFromWeekFields($this->internalGet(YoudsFrameworkDateDefinitions::YEAR_WOY), $this->internalGet(YoudsFrameworkDateDefinitions::WEEK_OF_YEAR));
				break;
	...
	/**
	 * @see        YoudsFrameworkCalendar::handleGetExtendedYearFromWeekFields
	 * 
	 * @author     Dominik del Bondio <ddb@bitxtender.com>
	 * @author     The ICU Project
	 * @since      0.11.0
	...
	/**
	 * @see        YoudsFrameworkCalendar::handleComputeFields
	 * 
	 * @author     Dominik del Bondio <ddb@bitxtender.com>
	 * @author     The ICU Project
	 * @since      0.11.0
	...
	protected function handleComputeFields($julianDay)
	{
		if($julianDay >= $this->fCutoverJulianDay) {
			...
			$julianEpochDay = $julianDay - (YoudsFrameworkDateDefinitions::JAN_1_1_JULIAN_DAY - 2);
		...
		$this->internalSet(YoudsFrameworkDateDefinitions::MONTH, $month);
		$this->internalSet(YoudsFrameworkDateDefinitions::DAY_OF_MONTH, $dayOfMonth);
		$this->internalSet(YoudsFrameworkDateDefinitions::DAY_OF_YEAR, $dayOfYear);
	...
	private static function computeJulianDayOfYear($isGregorian, $year, &$isLeap)
	{
		$isLeap = ($year % 4 == 0);
		$y = $year - 1;
		$julianDay = 365.0 * $y + floor($y / 4) + (YoudsFrameworkDateDefinitions::JAN_1_1_JULIAN_DAY - 3);

		if($isGregorian) {
			$isLeap = $isLeap && (($year % 100 != 0) || ($year % 400 == 0));
			// Add 2 because Gregorian calendar starts 2 days after Julian calendar
			$julianDay += YoudsFrameworkCalendarGrego::gregorianShift($year);
		}
	...
	private function validateFields()
	{
		for($field = 0; $field < YoudsFrameworkDateDefinitions::FIELD_COUNT; ++$field) {
			...
			if($field != YoudsFrameworkDateDefinitions::DATE &&
					$field != YoudsFrameworkDateDefinitions::DAY_OF_YEAR &&
					$this->_isSet($field) &&
		...
		// specially.
		if($this->_isSet(YoudsFrameworkDateDefinitions::DATE)) {
			$date = $this->internalGet(YoudsFrameworkDateDefinitions::DATE);
			if($date < $this->getMinimum(YoudsFrameworkDateDefinitions::DATE) ||
					$date > $this->monthLength($this->internalGet(YoudsFrameworkDateDefinitions::MONTH))) {
				return false;
			}
		}

		if($this->_isSet(YoudsFrameworkDateDefinitions::DAY_OF_YEAR)) {
			$days = $this->internalGet(YoudsFrameworkDateDefinitions::DAY_OF_YEAR);
		...
		if($this->_isSet(YoudsFrameworkDateDefinitions::DAY_OF_WEEK_IN_MONTH) &&
			0 == $this->internalGet(YoudsFrameworkDateDefinitions::DAY_OF_WEEK_IN_MONTH)) {
			return false;
		}
	...
	{
		return ((($stamp_a != YoudsFrameworkCalendar::kUnset && $stamp_b != YoudsFrameworkCalendar::kUnset)
			? max($stamp_a, $stamp_b)
			: YoudsFrameworkCalendar::kUnset
		));
	}
	...
	{
		return YoudsFrameworkDateDefinitions::EPOCH_START_AS_JULIAN_DAY + floor($millis / YoudsFrameworkDateDefinitions::MILLIS_PER_DAY);
	}
	...
	{
		return (($julian - YoudsFrameworkDateDefinitions::EPOCH_START_AS_JULIAN_DAY) * YoudsFrameworkDateDefinitions::MILLIS_PER_DAY);
	}
	...
	/**
	 * @see        YoudsFrameworkCalendar::haveDefaultCentury
	 * 
	 * @author     Dominik del Bondio <ddb@bitxtender.com>
	 * @author     The ICU Project
	 * @since      0.11.0
	...
	/**
	 * @see        YoudsFrameworkCalendar::defaultCenturyStart
	 * 
	 * @author     Dominik del Bondio <ddb@bitxtender.com>
	 * @author     The ICU Project
	 * @since      0.11.0
	...
	/**
	 * @see        YoudsFrameworkCalendar::defaultCenturyStartYear
	 * 
	 * @author     Dominik del Bondio <ddb@bitxtender.com>
	 * @author     The ICU Project
	 * @since      0.11.0
	...
	/**
	 * @see        YoudsFrameworkCalendar::getType
	 * 
	 * @author     Dominik del Bondio <ddb@bitxtender.com>
	 * @since      0.11.0
	 */
	...
	{
		return YoudsFrameworkCalendar::GREGORIAN;
	}
	...
	private static function initializeSystemDefaultCentury()
	{
		...
		if(self::$fgSystemDefaultCenturyStart == self::$fgSystemDefaultCentury) {
			...
			$calendar->setTime(YoudsFrameworkCalendar::getNow());
			$calendar->add(YoudsFrameworkDateDefinitions::YEAR, -80);

			$newStart = $calendar->getTime();
			$newYear  = $calendar->get(YoudsFrameworkDateDefinitions::YEAR);
