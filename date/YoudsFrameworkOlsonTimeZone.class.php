 ...
 * @version    $Id$
 */
class YoudsFrameworkOlsonTimeZone extends YoudsFrameworkTimeZone
{
	/**
	 * The transitions
	 *
	 ...
	 *
	 * @var        YoudsFrameworkSimpleTimeZone
	 * @since      0.11.0
	 */
	protected $finalZone; // owned, may be NULL

	const MAX_INT = 2147483647;
	const MAX_DBL = YoudsFrameworkCalendar::MAX_MILLIS;

	/**
	 * Constructor
	 *
	 * @see        YoudsFrameworkOlsonTimeZone::constructor()
	 * @see        YoudsFrameworkOlsonTimeZone::constructorOSA()
	 *
	 * @author     Dominik del Bondio <ddb@bitxtender.com>
	 * @author     The ICU Project
	 * @since      0.11.0
	 ...
	 * @since      0.11.0
	 */
	protected function constructorOSA(YoudsFrameworkTranslationManager $tm, $id, array $zoneInfo)
	{
		...
		if(!isset($zoneInfo['finalRule'])) {
			throw new YoudsFrameworkException($id);
		}
		...
		$rawOffset = $zoneInfo['finalRule']['offset'] * YoudsFrameworkDateDefinitions::MILLIS_PER_SECOND;
		...
		$this->finalMillis = YoudsFrameworkCalendarGrego::fieldsToDay($zoneInfo['finalRule']['startYear'], 0, 1) * YoudsFrameworkDateDefinitions::MILLIS_PER_DAY;

		if($zoneInfo['finalRule']['type'] == 'dynamic') {
			$fr = $zoneInfo['finalRule'];
			$this->finalZone = new YoudsFrameworkSimpleTimeZone(
				$tm, $rawOffset, $id, 
				...
				$fr['save'] * YoudsFrameworkDateDefinitions::MILLIS_PER_SECOND
				);
		} else {
			$this->finalZone = new YoudsFrameworkSimpleTimeZone($tm, $rawOffset, $id);
		}
	}
	 ...
	 *
	 * @param      YoudsFrameworkTimeZone The timezone to compare against.
	 * 
	 * @author     Dominik del Bondio <ddb@bitxtender.com>
	 * @author     The ICU Project
	 * @since      0.11.0
	...
	/**
	 * YoudsFrameworkTimeZone API.
	 *
	 * @see        YoudsFrameworkTimeZone::getOffsetIIIIII()
	 * 
	 * @author     Dominik del Bondio <ddb@bitxtender.com>
	 * @author     The ICU Project
	 * @since      0.11.0
	...
	protected function getOffsetIIIIII($era, $year, $month, $dom, $dow, $millis)
	{
		if($month < YoudsFrameworkDateDefinitions::JANUARY || $month > YoudsFrameworkDateDefinitions::DECEMBER) {
			...
			return $this->getOffsetIIIIIII($era, $year, $month, $dom, $dow, $millis, YoudsFrameworkCalendarGrego::monthLength($year, $month));
		}
	}
	 ...
	 * YoudsFrameworkTimeZone API.
	 *
	 * @see        YoudsFrameworkTimeZone::getOffsetIIIIIII()
	 * 
	 * @author     Dominik del Bondio <ddb@bitxtender.com>
	 * @author     The ICU Project
	 * @since      0.11.0
	...
	protected function getOffsetIIIIIII($era, $year, $month, $dom, $dow, $millis, $monthLength)
	{
		if(($era != YoudsFrameworkGregorianCalendar::AD && $era != YoudsFrameworkGregorianCalendar::BC)
				...
				|| $dom < 1
				|| $dom > $monthLength
				|| $dow < YoudsFrameworkDateDefinitions::SUNDAY
				|| $dow > YoudsFrameworkDateDefinitions::SATURDAY
				|| $millis < 0
		...
		if($era == YoudsFrameworkGregorianCalendar::BC) {
			$year = -$year;
		}
		...
		$time = YoudsFrameworkCalendarGrego::fieldsToDay($year, $month, $dom) * YoudsFrameworkDateDefinitions::SECONDS_PER_DAY + floor($millis / YoudsFrameworkDateDefinitions::MILLIS_PER_SECOND);

		$transition = $this->findTransition($time, true);
		return ($this->types[$transition['type']]['dstOffset'] + $this->types[$transition['type']]['rawOffset']) * YoudsFrameworkDateDefinitions::MILLIS_PER_SECOND;
	}

	/**
	 * YoudsFrameworkTimeZone API.
	 *
	 * @see        YoudsFrameworkTimeZone::getOffsetRef()
	 * 
	 * @author     Dominik del Bondio <ddb@bitxtender.com>
	 * @author     The ICU Project
	 * @since      0.11.0
	...
	public function getOffsetRef($date, $local, &$rawoff, &$dstoff)
	{
		...
		if($date >= $this->finalMillis && $this->finalZone !== null) {
			$millis = 0;
			$days = YoudsFrameworkToolkit::floorDivide($date, YoudsFrameworkDateDefinitions::MILLIS_PER_DAY, $millis);

			$year = 0; $month = 0; $dom = 0; $dow = 0;

			YoudsFrameworkCalendarGrego::dayToFields($days, $year, $month, $dom, $dow);
			...
			if(!$local) {
				...
				$date += $rawoff;
				$days2 = YoudsFrameworkToolkit::floorDivide($date, YoudsFrameworkDateDefinitions::MILLIS_PER_DAY, $millis);
				if($days2 != $days) {
					YoudsFrameworkCalendarGrego::dayToFields($days2, $year, $month, $dom, $dow);
				}
			}

			$dstoff = $this->finalZone->getOffset(YoudsFrameworkGregorianCalendar::AD, $year, $month, $dom, $dow, $millis) - $rawoff;
			return;
		}

		$secs = floor($date / YoudsFrameworkDateDefinitions::MILLIS_PER_SECOND);
		$transition = $this->findTransition($secs, $local);
		$rawoff = $this->types[$transition['type']]['rawOffset'] * YoudsFrameworkDateDefinitions::MILLIS_PER_SECOND;
		$dstoff = $this->types[$transition['type']]['dstOffset'] * YoudsFrameworkDateDefinitions::MILLIS_PER_SECOND;
	}

	/**
	 * YoudsFrameworkTimeZone API.
	 *
	 * @see        YoudsFrameworkTimeZone::setRawOffset()
	 * 
	 * @author     Dominik del Bondio <ddb@bitxtender.com>
	 * @author     The ICU Project
	 * @since      0.11.0
	 ...
	 * YoudsFrameworkTimeZone API.
	 *
	 * @see        YoudsFrameworkTimeZone::getRawOffset()
	 * 
	 * @author     Dominik del Bondio <ddb@bitxtender.com>
	 * @author     The ICU Project
	 * @since      0.11.0
	...
	public function getRawOffset()
	{
		$raw = 0;
		$dst = 0;
		$this->getOffsetRef(YoudsFrameworkCalendar::getNow(), false, $raw, $dst);
		return $raw;
	}
	 ...
	 * YoudsFrameworkTimeZone API.
	 * 
	 * @see        YoudsFrameworkTimeZone::useDaylightTime()
	 *
	 * @author     Dominik del Bondio <ddb@bitxtender.com>
	 * @author     The ICU Project
	 * @since      0.11.0
	...
	public function useDaylightTime()
	{
		...
		$days = floor(YoudsFrameworkCalendar::getNow() / YoudsFrameworkDateDefinitions::MILLIS_PER_DAY); // epoch days
		...
		$start = (int) YoudsFrameworkCalendarGrego::fieldsToDay($year, 0, 1) * YoudsFrameworkDateDefinitions::SECONDS_PER_DAY;
		$limit = (int) YoudsFrameworkCalendarGrego::fieldsToDay($year + 1, 0, 1) * YoudsFrameworkDateDefinitions::SECONDS_PER_DAY;
	 ...
	 * YoudsFrameworkTimeZone API.
	 *
	 * @see        YoudsFrameworkTimeZone::getDSTSavings()
	 * 
	 * @author     Dominik del Bondio <ddb@bitxtender.com>
	 * @author     The ICU Project
	 * @since      0.11.0
	 ...
	 * YoudsFrameworkTimeZone API.
	 *
	 * @see        YoudsFrameworkTimeZone::inDaylightTime()
	 * 
	 * @author     Dominik del Bondio <ddb@bitxtender.com>
	 * @author     The ICU Project
	 * @since      0.11.0
