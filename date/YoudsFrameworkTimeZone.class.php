 ...
 * @version    $Id$
 */
abstract class YoudsFrameworkTimeZone
{
	/**
	 * The translation manager instance.
	 *
	 * @var        YoudsFrameworkTranslationManager
	 */
	 ...
	 * 
	 * @return     YoudsFrameworkTimeZone The GMT time zone.
	 * 
	 * @author     Dominik del Bondio <ddb@bitxtender.com>
	 * @author     The ICU Project
	 * @since      0.11.0
	 */
	public static function getGMT(YoudsFrameworkTranslationManager $tm)
	{
		return new YoudsFrameworkSimpleTimeZone($tm, 0, 'GMT');
	}
	...
	public function getOffsetRef($date, $local, &$rawOffset, &$dstOffset)
	{
		...
		for($pass = 0; true; ++$pass) {
			...
			$day = floor($date / YoudsFrameworkDateDefinitions::MILLIS_PER_DAY);
			$millis = (int) ($date - $day * YoudsFrameworkDateDefinitions::MILLIS_PER_DAY);
			
			YoudsFrameworkCalendarGrego::dayToFields($day, $year, $month, $dom, $dow);
			
			$dstOffset = $this->getOffsetIIIIIII(YoudsFrameworkGregorianCalendar::AD, $year, $month, $dom, $dow, $millis, YoudsFrameworkCalendarGrego::monthLength($year, $month)) - $rawOffset;
			...
			$date += $dstOffset;
			if(floor($date / YoudsFrameworkDateDefinitions::MILLIS_PER_DAY) == $day) {
				break;
			}
		}
	}
	 ...
	 * @param      YoudsFrameworkLocale The locale in which to supply the display name.
	 *
	 ...
	 * @since      0.11.0
	 */
	public function getDisplayName($daylight = null, $style = null, YoudsFrameworkLocale $locale = null)
	{
		if($daylight === null) {
			$daylight = false;
			$style = self::LONG;
			$locale = $this->translationManager->getCurrentLocale();
		} elseif($daylight instanceof YoudsFrameworkLocale) {
			$locale = $daylight;
			$daylight = false;
			$style = self::LONG;
			...
			throw new InvalidArgumentException('Illegal arguments for YoudsFrameworkTimeZone::getDisplayName');
		}
	...
	public function formatOffset($daylight, $separator = ':', $prefix = 'GMT')
	{
		...
		$offsetSeconds = ((int) ($value / YoudsFrameworkDateDefinitions::MILLIS_PER_SECOND) % 60);
		if($offsetSeconds) {
	 ...
	 * 
	 * @param      YoudsFrameworkTimeZone The object to be compared with
	 * 
	 ...
	 * @since      0.11.0
	 */
	protected function __construct(YoudsFrameworkTranslationManager $tm, $id = '')
	{
	 ...
	 *                    GMT[+-]hh.
	 * 
	 * @return     YoudsFrameworkTimeZone A newly created YoudsFrameworkSimpleTimeZone with the 
	 ...
	 * @since      0.11.0
	 */
	public static function createCustomTimeZone(YoudsFrameworkTranslationManager $tm, $id)
	{
		$maxCustomHour = 23;
		$maxCustomMin = 59;
		$maxCustomSec = 59;
		...
		$tz = new YoudsFrameworkSimpleTimeZone($tm, $offset * 1000.0);
		$tz->setId($tz->formatOffset(false, ''));
		return $tz;
	}

	/**
	 * Returns true if the two TimeZones are equal. (The YoudsFrameworkTimeZone version 
	 ...
	 * @since      0.11.0
	 */
	public function __is_equal(YoudsFrameworkTimeZone $that) {
	 ...
	 * @since      0.11.0
	 */
	public function __is_not_equal(YoudsFrameworkTimeZone $that) {
		return get_class($this) != get_class($that) || $this->getId() != $that->getId();
	}
	 ...
	 * this time zone.
	 * This method is wasteful since it creates a new YoudsFrameworkGregorianCalendar and
	...
	{
		$cal = new YoudsFrameworkGregorianCalendar($this);
		$cal->setTime($date);
