...
/**
 * YoudsFrameworkTimeZoneDataParser allows you to retrieve the contents of the olson 
 ...
 * @version    $Id$
 */
class YoudsFrameworkTimeZoneDataParser
{
	/**
	 ...
	 * @since      0.11.0
	 */
	public function initialize(YoudsFrameworkContext $context)
	{
	...
	/**
	 * @see        YoudsFrameworkConfigParser::parse()
	 *
	 * @author     Dominik del Bondio <ddb@bitxtender.com>
	 * @since      0.11.0
	 */
	...
	protected function parseFile($file)
	{
		...
		while(list($i, $line) = each($zoneLines)) { // for($i = 0, $c = count($zoneLines); $i < $c; ++$i) {
			...
			if(preg_match('!^\s*Rule\s*(.*)!', $line, $match)) {
				...
				throw new YoudsFrameworkException('Unknown line ' . $line . ' in file ' . $file);
			}
		}
				...
				do {
					...
					foreach($activeRules as $activeRuleIdx => $activeRule) {
						if(!is_numeric($activeRule['endYear'])) {
							throw new YoudsFrameworkException('endYear should be numeric but was: ' . $activeRule['endYear']);
						}
			...
			for($z = 0; $z < $zoneRuleCnt; ++$z) {
				...
				if($lastZoneRule) {
					if(count($myRules) == 0) {
						...
						$cal->setTime($myRules[$i]['time'] * YoudsFrameworkDateDefinitions::MILLIS_PER_SECOND);
						// + 1 because this specifies the first year in which the final rule will apply
						$lastRuleStartYear = $cal->get(YoudsFrameworkDateDefinitions::YEAR) + 1;
					}

					if($activeSubRules !== null) {
						$cnt = count($activeSubRules);
						if($cnt != 0 && $cnt != 2) {
							throw new YoudsFrameworkException('unexpected active rule count ' . $cnt);
						}
						if($cnt == 0) {
							...
							$finalRule = array(
								...
								'start' => array(
									...
									'date' => null,
									'day_of_week' => null,
									'time' => $sr[$on]['at']['secondsInDay'] * YoudsFrameworkDateDefinitions::MILLIS_PER_SECOND,
									'type' => YoudsFrameworkSimpleTimeZone::WALL_TIME,
								),
								'end' => array(
									'month' => $sr[$off]['month'],
									'date' => null,
									'day_of_week' => null,
									'time' => $sr[$off]['at']['secondsInDay'] * YoudsFrameworkDateDefinitions::MILLIS_PER_SECOND,
									'type' => YoudsFrameworkSimpleTimeZone::WALL_TIME,
								),
							...
							for($i = 0; $i < count($sr); ++$i) {
								...
								if($sr[$i]['at']['type'] == 'standard') {
									$finalRule[$frIdx]['type'] = YoudsFrameworkSimpleTimeZone::STANDARD_TIME;
								} elseif($sr[$on]['at']['type'] == 'universal') {
									$finalRule[$frIdx]['type'] = YoudsFrameworkSimpleTimeZone::UTC_TIME;
								}
	...
	protected function getOnDate($year, $month, $dateDef, $atDef, $gmtOff, $dstOff)
	{
		...
		$cal->set(YoudsFrameworkDateDefinitions::MONTH, $month);
		if($dateDef['type'] == 'date') {
			$cal->set(YoudsFrameworkDateDefinitions::DATE, $dateDef['date']);
		} elseif($dateDef['type'] == 'last') {
			$daysInMonth = YoudsFrameworkCalendarGrego::monthLength($year, $month);
			$cal->set(YoudsFrameworkDateDefinitions::DATE, $daysInMonth);
			// loop backwards until we found the last occurrence of the day
			while($cal->get(YoudsFrameworkDateDefinitions::DAY_OF_WEEK) != $dateDef['day']) {
				$cal->roll(YoudsFrameworkDateDefinitions::DATE, -1);
			}
		} elseif($dateDef['type'] == '<=') {
			$cal->set(YoudsFrameworkDateDefinitions::DATE, $dateDef['date']);
			while($cal->get(YoudsFrameworkDateDefinitions::DAY_OF_WEEK) != $dateDef['day']) {
			...
			throw new YoudsFrameworkException('Unknown on type ' . $dateDef['type']);
		}
	 ...
	 *
	 * @return     int The definition of this month from YoudsFrameworkDateDefinitions.
	 *
	 * @author     Dominik del Bondio <ddb@bitxtender.com>
	 * @since      0.11.0
	 */
	...
	{
		static $months = array(YoudsFrameworkDateDefinitions::JANUARY => 'january', YoudsFrameworkDateDefinitions::FEBRUARY => 'february', YoudsFrameworkDateDefinitions::MARCH => 'march', YoudsFrameworkDateDefinitions::APRIL => 'april', YoudsFrameworkDateDefinitions::MAY => 'may', YoudsFrameworkDateDefinitions::JUNE => 'june', YoudsFrameworkDateDefinitions::JULY => 'july', YoudsFrameworkDateDefinitions::AUGUST => 'august', YoudsFrameworkDateDefinitions::SEPTEMBER => 'september', YoudsFrameworkDateDefinitions::OCTOBER => 'october', YoudsFrameworkDateDefinitions::NOVEMBER => 'november', YoudsFrameworkDateDefinitions::DECEMBER => 'december');
	 ...
	 *
	 * @return     int The definition of this day from YoudsFrameworkDateDefinitions.
	 *
	 * @author     Dominik del Bondio <ddb@bitxtender.com>
	 * @since      0.11.0
	 */
	...
	{
		static $days = array(YoudsFrameworkDateDefinitions::SUNDAY => 'sunday', YoudsFrameworkDateDefinitions::MONDAY => 'monday', YoudsFrameworkDateDefinitions::TUESDAY => 'tuesday', YoudsFrameworkDateDefinitions::WEDNESDAY => 'wednesday', YoudsFrameworkDateDefinitions::THURSDAY => 'thursday', YoudsFrameworkDateDefinitions::FRIDAY => 'friday', YoudsFrameworkDateDefinitions::SATURDAY => 'saturday');
