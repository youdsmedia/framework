...
/**
 * YoudsFrameworkExecutionFilter is the last filter registered for each filter chain.
 ...
 * @version    $Id$
 */
class YoudsFrameworkExecutionFilter extends YoudsFrameworkFilter implements YoudsFrameworkIActionFilter
{
	/*
	 ...
	 * @since      1.0.0
	 */
	public function startedCacheCreationCallback($eventType, array $groups, array $config, YoudsFrameworkExecutionContainer $container)
	{
	}
	 ...
	 *
	 * @see        YoudsFrameworkExecutionFilter::startedCacheCreationCallback()
	 *
	 * @param      int                     The type of the event that occurred.
	 *                                     See CACHE_CALLBACK_* constants.
	 * @param      array                   The groups.
	 ...
	 * @since      1.0.0
	 */
	public function abortedCacheCreationCallback($eventType, array $groups, array $config, YoudsFrameworkExecutionContainer $container)
	{
	}
	 ...
	 * @since      1.0.0
	 */
	public function finishedCacheCreationCallback($eventType, array $groups, array $config, YoudsFrameworkExecutionContainer $container)
	{
	}
	...
	public function checkCache(array $groups, $lifetime = null)
	{
		...
		$filename = YoudsFrameworkConfig::get('core.cache_dir') . DIRECTORY_SEPARATOR . self::CACHE_SUBDIR . DIRECTORY_SEPARATOR . implode(DIRECTORY_SEPARATOR, $groups) . '.cefcache';
	...
	public function readCache(array $groups)
	{
		...
		if($data !== false) {
			...
			throw new YoudsFrameworkException(sprintf('Failed to read cache file "%s"', $filename));
		}
	}
	...
	public function writeCache(array $groups, $data, $lifetime = null)
	{
		...
		@mkdir(YoudsFrameworkConfig::get('core.cache_dir') . DIRECTORY_SEPARATOR  . self::CACHE_SUBDIR . DIRECTORY_SEPARATOR . implode(DIRECTORY_SEPARATOR , array_slice($groups, 0, -1)), 0777, true);
		return file_put_contents(YoudsFrameworkConfig::get('core.cache_dir') . DIRECTORY_SEPARATOR . self::CACHE_SUBDIR . DIRECTORY_SEPARATOR . implode(DIRECTORY_SEPARATOR, $groups) . '.cefcache', serialize($data), LOCK_EX);
	}
	...
	public static function clearCache(array $groups = array())
	{
		...
		if(is_file(YoudsFrameworkConfig::get('core.cache_dir') . DIRECTORY_SEPARATOR . $path . '.cefcache')) {
			YoudsFrameworkToolkit::clearCache($path . '.cefcache');
		} else {
			YoudsFrameworkToolkit::clearCache($path);
		}
	}
	 ...
	 * @param      YoudsFrameworkExecutionContainer The execution container.
	 *
	 ...
	 * @since      0.11.0
	 */
	public function determineGroups(array $groups, YoudsFrameworkExecutionContainer $container)
	{
		$retval = array();
		...
		foreach($groups as $group) {
			...
			if(!is_scalar($val)) {
				throw new YoudsFrameworkUncacheableException('Group value is not a scalar, cannot construct a meaningful string representation.');
			}
	 ...
	 * @param      YoudsFrameworkExecutionContainer The container to use, if necessary.
	 *
	 ...
	 * @since      0.11.0
	 */
	public function getVariable($name, $source = 'string', $namespace = null, YoudsFrameworkExecutionContainer $container = null)
	{
		$val = $name;
		...
		switch($source) {
			...
			case 'configuration_directive':
				$val = YoudsFrameworkConfig::get($name);
				break;
			case 'constant':
				$val = constant($name);
				break;
			...
			case 'global_request_data':
				$val = $this->context->getRequest()->getRequestData()->get($namespace ? $namespace : YoudsFrameworkRequestDataHolder::SOURCE_PARAMETERS, $name);
				break;
			case 'locale':
			...
			case 'request_attribute':
				...
				break;
			case 'request_data':
				$val = $container->getRequestData()->get($namespace ? $namespace : YoudsFrameworkRequestDataHolder::SOURCE_PARAMETERS, $name);
				break;
			...
			case 'user_authenticated':
				if(($user = $this->context->getUser()) instanceof YoudsFrameworkISecurityUser) {
	 ...
	 * @param      YoudsFrameworkExecutionContainer The current execution container.
	 *
	 * @throws     <b>YoudsFrameworkInitializationException</b> If an error occurs during
	 *                                                 View initialization.
	 * @throws     <b>YoudsFrameworkViewException</b>           If an error occurs while
	 *                                                 executing the View.
	 *
	 * @author     David Zülke <dz@bitxtender.com>
	...
	public function execute(YoudsFrameworkFilterChain $filterChain, YoudsFrameworkExecutionContainer $container)
	{
		...
		$isCacheable = false;
		$cachingDotXml = YoudsFrameworkToolkit::evaluateModuleDirective(
			$moduleName,
			'youds.cache.path',
			array(
		...
		if($this->getParameter('enable_caching', true) && is_readable($cachingDotXml)) {
			...
			// no _once please!
			include(YoudsFrameworkConfigCache::checkConfig($cachingDotXml, $this->context->getName()));
		}
		...
		if($isCacheable) {
			...
			} catch(YoudsFrameworkUncacheableException $e) {
		...
		while(true) {
			...
			if($actionCache['view_name'] !== YoudsFrameworkView::NONE) {
				...
				if($viewCache['next'] instanceof YoudsFrameworkExecutionContainer) {
					...
					for($i = 0; $i < count($layers); $i++) {
						...
						foreach($layer->getSlots() as $slotName => $slotContainer) {
							...
							YoudsFrameworkArrayPathDefinition::setValue($slotName, $output, $slotResponse->getContent());
			...
			// action cache writing must occur here, so actions that return YoudsFrameworkView::NONE also get their cache written
	 ...
	 * @since      0.11.0
	 * 
	 * @deprecated since 1.0.0, use YoudsFrameworkExecutionContainer::runAction()
	 */
	protected function runAction(YoudsFrameworkExecutionContainer $container)
	{
	 ...
	 * @since      1.0.0
	 */
	protected function executeView(YoudsFrameworkExecutionContainer $container)
	{
		$outputType = $container->getOutputType()->getName();
		$request = $this->context->getRequest();
		$viewInstance = $container->getViewInstance();
