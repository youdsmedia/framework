...
/**
 * YoudsFrameworkFormPopulationFilter automatically populates a form that is re-posted,
 ...
 * @version    $Id$
 */
class YoudsFrameworkFormPopulationFilter extends YoudsFrameworkFilter implements YoudsFrameworkIGlobalFilter
{
	...
	public function execute(YoudsFrameworkFilterChain $filterChain, YoudsFrameworkExecutionContainer $container)
	{
		...
		$cfg = $rq->getAttributes('org.youds.filter.FormPopulationFilter');
		...
		if(is_array($cfg['populate']) || $cfg['populate'] instanceof YoudsFrameworkParameterHolder) {
		...
		$skip = null;
		if($cfg['skip'] instanceof YoudsFrameworkParameterHolder) {
		...
		if(isset($cfg['validation_report']) && $cfg['validation_report'] instanceof YoudsFrameworkValidationReport) {
		...
		if(libxml_get_last_error() !== false) {
			...
			if(YoudsFrameworkConfig::get('core.use_logging') && $cfg['log_parse_errors'] !== false && $maxError >= $cfg['log_parse_errors']) {
				$severity = YoudsFrameworkLogger::INFO;
				switch($maxError) {
					case LIBXML_ERR_WARNING:
						$severity = YoudsFrameworkLogger::WARN;
						break;
					case LIBXML_ERR_ERROR:
						$severity = YoudsFrameworkLogger::ERROR;
						break;
					case LIBXML_ERR_FATAL:
						$severity = YoudsFrameworkLogger::FATAL;
						break;
				}
				$lmsg = $emsg . "\n\nResponse content:\n\n" . $response->getContent();
		...
		if(!$utf8 && $encoding != self::ENCODING_ISO_8859_1 && !function_exists('iconv')) {
			throw new YoudsFrameworkException('No iconv module available, input encoding "' . $encoding . '" cannot be handled.');
		}
		...
		if(is_array($populate)) {
			...
			$forms = $this->xpath->query(YoudsFrameworkToolkit::expandVariables($cfg['forms_xpath'], array('htmlnsPrefix' => $this->xmlnsPrefix)));
		}
		...
		foreach($forms as $form) {
			if($form->tagName == 'form') {
				if($populate instanceof YoudsFrameworkParameterHolder) {
					...
					if(isset($populate[$form->getAttribute('id')])) {
						if($populate[$form->getAttribute('id')] instanceof YoudsFrameworkParameterHolder) {
			...
			foreach($this->xpath->query($query, $form) as $element) {
				...
				$argument = new YoudsFrameworkValidationArgument(
					$pname,
					($element->nodeName == 'input' && $element->getAttribute('type') == 'file')
						? YoudsFrameworkWebRequestDataHolder::SOURCE_FILES
						: YoudsFrameworkRequestDataHolder::SOURCE_PARAMETERS
				);
				
				// there's an error with the element's name in the request? good. let's give the baby a class!
				if($vr->getAuthoritativeArgumentSeverity($argument) > YoudsFrameworkValidator::SILENT) {
					...
					foreach($errorClassElements as $errorClassElement) {
						...
						foreach($cfg['error_class_map'] as $xpathExpression => $errorClassName) {
							...
							$errorClassResults = $this->xpath->query(YoudsFrameworkToolkit::expandVariables($xpathExpression, array('htmlnsPrefix' => $this->xmlnsPrefix)), $errorClassElement);
					...
					foreach($vr->byArgument($argument)->getIncidents() as $incident) {
						if(($incidentKey = array_search($incident, $allIncidents, true)) !== false) {
							...
							foreach($incidentArguments as $incidentArgument) {
								if(in_array($incidentArgument->getSource(), array(YoudsFrameworkWebRequestDataHolder::SOURCE_FILES, YoudsFrameworkRequestDataHolder::SOURCE_PARAMETERS))) {
		...
		$rq->setAttribute('orphaned_errors', $allIncidents, 'org.youds.filter.FormPopulationFilter');
			...
			if($firstError) {
				...
				switch($firstError) {
					...
					case PREG_RECURSION_LIMIT_ERROR:
						$error .= "the recursion limit (defined by \"pcre.recursion_limit\") was hit. This shouldn't happen unless you changed that limit yourself in php.ini or using ini_set(). If the problem is not on your end, please file a bug report with a reproduce case on the YoudsFramework issue tracker or drop by on the IRC support channel.";
						break;
					case PREG_BACKTRACK_LIMIT_ERROR:
						$error .= "the backtrack limit (defined by \"pcre.backtrack_limit\") was hit. This shouldn't happen unless you changed that limit yourself in php.ini or using ini_set(). If the problem is not on your end, please file a bug report with a reproduce case on the YoudsFramework issue tracker or drop by on the IRC support channel.";
						break;
					case PREG_INTERNAL_ERROR:
					default:
						$error .= "an internal PCRE error occurred. As a quick countermeasure, try to upgrade PHP (and the bundled PCRE) as well as libxml (yes!) to the latest versions to see if the problem goes away. If the issue persists, file a bug report with a reproduce case on the YoudsFramework issue tracker or drop by on the IRC support channel.";
				}
				throw new YoudsFrameworkException($error);
			}
	 ...
	 * @param      array      An array of YoudsFrameworkValidationIncidents.
	 *
	...
	protected function insertErrorMessages(DOMElement $element, array $rules, array $incidents)
	{
		...
		foreach($incidents as $incident) {
			if($incident->getSeverity() <= YoudsFrameworkValidator::SILENT) {
				continue;
			}
		...
		foreach($rules as $xpathExpression => $errorMessageInfo) {
			$targets = $this->xpath->query(YoudsFrameworkToolkit::expandVariables($xpathExpression, array('htmlnsPrefix' => $this->xmlnsPrefix)), $element);
			...
			if(!$errorMarkup && !$errorContainer) {
				throw new YoudsFrameworkException('Form Population Filter was unable to insert error messages into the document using the XPath expression "' . $xpathExpression . '" because the element information did not contain either a "markup" or "container" entry to use.');
			}
				...
				foreach($errors as $error) {
					if(is_callable($errorMarkup)) {
						...
						$errorElement->appendXML(
							YoudsFrameworkToolkit::expandVariables(
								$errorMarkup,
								array(
						...
						throw new YoudsFrameworkException('Form Population Filter was unable to insert an error message into the document using the XPath expression "' . $xpathExpression . '" because the element information could not be evaluated as an XML/HTML fragment or as a PHP callback.');
					}
			...
			if($errorContainer) {
				...
				if(is_callable($errorContainer)) {
					...
					throw new YoudsFrameworkException('Form Population Filter was unable to insert an error message container into the document using the XPath expression "' . $xpathExpression . '" because the element information could not be evaluated as an XML/HTML fragment or as a PHP callback.');
				}
	...
	public function initialize(YoudsFrameworkContext $context, array $parameters = array())
	{
		...
		$this->context->getRequest()->setAttributes($this->getParameters(), 'org.youds.filter.FormPopulationFilter');
	}
}
