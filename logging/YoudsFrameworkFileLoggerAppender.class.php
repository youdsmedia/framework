...
/**
 * YoudsFrameworkFileLoggerAppender appends YoudsFrameworkLoggerMessages to a given file.
 *
 * @package    youds
 * @subpackage logging
 *
 ...
 * @version    $Id$
 */
class YoudsFrameworkFileLoggerAppender extends YoudsFrameworkStreamLoggerAppender
{
	/**
	 ...
	 *
	 * @throws     <b>YoudsFrameworkLoggingException</b> if file cannot be opened for
	 *                                          appending.
	 *
	...
	protected function getHandle()
	{
		...
		if(is_null($this->handle) && (!is_writable(dirname($destination)) || (file_exists($destination) && !is_writable($destination)))) {
			throw new YoudsFrameworkLoggingException('Cannot open file "' . $destination . '", please check permissions on file or directory.');
		}
