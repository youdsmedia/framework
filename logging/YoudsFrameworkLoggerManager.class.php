...
/**
 * YoudsFrameworkLoggerManager provides accessibility and management of all loggers.
 *
 * @package    youds
 * @subpackage logging
 *
 ...
 * @version    $Id$
 */
class YoudsFrameworkLoggerManager
{
	/**
	 * @var        array An array of YoudsFrameworkLoggers.
	 */
	 ...
	 */
	protected $defaultMessageClass = 'YoudsFrameworkLoggerMessage';
	...
	/**
	 * Initialize this YoudsFrameworkLoggingManager.
	 *
	...
	public function initialize(YoudsFrameworkContext $context, array $parameters = array())
	{
		...
		require(YoudsFrameworkConfigCache::checkConfig(YoudsFrameworkConfig::get('core.config_dir') . '/logging.xml', $context->getName()));
	}
	 ...
	 *
	 * @return     YoudsFrameworkLogger A Logger, if a logger with the name exists,
	 *                         otherwise null.
	 *
	 * @author     David Zülke <dz@bitxtender.com>
	 * @author     Sean Kerr <skerr@mojavi.org>
	 ...
	 * @param      string A logger name.
	 *
	 * @return     YoudsFrameworkLogger A Logger, if the logger has been removed, else 
	 *                         null.
	 *
	 * @throws     <b>YoudsFrameworkLoggingException</b> If the logger name is default,
	 ...
	 *
	 * @param      string       A logger name.
	 * @param      YoudsFrameworkILogger A Logger instance.
	 *
	 * @throws     <b>YoudsFrameworkLoggingException</b> If a logger with the name already
	 *                                          exists.
	 *
	 * @author     David Zülke <dz@bitxtender.com>
	 * @author     Sean Kerr <skerr@mojavi.org>
	 * @since      0.9.0
	 */
	public function setLogger($name, YoudsFrameworkILogger $logger)
	{
		if(!isset($this->loggers[$name])) {
	 ...
	 *
	 * @throws     <b>YoudsFrameworkLoggingException</b> if the logger was not found.
	 *
	 * @author     Dominik del Bondio <ddb@bitxtender.com>
	 * @since      0.11.0
	 */
	...
	{
		if(!isset($this->loggers[$name])) {
			throw new YoudsFrameworkLoggingException('A logger with the name ' . $name . ' does not exist');
		}
	 ...
	 * Log a Message.
	 *
	 * @param      mixed A message to log - either an YoudsFrameworkLoggerMessage instance,
	 *                   or a message string.
	 * @param      mixed Optional logger to log to (instance or name), or an int
	 *                   with the severity of the message.
	 *
	 * @throws     YoudsFrameworkLoggingException if the logger was not found.
	 *
	 * @author     David Zülke <dz@bitxtender.com>
	...
	{
		if(!($message instanceof YoudsFrameworkLoggerMessage)) {
		...
		if($loggerOrSeverity === null) {
		...
		} elseif($loggerOrSeverity instanceof YoudsFrameworkILogger) {
			...
			// nothing found? bark!
			throw new YoudsFrameworkLoggingException(sprintf('Logger "%s" has not been configured.', $loggerOrSeverity));
		}
