...
/**
 * YoudsFrameworkScribeLoggerAppender sends YoudsFrameworkLoggerMessages to a Scribe server or an
 ...
 * @version    $Id$
 */
class YoudsFrameworkScribeLoggerAppender extends YoudsFrameworkLoggerAppender
{
	/**
	...
	protected function getScribeClient()
	{
		if(!$this->scribeClient) {
			...
			try {
				...
				$this->transport = null;
				throw new YoudsFrameworkLoggingException(sprintf("Failed to connect to Scribe server:\n\n%s", $e->getMessage()), 0, $e);
			}
		}
	...
	public function shutdown()
	{
		try {
			$this->flush();
		} catch(YoudsFrameworkLoggingException $e) {
