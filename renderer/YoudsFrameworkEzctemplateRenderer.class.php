 ...
 * @version    $Id$
 */
class YoudsFrameworkEzctemplateRenderer extends YoudsFrameworkRenderer implements YoudsFrameworkIReusableRenderer
{
	/**
	 ...
	 * Returns an instance of YoudsFrameworkEzctemplateTemplate by default.
	 *
	...
	{
		$cls = $this->getParameter('template_class', 'YoudsFrameworkEzctemplateTemplate');
		
		$ezcTemplate = new $cls();
		if($ezcTemplate instanceof YoudsFrameworkIEzctemplateTemplate) {
	...
	protected function getEngine()
	{
		...
		$parentMode = fileperms(YoudsFrameworkConfig::get('core.cache_dir'));

		$compileDir = YoudsFrameworkConfig::get('core.cache_dir') . DIRECTORY_SEPARATOR . self::COMPILE_DIR . DIRECTORY_SEPARATOR . self::COMPILE_SUBDIR;
		YoudsFrameworkToolkit::mkdir($compileDir, $parentMode, true);

		// templatePath unnessesary because YoudsFramework will always supply the absolute ressource path
		...
		// add some usefull YoudsFramework Functions/Blocks as Extension
		$config->addExtension('YoudsFrameworkEzctemplateCustomBlocks');
		$config->addExtension('YoudsFrameworkEzctemplateCustomFunctions');
