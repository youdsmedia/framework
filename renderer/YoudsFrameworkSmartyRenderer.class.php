 ...
 * @version    $Id$
 */
class YoudsFrameworkSmartyRenderer extends YoudsFrameworkRenderer implements YoudsFrameworkIReusableRenderer
{
	/**
	...
	protected function getEngine()
	{
		...
		if($this->isSmarty2) {
			$this->smarty->config_dir = YoudsFrameworkConfig::get('core.config_dir');
		} else {
			$this->smarty->setConfigDir(YoudsFrameworkConfig::get('core.config_dir'));
		}
		...
		$cacheDir = YoudsFrameworkConfig::get('core.cache_dir') . DIRECTORY_SEPARATOR . self::CACHE_DIR;
		YoudsFrameworkToolkit::mkdir($cacheDir, $parentMode, true);
		if($this->isSmarty2) {
		...
		if(YoudsFrameworkConfig::get('core.debug', false)) {
