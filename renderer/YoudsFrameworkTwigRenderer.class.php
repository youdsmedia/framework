 ...
 * @version    $Id$
 */
class YoudsFrameworkTwigRenderer extends YoudsFrameworkRenderer implements YoudsFrameworkIReusableRenderer
{
	/**
	...
	public function initialize(YoudsFrameworkContext $context, array $parameters = array())
	{
		...
		$this->setParameter('options', array_merge(
			array(
				'debug' => YoudsFrameworkConfig::get('core.debug'),
				'cache' => YoudsFrameworkConfig::get('core.debug') ? false : YoudsFrameworkConfig::get('core.cache_dir') . DIRECTORY_SEPARATOR . 'templates' . DIRECTORY_SEPARATOR . 'twig',
			),
	...
	public function render(YoudsFrameworkTemplateLayer $layer, array &$attributes = array(), array &$slots = array(), array &$moreAssigns = array())
	{
		...
		if($layer instanceof YoudsFrameworkFileTemplateLayer) {
			...
			foreach((array)$this->getParameter('template_dirs', array(YoudsFrameworkConfig::get('core.template_dir'))) as $dir) {
				$paths[] = $dir;
			}
