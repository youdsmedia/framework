...
/**
 * YoudsFrameworkXsltRenderer uses an XML Stylesheet Language Template to render the
 ...
 * @version    $Id$
 */
class YoudsFrameworkXsltRenderer extends YoudsFrameworkRenderer implements YoudsFrameworkIReusableRenderer
{
	const ENVELOPE_XMLNS = 'http://youds.com/youds/renderer/xslt/envelope/1.0';
	...
	public function render(YoudsFrameworkTemplateLayer $layer, array &$attributes = array(), array &$slots = array(), array &$moreAssigns = array())
	{
		if($this->getParameter('envelope', true)) {
			if(!($moreAssigns['inner'] instanceof DOMDocument)) {
				...
				try {
					...
					throw new YoudsFrameworkRenderException(sprintf("Unable to load input document for layer '%s'.\n\n%s", $layer->getName(), $e->getMessage()), 0, $e);
				}
			} else {
			...
			$flattenedSlots = YoudsFrameworkArrayPathDefinition::flatten($slots);
			foreach($flattenedSlots as $slotName => $slotContent) {
				if(!($slotContent instanceof DOMDocument)) {
					try {
						...
						throw new YoudsFrameworkRenderException(sprintf("Unable to load contents for slot '%s'.\n\n%s", $slotName, $e->getMessage()), 0, $e);
					}
				} else {
					$slot = $slotContent;
				}
		...
		try {
			...
			throw new YoudsFrameworkRenderException(sprintf("Unable to load template '%s'.\n\n%s", $layer->getResourceStreamIdentifier(), $e->getMessage()), 0, $e);
		}
