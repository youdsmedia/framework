...
/**
 * YoudsFrameworkRequest provides methods for manipulating client request information
 ...
 * @version    $Id$
 */
abstract class YoudsFrameworkRequest extends YoudsFrameworkAttributeHolder
{
	/**
	...
	/**
	 * @var        YoudsFrameworkRequestDataHolder The request data holder instance.
	 */
	...
	public function __construct()
	{
		$this->setParameters(array(
			...
			'request_data_holder_class' => 'YoudsFrameworkRequestDataHolder',
		));
	}
	 ...
	 * @since      0.11.0
	 */
	final protected function setRequestData(YoudsFrameworkRequestDataHolder $rd)
	{
		if(!$this->isLocked()) {
			$this->requestData = $rd;
		}
	...
	{
		if($this->isLocked()) {
			throw new YoudsFrameworkException("Access to request data is locked during Action and View execution and while templates are rendered. Please use the local request data holder passed to your Action's or View's execute*() method to access request data.");
		}
	...
	public function startup()
	{
		if($this->getParameter('unset_input', true)) {
			...
			if($rla) {
				trigger_error('Support for php.ini directive "register_long_arrays" is deprecated and will be dropped in YoudsFramework 1.2. The setting is deprecated in PHP 5.3 and will be removed in PHP 5.4. Please refer to the PHP manual for details.', E_USER_DEPRECATED);
			}
	...
	public final function toggleLock($key = null)
	{
		if(!$this->isLocked() && $key === null) {
			return $this->key = YoudsFrameworkToolkit::uniqid();
