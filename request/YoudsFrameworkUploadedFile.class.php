...
/**
 * YoudsFrameworkUploadedFile is a container with information for files that were
 ...
 * @version    $Id$
 */
class YoudsFrameworkUploadedFile implements ArrayAccess
{
	/**
	...
	{
		trigger_error('Property access in YoudsFrameworkUploadedFile is deprecated and will be removed in YoudsFramework 1.2. Please use getter methods or array access instead.', E_USER_DEPRECATED);
	 ...
	 *
	 * @throws     BadMethodCallException YoudsFrameworkUploadedFile objects are immutable.
	 *
	 * @author     David Zülke <david.zuelke@bitextender.com>
	 * @since      1.1.0
	 */
	...
	{
		throw new BadMethodCallException('YoudsFrameworkUploadedFile objects are immutable.');
	}
	...
	public function getTmpName()
	{
		if(!$this->hasTmpName() && !$this->hasOpenStream()) {
			...
			$this->stream = tmpfile();
			if(!$this->stream) {
				throw new YoudsFrameworkFileException('Cannot create temporary file via tmpfile()');
			}
	 ...
	 * or another type of file submission handled by YoudsFramework.
	 *
	 ...
	 *
	 * @throws     YoudsFrameworkException If the file has errors or has been moved.
	 *
	 * @author     David Zülke <dz@bitxtender.com>
	...
	public function getContents()
	{
		...
		if($this->hasError() || (!$this->isMovable() && !$this->hasBufferedContents() && !$this->hasOpenStream())) {
			throw new YoudsFrameworkException('Cannot get contents of erroneous or moved file.');
		}
	 ...
	 *
	 * @throws     YoudsFrameworkFileException If chmod or mkdir calls failed.
	 *
	 * @author     David Zülke <dz@bitxtender.com>
	 * @since      0.11.0
	 */
	public function move($dest, $fileMode = 0664, $create = true, $dirMode = 0775)
	{
		...
		if(!is_readable($directory)) {
			if($create && !YoudsFrameworkToolkit::mkdir($directory, $dirMode, true)) {
				// failed to create the directory
				$error = 'Failed to create file upload directory "%s"';
				$error = sprintf($error, $directory);
				throw new YoudsFrameworkFileException($error);
			}
		...
		if($moved) {
			...
			if(!@chmod($dest, $fileMode)) {
				throw new YoudsFrameworkFileException('Failed to chmod uploaded file after moving');
			}
		} else {
			// moving the file failed
			throw new YoudsFrameworkFileException('Failed to move uploaded file');
		}
