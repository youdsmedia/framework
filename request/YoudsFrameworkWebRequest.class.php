...
/**
 * YoudsFrameworkWebRequest provides additional support for web-only client requests
 ...
 * @version    $Id$
 */
class YoudsFrameworkWebRequest extends YoudsFrameworkRequest
{
	/**
	...
	public function getUrlAuthority($forcePort = false)
	{
		...
		return $this->getUrlHost() . ($forcePort || YoudsFrameworkToolkit::isPortNecessary($scheme, $port) ? ':' . $port : '');
	}
	...
	public function __construct()
	{
		...
		$this->setParameters(array(
			'request_data_holder_class' => 'YoudsFrameworkWebRequestDataHolder',
		));
	}
	...
	public final static function clearMagicQuotes($input)
	{
		...
		// http://trac.youds.com/ticket/953
		// http://trac.youds.com/ticket/944
		// http://bugs.php.net/bug.php?id=41093
	...
	public function initialize(YoudsFrameworkContext $context, array $parameters = array())
	{
		...
		if(get_magic_quotes_gpc()) {
			trigger_error('Support for php.ini directive "magic_quotes_gpc" is deprecated and will be dropped in YoudsFramework 1.2. The setting is deprecated in PHP 5.3 and will be removed in PHP 5.4. Please refer to the PHP manual for details.', E_USER_DEPRECATED);
		...
		if(isset($_SERVER['SERVER_SOFTWARE']) && preg_match('#^Apache(/\d+(\.\d+)?)?\.?$#', $_SERVER['SERVER_SOFTWARE'])) {
			throw new YoudsFrameworkException(
				...
				"YoudsFramework needs detailed Apache version information to apply URL decoding and parsing workarounds specific to certain versions of Apache that exhibit buggy behavior.\n\n" .
				...
				"http://trac.youds.com/ticket/1029\n\n" .
		...
		// the result is an XSS attack vector, e.g. on YoudsFrameworkWebRouting::gen(null)
		...
		$files = array();
		$ufc = $this->getParameter('uploaded_file_class', 'YoudsFrameworkUploadedFile');
	 ...
	 * The cleaned up array of YoudsFrameworkUploadedFile objects is returned.
	 *
	 ...
	 * @since      1.1.0
	 */
	protected static function fixFilesArray($input, $uploadedFileClass = 'YoudsFrameworkUploadedFile', $index = array(), &$output = array())
	{
		$fromIndex = $index;
		...
		$sub = YoudsFrameworkArrayPathDefinition::getValue($fromIndex, $input);
		$theIndices = array();
			...
			foreach($sub as $key => $value) {
				...
				if(is_array($value)) {
					...
					foreach($theIndices as $name => $theIndex) {
						$data[$name] = YoudsFrameworkArrayPathDefinition::getValue(array_merge($theIndex, array($key)), $input, $name == 'is_uploaded_file' ? true : null);
					}
					$data = new $uploadedFileClass($data);
					YoudsFrameworkArrayPathDefinition::setValue($toIndex, $output, $data);
				}
			}
		} else {
			$data = array();
			foreach($theIndices as $name => $theIndex) {
				$data[$name] = YoudsFrameworkArrayPathDefinition::getValue($theIndex, $input, $name == 'is_uploaded_file' ? true : null);
			}
			$data = new $uploadedFileClass($data);
			YoudsFrameworkArrayPathDefinition::setValue($index, $output, $data);
		}
