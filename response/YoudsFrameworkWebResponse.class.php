...
/**
 * YoudsFrameworkWebResponse handles HTTP responses.
 *
 * @package    youds
 * @subpackage response
 *
 ...
 * @version    $Id$
 */
class YoudsFrameworkWebResponse extends YoudsFrameworkResponse
{
	/**
	...
	public function initialize(YoudsFrameworkContext $context, array $parameters = array())
	{
		...
		// if 'cookie_secure' is set, and null, then we need to set whatever YoudsFrameworkWebRequest::isHttps() returns
	 ...
	 *
	 * @param      YoudsFrameworkOutputType An optional Output Type object with information
	 ...
	 * @since      0.11.0
	 */
	public function send(YoudsFrameworkOutputType $outputType = null)
	{
		if($this->redirect) {
	 ...
	 *
	 * @param      YoudsFrameworkResponse The other response to import information from.
	 *
	 * @author     David Zülke <dz@bitxtender.com>
	 * @since      0.11.0
	 */
	public function merge(YoudsFrameworkResponse $otherResponse)
	{
		parent::merge($otherResponse);
		
		if($otherResponse instanceof YoudsFrameworkWebResponse) {
	...
	public function setHttpStatusCode($code)
	{
		...
		if($this->validateHttpStatusCode($code)) {
			...
			throw new YoudsFrameworkException(sprintf('Invalid %s Status code: %s', $this->context->getRequest()->getProtocol(), $code));
		}
	}
	 ...
	 *
	 * @throws     YoudsFrameworkException If $encodeCallback is neither false nor callable.
	 * 
	...
	public function setCookie($name, $value, $lifetime = null, $path = null, $domain = null, $secure = null, $httponly = null, $encodeCallback = null)
	{
		...
		if($encodeCallback !== false && !is_callable($encodeCallback)) {
			throw new YoudsFrameworkException(sprintf('setCookie() $encodeCallback argument is not callable: %s', $encodeCallback));
		}
	...
	public function unsetCookie($name, $path = null, $domain = null, $secure = null, $httponly = null)
	{
		...
		// null for the lifetime, since YoudsFramework automatically sets that when the value is false or null
	 ...
	 * @since      0.11.0
	 */
	protected function sendHttpResponseHeaders(YoudsFrameworkOutputType $outputType = null)
	{
		if($outputType === null) {
		...
		if($this->getParameter('expose_youds', true) && !$this->hasHttpHeader('X-Powered-By')) {
			if(YoudsFrameworkConfig::get('expose_youds_version', $expose_php = ini_get('expose_php'))) {
				$xpbh = YoudsFrameworkConfig::get('youds.release');
			} else {
				$xpbh = YoudsFrameworkConfig::get('youds.name');
			}
			if($expose_php) {
		...
		if($routing instanceof YoudsFrameworkWebRouting) {
	...
	{
		if(!$this->validateHttpStatusCode($code)) {
			throw new YoudsFrameworkException(sprintf('Invalid %s Redirect Status code: %s', $this->context->getRequest()->getProtocol(), $code));
		}
