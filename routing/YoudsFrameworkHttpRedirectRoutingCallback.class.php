...
/**
 * YoudsFrameworkHttpRedirectRoutingCallback allows redirection of a matched route to a
 ...
 * @version    $Id$
 */
class YoudsFrameworkHttpRedirectRoutingCallback extends YoudsFrameworkRoutingCallback
{
	/**
	...
	public function initialize(YoudsFrameworkContext $context, array &$route)
	{
		...
		// sanity check
		if(!($context->getRouting() instanceof YoudsFrameworkWebRouting)) {
			throw new YoudsFrameworkException('YoudsFrameworkHttpRedirectRoutingCallback can only be used in combination with YoudsFrameworkWebRouting.');
		}
	}
	...
	public function onMatched(array &$parameters, YoudsFrameworkExecutionContainer $container)
	{
		...
		if($this->hasParameter('route')) {
			...
			array_walk_recursive($arguments, function(&$argument) use($parameters) { $argument = YoudsFrameworkToolkit::expandVariables($argument, $parameters); });
			...
			$url = YoudsFrameworkToolkit::expandVariables(
			...
			if($parts) {
				$url = YoudsFrameworkToolkit::buildUrl(array_merge(parse_url($this->getContext()->getRequest()->getUrl()), $parts));
			} else {
		...
		if(!($response instanceof YoudsFrameworkWebResponse)) {
			throw new YoudsFrameworkException('YoudsFrameworkHttpRedirectRoutingCallback can only be used in combination with YoudsFrameworkWebResponse.');
		}
