...
/**
 * YoudsFrameworkRouting allows you to centralize your entry point urls in your web
 * application.
 *
 * @package    youds
 * @subpackage routing
 ...
 * @version    $Id$
 */
abstract class YoudsFrameworkRouting extends YoudsFrameworkParameterHolder
{
	const ANCHOR_NONE = 0;
	...
	/**
	 * @var        array An array of YoudsFrameworkRoutingArraySource.
	 */
	...
	public function __construct()
	{
		...
		$this->setParameter('enabled', YoudsFrameworkConfig::get('core.use_routing', true));
	 ...
	 *
	 * @param      YoudsFrameworkContext The Context.
	...
	{
		$cfg = YoudsFrameworkConfig::get('core.config_dir') . '/routing.xml';
		// allow missing routing.xml when routing is not enabled
		if($this->isEnabled() || is_readable($cfg)) {
			$this->importRoutes(unserialize(file_get_contents(YoudsFrameworkConfigCache::checkConfig($cfg, $this->context->getName()))));
		}
	}
	...
	{
		$this->sources['_ENV'] = new YoudsFrameworkRoutingArraySource($_ENV);

		$this->sources['_SERVER'] = new YoudsFrameworkRoutingArraySource($_SERVER);

		if(YoudsFrameworkConfig::get('core.use_security')) {
			$this->sources['user'] = new YoudsFrameworkRoutingUserSource($this->context->getUser());
		}
	}
	 ...
	 *
	 * @return     YoudsFrameworkContext A Context instance.
	 *
	 * @author     Dominik del Bondio <ddb@bitxtender.com>
	 * @since      0.11.0
	 */
	...
	public function addRoute($route, array $options = array(), $parent = null)
	{
		...
		if(isset($options['name']) && isset($this->routes[$options['name']])) {
			...
			if($parent !== null && $defaultOpts['parent'] != $parent) {
				throw new YoudsFrameworkException('You are trying to overwrite a route but are not staying in the same hierarchy');
			}
			...
			$defaultOpts = array('name' => YoudsFrameworkToolkit::uniqid(), 'stop' => true, 'output_type' => null, 'module' => null, 'action' => null, 'parameters' => array(), 'ignores' => array(), 'defaults' => array(), 'childs' => array(), 'callbacks' => array(), 'imply' => false, 'cut' => null, 'source' => null, 'method' => null, 'constraint' => array(), 'locale' => null, 'pattern_parameters' => array(), 'optional_parameters' => array(), 'parent' => $parent, 'reverseStr' => '', 'nostops' => array(), 'anchor' => self::ANCHOR_NONE);
		}
		...
		foreach($this->routes as $name => $route) {
			...
			if($route['opt']['parent'] == $routeName && !in_array($name, $options['childs'])) {
				throw new YoudsFrameworkException('The route ' . $routeName . ' specifies a child route with the same name');
			}
		}
	...
	public function getAffectedRoutes($route, &$isNullRoute = false)
	{
		...
		if($route === null) {
			$includedRoutes = array_reverse($this->getContext()->getRequest()->getAttribute('matched_routes', 'org.youds.routing', array()));
			$isNullRoute = true;
	 ...
	 *
	 * @throws     YoudsFrameworkException If the given preset name doesn't exist.
	 *
	 * @author     David Zülke <dz@bitxtender.com>
	 * @since      0.11.0
	 */
	protected function resolveGenOptions($input = array())
	{
			...
			foreach($input as $key => $value) {
				if(is_numeric($key)) {
					...
					if(isset($this->genOptionsPresets[$value])) {
						...
						throw new YoudsFrameworkException('Undefined Routing gen() options preset "' . $value . '"');
					}
				} else {
		...
		throw new YoudsFrameworkException('Unexpected type "' . gettype($input) . '" used as Routing gen() option preset identifier');
	}
		...
		foreach($routeNames as $routeName) {
			...
			if(count($r['opt']['callbacks']) > 0) {
				...
				foreach($r['callback_instances'] as $callbackInstance) {
					...
					if($callbackInstance instanceof YoudsFrameworkILegacyRoutingCallback) {
						...
						foreach($paramsCopy as &$param) {
							if($param instanceof YoudsFrameworkIRoutingValue) {
								$param = $param->getValue();
							}
						}
						foreach($defaultsCopy as &$default) {
							if($default instanceof YoudsFrameworkIRoutingValue) {
								$default = array(
					...
					if(count($diff)) {
						...
						foreach($diffKeys as $key) {
							...
							if($value !== null && !($value instanceof YoudsFrameworkIRoutingValue)) {
								$routingValue = $this->createValue($value, false);
								if(isset($myDefaults[$key])) {
									if($myDefaults[$key] instanceof YoudsFrameworkIRoutingValue) {
			...
			if(!(isset($finalParams[$name]) || array_key_exists($name, $finalParams))) {
				if($param === null && isset($optionalParams[$name])) {
					...
					if(isset($defaultParams[$name])) {
						if($param === null || ($param instanceof YoudsFrameworkIRoutingValue && $param->getValue() === null)) {
	 ...
	 * 
	 * @param      mixed An YoudsFrameworkIRoutingValue object or a string
	 * 
	...
	{
		if($parameter instanceof YoudsFrameworkIRoutingValue) {
	...
	/**
	 * Converts all members of an array to YoudsFrameworkIRoutingValues.
	 * 
	 * @param      array The parameters
	 * 
	 * @return     array An array containing all parameters as YoudsFrameworkIRoutingValues
	 * 
	 * @author     Dominik del Bondio <dominik.del.bondio@bitextender.com>
	 * @since      1.0.0
	 */
	...
	/**
	 * Generate a formatted YoudsFramework URL.
	 *
	...
	public function gen($route, array $params = array(), $options = array())
	{
		...
		foreach($extras as &$extra) {
			$extra = ($extra instanceof YoudsFrameworkIRoutingValue) ? $extra->getValue() : $extra;
		}
	 ...
	 * parameter.
	 *
	 * @return     mixed An YoudsFrameworkExecutionContainer as a result of this execution,
	 *                   or an YoudsFrameworkResponse if a callback returned one.
	 *
	 * @author     Dominik del Bondio <ddb@bitxtender.com>
	 * @since      0.11.0
	 */
	public function execute()
	{
			...
			foreach($routes as $key) {
				...
				if(count($opts['constraint']) == 0 || in_array($requestMethod, $opts['constraint'])) {
					...
					if($this->parseInput($route, $input, $match)) {
						...
						if($opts['module']) {
							$module = YoudsFrameworkToolkit::expandVariables($opts['module'], $matchvals);
						...
						if($opts['action']) {
							$action = YoudsFrameworkToolkit::expandVariables($opts['action'], $matchvals);
						...
						if($opts['output_type']) {
							...
							$ot = YoudsFrameworkToolkit::expandVariables($opts['output_type'], $matchvals);
						...
						if($opts['locale']) {
							...
							if($locale = YoudsFrameworkToolkit::expandVariables($opts['locale'], $matchvals)) {
						...
						if($opts['method']) {
							...
							if($method = YoudsFrameworkToolkit::expandVariables($opts['method'], $matchvals)) {
						...
						if(count($opts['callbacks']) > 0) {
							...
							foreach($route['callback_instances'] as $callbackInstance) {
								...
								if($callbackSuccess) {
									...
									if($onMatched instanceof YoudsFrameworkResponse) {
										return $onMatched;
									}
									if(!$onMatched) {
								...
								if(!$callbackSuccess) {
									...
									if($onNotMatched instanceof YoudsFrameworkResponse) {
										return $onNotMatched;
									}
								...
								if($opts['module'] && $oldModule == $container->getModuleName() && (!$umap || !array_key_exists($ma, $vars) || $oldModule == $vars[$ma])) {
									$module = YoudsFrameworkToolkit::expandVariables($opts['module'], $expandVars);
									$container->setModuleName($module);
									if($umap) {
										$vars[$ma] = $module;
									}
								...
								if($opts['action'] && $oldAction == $container->getActionName() && (!$umap || !array_key_exists($aa, $vars) || $oldAction == $vars[$aa])) {
									$action = YoudsFrameworkToolkit::expandVariables($opts['action'], $expandVars);
									$container->setActionName($action);
									if($umap) {
										$vars[$aa] = $action;
									}
								...
								if($opts['output_type'] && $oldOutputTypeName == ($container->getOutputType() ? $container->getOutputType()->getName() : null)) {
									$ot = YoudsFrameworkToolkit::expandVariables($opts['output_type'], $expandVars);
									$container->setOutputType($this->context->getController()->getOutputType($ot));
								}
								if($opts['locale'] && $oldLocale == $tm->getCurrentLocaleIdentifier()) {
									if($locale = YoudsFrameworkToolkit::expandVariables($opts['locale'], $expandVars)) {
								...
								if($opts['method']) {
									if($oldRequestMethod == $rq->getMethod() && $oldContainerMethod == $container->getRequestMethod()) {
										if($method = YoudsFrameworkToolkit::expandVariables($opts['method'], $expandVars)) {
											// see above for the reason of the if
											$rq->setMethod($method);
											$container->setRequestMethod($method);
										}
		...
		if($container->getModuleName() === null || $container->getActionName() === null) {
			...
			$container->setModuleName(YoudsFrameworkConfig::get('actions.error_404_module'));
			$container->setActionName(YoudsFrameworkConfig::get('actions.error_404_action'));
		...
		$rq->setAttribute('matched_routes', $matchedRoutes, 'org.youds.routing');
	...
	{
		if($route['opt']['source'] !== null) {
			$parts = YoudsFrameworkArrayPathDefinition::getPartsFromPath($route['opt']['source']);
			...
			if($count > 0 && isset($this->sources[$partArray[0]])) {
				...
				if($count > 1) {
					...
					if(is_array($input)) {
						$input = YoudsFrameworkArrayPathDefinition::getValue($partArray, $input);
					} elseif($input instanceof YoudsFrameworkIRoutingSource) {
			...
			if($state == 'start') {
				...
				if($c == '{') {
					...
					if($parenthesisCount > 0) {
						...
						if($parenthesisCount < 0) {
							throw new YoudsFrameworkException('The pattern ' . $str . ' contains an unbalanced set of parentheses!');
						}
						...
						if(!$hasBrackets) {
							...
							if($bracketCount != 0) {
								throw new YoudsFrameworkException('The pattern ' . $str . ' contains an unbalanced set of brackets!');
							}
							$rxPostfix = $tmpStr;
						}
	...
	{
		$value = new YoudsFrameworkRoutingValue($value, $valueNeedsEncoding);
