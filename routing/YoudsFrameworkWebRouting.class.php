...
/**
 * YoudsFrameworkWebRouting sets the prefix and input with some magic from the request
 * uri and path_info
 *
 * @package    youds
 * @subpackage routing
 ...
 * @version    $Id$
 */
class YoudsFrameworkWebRouting extends YoudsFrameworkRouting
{
	/**
	...
	public function initialize(YoudsFrameworkContext $context, array $parameters = array())
	{
		...
		if($this->isEnabled() && $rewritten) {
			...
			if(get_magic_quotes_gpc()) {
				$parsedRuQuery = YoudsFrameworkWebRequest::clearMagicQuotes($parsedRuQuery);
				$parsedInput = YoudsFrameworkWebRequest::clearMagicQuotes($parsedInput, false /* start on the first level */);
			}
			...
			$appendFrom = 0;
			$this->prefix = YoudsFrameworkToolkit::stringBase($sn, $path, $appendFrom);
		...
		if(
			...
			if($options['authority'] === null) {
				...
				if($options['port'] !== null && $options['port'] !== false) {
					if(YoudsFrameworkToolkit::isPortNecessary($options['scheme'] !== null && $options['scheme'] !== false ? $options['scheme'] : $req->getUrlScheme(), $options['port'])) {
						$port = $options['port'];
					} else {
						$port = null;
					}
					...
					if(!YoudsFrameworkToolkit::isPortNecessary($req->getUrlScheme(), $port = $req->getUrlPort())) {
						$port = null;
					}
				}
				if($port !== null) {
