...
xmlns:youds_annotations="http://youds.com/youds/config/global/annotations/1.0"
xmlns:youds_envelope="http://youds.com/youds/config/global/envelope/1.1"
xmlns:youds_routing="http://youds.com/youds/config/parts/routing/1.1"
...
>
	...
	<xsl:variable name="targetNamespace">
		<xsl:choose>
			...
			<xsl:when test="/youds_envelope:configurations/youds_envelope:configuration[@youds_annotations:matched and @targetNamespace]/@targetNamespace">
				<!-- take that one -->
				<xsl:value-of select="/youds_envelope:configurations/youds_envelope:configuration[@youds_annotations:matched and @targetNamespace][1]/@targetNamespace" />
			</xsl:when>
			<xsl:otherwise>
				<!-- there is none, take the one from the root element -->
				<xsl:value-of select="/youds_envelope:configurations/@targetNamespace" />
			</xsl:otherwise>
		</xsl:choose>
	</xsl:variable>
	<xsl:variable name="tns">
		<xsl:choose>
			...
			<xsl:when test="/youds_envelope:configurations/youds_envelope:configuration[@youds_annotations:matched and @targetNamespace and namespace::*[.=../@targetNamespace]]">
				<xsl:value-of select="name(/youds_envelope:configurations/youds_envelope:configuration[@youds_annotations:matched and @targetNamespace and namespace::*[.=../@targetNamespace]][1]/namespace::*[.=../@targetNamespace])" />
			</xsl:when>
			<!-- otherwise, take the namespace prefix from <configurations>, assuming that the targetNamespace is defined there, too -->
			<xsl:otherwise>
				<xsl:value-of select="name(/youds_envelope:configurations/namespace::*[.=../@targetNamespace])" />
			</xsl:otherwise>
		</xsl:choose>
	</xsl:variable>
	...
	<xsl:template match="/youds_envelope:configurations">
		<wsdl:definitions name="Dummy">
			<!-- copy namespace nodes -->
			<xsl:copy-of select="namespace::*"/>
			<!-- copy per-context namespace nodes -->
			<xsl:copy-of select="youds_envelope:configuration[@youds_annotations:matched]/namespace::*"/>
			...
			<wsdl:types>
				...
				<xsl:copy-of select="youds_envelope:configuration[@youds_annotations:matched]/wsdl:types/@*" />
				<xsl:copy-of select="wsdl:types/node()" />
				<xsl:copy-of select="youds_envelope:configuration[@youds_annotations:matched]/wsdl:types/node()" />
			</wsdl:types>
			<!-- copy message defs -->
			<xsl:copy-of select="wsdl:message" />
			<!-- copy per-context message defs -->
			<xsl:copy-of select="youds_envelope:configuration[@youds_annotations:matched]/wsdl:message" />
			<!-- all the rest -->
			<xsl:apply-templates select="youds_envelope:configuration[.//youds_routing:route//wsdl:* | .//youds_routing:route//soap:*]" />
		</wsdl:definitions>
	</xsl:template>
	<xsl:template match="youds_envelope:configuration[@youds_annotations:matched]">
		<wsdl:portType name="DummyPortType">
			<xsl:apply-templates select=".//youds_routing:route" mode="port" />
		</wsdl:portType>
		<binding name="DummyBinding" type="tns:DummyPortType">
			<soap:binding style="rpc" transport="http://schemas.xmlsoap.org/soap/http"/>
			<xsl:apply-templates select=".//youds_routing:route" mode="binding" />
		</binding>
		...
		<xsl:apply-templates select=".//youds_routing:route" mode="messages" />
	</xsl:template>
	<xsl:template match="youds_routing:route" mode="port">
	...
	<xsl:template match="youds_routing:route" mode="messages">
		<xsl:variable name="name" select="translate(@pattern, '^$', '')" />
	...
	<xsl:template match="youds_routing:route" mode="binding">
		<xsl:variable name="name" select="translate(@pattern, '^$', '')" />
		<wsdl:operation name="{$name}">
