 ...
 * @version    $Id$
 */
class YoudsFrameworkSqlsrvSessionStorage extends YoudsFrameworkSessionStorage
{
	/**
	 ...
	 *
	 * @throws     YoudsFrameworkInitializationException If an error occured during init.
	 *
	 * @author     David Zülke <david.zuelke@bitextender.com>
	 * @since      1.0.6
	 */
	public function initialize(YoudsFrameworkContext $context, array $parameters = array())
	{
		...
		if(!$this->hasParameter('db_table')) {
			...
			$error = 'No "db_table" configuration parameter given for YoudsFrameworkSqlsrvSessionStorage.';
			throw new YoudsFrameworkInitializationException($error);
		}
	 ...
	 *
	 * @throws     YoudsFrameworkStorageException If the session cannot be destroyed.
	 *
	 * @author     David Zülke <david.zuelke@bitextender.com>
	 * @since      1.0.6
	 */
	public function sessionDestroy($id)
	{
		...
		$error = "YoudsFrameworkSqlsrvSessionStorage cannot destroy session, error reported by server:\n\n%s";
		$error = sprintf($error, implode("\n", $this->getContext()->getDatabaseManager()->getDatabase($this->getParameter('database'))->getErrors()));
		throw new YoudsFrameworkStorageException($error);
	}
	 ...
	 *
	 * @throws     YoudsFrameworkStorageException If old sessions cannot be cleaned up.
	 *
	 * @author     David Zülke <david.zuelke@bitextender.com>
	 * @since      1.0.6
	 */
	public function sessionGC($lifetime)
	{
		...
		$error = "YoudsFrameworkSqlsrvSessionStorage cannot garbage collect old sessions, error reported by server:\n\n%s";
		$error = sprintf($error, implode("\n", $this->getContext()->getDatabaseManager()->getDatabase($this->getParameter('database'))->getErrors()));
		throw new YoudsFrameworkStorageException($error);
	}
	 ...
	 *
	 * @throws     YoudsFrameworkStorageException If the database connection failed.
	 *
	 * @author     David Zülke <david.zuelke@bitextender.com>
	 * @since      1.0.6
	 */
	public function sessionOpen($path, $name)
	{
		...
		if($database === null || !($database instanceof YoudsFrameworkSqlsrvDatabase)) {
			$error = 'Database connection "' . $database . '" could not be found or is not an YoudsFrameworkSqlsrvDatabase connection.';
			throw new YoudsFrameworkStorageException($error);
		}
	 ...
	 *
	 * @throws     YoudsFrameworkStorageException If the session cannot be read.
	 *
	 * @author     David Zülke <david.zuelke@bitextender.com>
	 * @since      1.0.6
	 */
	public function sessionRead($id)
	{
		...
		$error = "YoudsFrameworkSqlsrvSessionStorage cannot read session, error reported by server:\n\n%s";
		$error = sprintf($error, implode("\n", $this->getContext()->getDatabaseManager()->getDatabase($this->getParameter('database'))->getErrors()));
		throw new YoudsFrameworkStorageException($error);
	}
	 ...
	 *
	 * @throws     YoudsFrameworkStorageException If the session data cannot be written.
	 *
	 * @author     David Zülke <david.zuelke@bitextender.com>
	 * @since      1.0.6
	 */
	public function sessionWrite($id, &$data)
	{
		...
		if($result !== false && sqlsrv_rows_affected($result)) {
			...
			$error = "YoudsFrameworkSqlsrvSessionStorage cannot insert or update session, error reported by server:\n\n%s";
			$error = sprintf($error, implode("\n", $this->getContext()->getDatabaseManager()->getDatabase($this->getParameter('database'))->getErrors()));
			throw new YoudsFrameworkStorageException($error);
		}
	}
