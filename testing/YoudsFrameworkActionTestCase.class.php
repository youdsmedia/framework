...
/**
 * YoudsFrameworkActionTestCase is the base class for all action testcases and provides
 ...
 * @version    $Id$
 */
abstract class YoudsFrameworkActionTestCase extends YoudsFrameworkFragmentTestCase
{	
	/**
	 ...
	 * @return     void
	 * 
	 * @see        YoudsFrameworkAction::getDefaultViewName()
	 * 
	 * @author     Felix Gilcher <felix.gilcher@bitextender.com>
	 * @since      1.0.0 
	 */
	...
	{
		$actionInstance = $this->createActionInstance();
		$constraint = new YoudsFrameworkConstraintActionHandlesMethod($actionInstance, $acceptGeneric);
	...
	{
		$actionInstance = $this->createActionInstance();
		$constraint = self::logicalNot(new YoudsFrameworkConstraintActionHandlesMethod($actionInstance, $acceptGeneric));
	 ...
	 * @since      1.0.0
	 */
	protected function assertValidatedArgument($argumentName, $source = YoudsFrameworkRequestDataHolder::SOURCE_PARAMETERS, $message = 'Failed asserting that the argument <%1$s> is validated.')
	{
		$report = $this->container->getValidationManager()->getReport();
		$this->assertTrue($report->isArgumentValidated(new YoudsFrameworkValidationArgument($argumentName, $source)), sprintf($message, $argumentName));
	}
	 ...
	 * @since      1.0.0
	 */
	protected function assertFailedArgument($argumentName, $source = YoudsFrameworkRequestDataHolder::SOURCE_PARAMETERS, $message = 'Failed asserting that the argument <%1$s> is failed.')
	{
		$report = $this->container->getValidationManager()->getReport();
		$this->assertTrue($report->isArgumentFailed(new YoudsFrameworkValidationArgument($argumentName, $source)), sprintf($message, $argumentName));
	}
	 ...
	 * @since      1.0.0
	 */
	protected function assertSucceededArgument($argumentName, $source = YoudsFrameworkRequestDataHolder::SOURCE_PARAMETERS, $message = 'Failed asserting that the argument <%1$s> is succeeded.')
	{
		$report = $this->container->getValidationManager()->getReport();
		$success = $report->isArgumentValidated(new YoudsFrameworkValidationArgument($argumentName, $source)) && ! $report->isArgumentFailed(new YoudsFrameworkValidationArgument($argumentName, $source));
