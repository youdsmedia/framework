...
// | Copyright (c) 2005-2014 the YoudsFramework Project.                                |
// |                                                                           |
 ...
 * @since      1.1.0
 */
class YoudsFrameworkPhpUnitCli extends PHPUnit_TextUI_Command
{
	...
	public function __construct()
	{
		...
		$this->arguments['youdsEnvironment'] = !empty($_SERVER['AGAVI_ENVIRONMENT']) ? $_SERVER['AGAVI_ENVIRONMENT'] : 'testing';
		$this->arguments['youdsIncludeSuites'] = array();
		$this->arguments['youdsExcludeSuites'] = array();
		$this->arguments['youdsExpandConfiguration'] = true;
	}

	/**
	 * Callback handling the --environment command line option.
	 *
	 * @param      string The YoudsFramework environment name.
	 *
	 * @author     Dominik del Bondio <dominik.del.bondio@bitextender.com>
	 * @since      1.1.0
	 */
	...
	{
		$this->arguments['youdsEnvironment'] = $value;
	}
	...
	{
		$this->arguments['youdsIncludeSuites'] = array_merge(
			$this->arguments['youdsIncludeSuites'],
			explode(',', $value)
		);
	}
	...
	{
		$this->arguments['youdsExcludeSuites'] = array_merge(
			$this->arguments['youdsExcludeSuites'],
			explode(',', $value)
		);
	}
	...
	{
		$this->arguments['youdsExpandConfiguration'] = false;
	}
...
YoudsFramework specific arguments:
...
NOTE:
  ...
  paths in the configuration file. Use  %youds.app_dir%, %core.testing_dir% or
...
EOT;
	...
	{
		// ensure the bootstrap script doesn't run and bootstraps youds another time
		define('AGAVI_TESTING_BOOTSTRAPPED', true);
		YoudsFrameworkToolkit::clearCache();
		$this->bootstrap($this->arguments['youdsEnvironment']);
		
		
		// use the default configuration only if another configuration was not given as command line argument
		$defaultConfigPath = YoudsFrameworkConfig::get('core.testing_dir') . '/config/phpunit.xml';
		...
		$suites = require(YoudsFrameworkConfigCache::checkConfig(YoudsFrameworkConfig::get('core.testing_dir') . '/config/suites.xml'));
		
		$masterSuite = new YoudsFrameworkTestSuite('Master');
		
		if($this->arguments['youdsIncludeSuites']) {
			foreach($this->arguments['youdsIncludeSuites'] as $name) {
			...
			foreach($suites as $name => $suite) {
				if(!in_array($name, $this->arguments['youdsExcludeSuites'])) {
	 ...
	 *
	 * @return     YoudsFrameworkTestSuite The initialized test suite object.
	 *
	...
	protected static function createSuite($name, array $suite)
	{
		...
		if(!YoudsFrameworkToolkit::isPathAbsolute($base)) {
			$base = YoudsFrameworkConfig::get('core.testing_dir') . '/' . $base;
		}
		$s = new $suite['class']($name);
		if(!empty($suite['includes'])) {
			$files = iterator_to_array(new RecursiveIteratorIterator(
				new YoudsFrameworkRecursiveDirectoryFilterIterator(
		...
		foreach($suite['testfiles'] as $file) {
			if(!YoudsFrameworkToolkit::isPathAbsolute($file)) {
				$file = $base . '/' . $file;
			}
			$s->addTestFile($file);
		}
	...
	/**
	 * Runs YoudsFrameworkToolkit::expandDirectives() on all attributes and text nodes of
	 * the given file and writes a it to a new file in the YoudsFramework cache directory.
	 *
	...
	private static function expandConfiguration($file) {
		...
		foreach($attributeNodes as $attributeNode) {
			$attributeNode->value = YoudsFrameworkToolkit::expandDirectives($attributeNode->value);
		}
		$textNodes = $xpath->query('//text()');
		foreach($textNodes as $textNode) {
			$textNode->nodeValue = YoudsFrameworkToolkit::expandDirectives($textNode->nodeValue);
		}
		
		$translatedFile = YoudsFrameworkConfigCache::getCacheName($file);
		YoudsFrameworkConfigCache::writeCacheFile($file, $translatedFile, $doc->saveXML());
	...
	public static function bootstrap($environment = null)
	{
		if($environment === null) {
			...
			$environment = YoudsFrameworkConfig::get('testing.environment');
		} elseif(YoudsFrameworkConfig::has('testing.environment') && YoudsFrameworkConfig::isReadonly('testing.environment')) {
		...
		if($environment === null) {
			// still no env? oh man...
			throw new Exception('You must supply an environment name to YoudsFrameworkTesting::bootstrap() or set the name of the default environment to be used for testing in the configuration directive "testing.environment".');
		}
		
		// finally set the env to what we're really using now.
		YoudsFrameworkConfig::set('testing.environment', $environment, true, true);
		
		// bootstrap the framework for autoload, config handlers etc.
		YoudsFramework::bootstrap($environment);
		
		ini_set('include_path', get_include_path().PATH_SEPARATOR.dirname(__DIR__));
		
		$GLOBALS['AGAVI_CONFIG'] = YoudsFrameworkConfig::toArray();
	}
}
