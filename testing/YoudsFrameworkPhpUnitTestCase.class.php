...
/**
 * YoudsFrameworkPhpUnitTestCase is the base class for all YoudsFramework Testcases.
 * 
 * 
 * @package    youds
 * @subpackage testing
 ...
 * @version    $Id$
 */
abstract class YoudsFrameworkPhpUnitTestCase extends PHPUnit_Framework_TestCase
{
	/**
	...
	public function getIsolationEnvironment()
	{
		...
		if(!empty($annotations['method']['youdsIsolationEnvironment'])) {
			$environmentName = $annotations['method']['youdsIsolationEnvironment'][0];
		} elseif(!empty($annotations['class']['youdsIsolationEnvironment'])) {
			$environmentName = $annotations['class']['youdsIsolationEnvironment'][0];
	...
	public function getIsolationDefaultContext()
	{
		...
		if(!empty($annotations['method']['youdsIsolationDefaultContext'])) {
			$ctxName = $annotations['method']['youdsIsolationDefaultContext'][0];
		} elseif(!empty($annotations['class']['youdsIsolationDefaultContext'])) {
			$ctxName = $annotations['class']['youdsIsolationDefaultContext'][0];
	...
	public function getClearCache()
	{
		...
		if(!empty($annotations['method']['youdsClearIsolationCache'])) {
			$flag = true;
		} elseif(!empty($annotations['class']['youdsClearIsolationCache'])) {
			$flag = true;
		} else {
	...
	private function getDependendClasses() {
		...
		// has set @youdsBootstrap to off. That results in the YoudsFramework autoloader not
		// being started and if the test class depends on any files from YoudsFramework (like
		// YoudsFrameworkPhpUnitTestCase) it would not be loaded when the test is instantiated
		...
		$classesToFile = array('YoudsFrameworkTesting' => realpath(__DIR__ . '/YoudsFrameworkTesting.class.php'));
	...
	*/
	protected function prepareTemplate(Text_Template $template)
	{
		...
		// the youds test class files would be missing.
		// We can't write include()s directly since YoudsFramework possibly get's bootstrapped later
		...
		$globals = sprintf('
			...
			',
			var_export(YoudsFrameworkConfig::toArray(), true),
	...
	/**
	 * Whether or not an youds bootstrap should be done in isolation.
	 * 
	 * @return       boolean true if youds should be bootstrapped
	 * 
	 * @author       Felix Gilcher <felix.gilcher@bitextender.com>
	 *
	 * @since        1.0.2
	...
	protected function doBootstrap()
	{
		...
		$annotations = $this->getAnnotations();
		if(!empty($annotations['method']['youdsBootstrap'])) {
			$flag = YoudsFrameworkToolkit::literalize($annotations['method']['youdsBootstrap'][0]);
		} elseif(!empty($annotations['class']['youdsBootstrap'])) {
			$flag = YoudsFrameworkToolkit::literalize($annotations['class']['youdsBootstrap'][0]);
		}
		return $flag;
	}
