 ...
 * YoudsFramework testing environment
 * 
 * @package    youds
 * @subpackage testing
 *
 ...
 *
 * @since      1.0.0
 * @deprecated 1.1.0 Use YoudsFrameworkPhpUnitCli
 *
 * @version    $Id$
 */
class YoudsFrameworkTesting
{
	/**
	 ...
	 * this same instance will be passed to PHPUnit in YoudsFrameworkTesting::dispatch().
	 *
	...
	{
		YoudsFrameworkPhpUnitCli::bootstrap($environment);
	}
	...
	public static function dispatch($arguments = array(), $exit = true)
	{
		...
		$suites = include YoudsFrameworkConfigCache::checkConfig(YoudsFrameworkConfig::get('core.testing_dir').'/config/suites.xml');
		$master_suite = new YoudsFrameworkTestSuite('Master');
			...
			switch($option[0]) {
				...
				case '--log-graphviz':
					if(PHPUnit_Util_Filesystem::fileExistsInIncludePath('Image/GraphViz.php')) {
						...
						throw new YoudsFrameworkException('The Image_GraphViz package is not installed.');
					}
					break;
	 ...
	 * @throws     YoudsFrameworkExecption if a dependency is missing
	 * 
	 * @author     Felix Gilcher <felix.gilcher@bitextender.com>
	 * @since      1.0.0
	 * @deprecated 1.1.0 Use YoudsFrameworkPhpUnitCli
	...
	protected static function checkCodeCoverageDeps()
	{
		if(extension_loaded('tokenizer') && extension_loaded('xdebug')) {
			...
			if(!extension_loaded('tokenizer')) {
				throw new YoudsFrameworkException('The tokenizer extension is not loaded.');
			} else {
				throw new YoudsFrameworkException('The Xdebug extension is not loaded.');
			}
		}
