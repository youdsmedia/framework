...
/**
 * YoudsFrameworkGettextTranslator defines the translator interface for gettext.
 * 
 * @package    youds
 ...
 * @version    $Id$
 */
class YoudsFrameworkGettextTranslator extends YoudsFrameworkBasicTranslator
{
	/**
	...
	public function initialize(YoudsFrameworkContext $context, array $parameters = array())
	{
		...
		if(isset($parameters['store_calls'])) {
			...
			YoudsFrameworkToolkit::mkdir($parameters['store_calls'], 0777, true);
		}
	}
	...
	public function loadDomainData($domain)
	{
		...
		$localeNameBases = YoudsFrameworkLocale::getLookupPath($localeName);

		if(!isset($this->domainPaths[$domain])) {
			if(!$this->domainPathPattern) {
				throw new YoudsFrameworkException('Using domain "' . $domain . '" which has no path specified');
			} else {
		...
		$basePath = YoudsFrameworkToolkit::expandVariables($basePath, array('domain' => $domain));

		$data = array();

		foreach($localeNameBases as $localeNameBase) {
			$fileName = YoudsFrameworkToolkit::expandVariables($basePath, array('locale' => $localeNameBase));
			if($fileName === $basePath) {
				// no replacing of $locale happened
				$fileName = $basePath . '/' . $localeNameBase . '.mo';
			}
			if(is_readable($fileName)) {
				$fileData = YoudsFrameworkGettextMoReader::readFile($fileName);
		...
		if(isset($headers['Plural-Forms'])) {
			...
			if(preg_match('#nplurals=\d+;\s+plural=(.*)$#D', $pf, $match)) {
				...
				if(preg_match('#[^\d' . preg_quote(implode('', $validOpChars), '#') . ']#', $funcCode, $errorMatch)) {
					throw new YoudsFrameworkException('Illegal character ' . $errorMatch[0] . ' in plural form ' . $funcCode);
				}
