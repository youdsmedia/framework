 ...
 * @version    $Id$
 */
class YoudsFrameworkLocale extends YoudsFrameworkParameterHolder
{
	/**
	 ...
	 * 
	 * @param      YoudsFrameworkCalendar|DateTime|int The item to determine the timezone
	 *                                        from
	...
	{
		$tzId = '';
		if($item instanceof YoudsFrameworkCalendar) {
	 ...
	 * @since      0.11.0
	 */
	public function initialize(YoudsFrameworkContext $context, array $parameters = array(), $identifier, array $data = array())
	{
	...
	public static function parseLocaleIdentifier($identifier)
	{
		...
		if(preg_match($localeRx, $identifier, $match)) {
			...
			throw new YoudsFrameworkException('Invalid locale identifier (' . $identifier . ') specified');
		}
	 ...
	 *                   YoudsFrameworkLocale::parseLocaleIdentifier
	 *
