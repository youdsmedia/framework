 ...
 * @version    $Id$
 */
class YoudsFrameworkTranslationManager
{
	const MESSAGE = 'msg';
	const NUMBER = 'num';
	const CURRENCY = 'cur';
	...
	/**
	 * @var        YoudsFrameworkLocale The current locale.
	 */
	...
	public function initialize(YoudsFrameworkContext $context, array $parameters = array())
	{
		...
		include(YoudsFrameworkConfigCache::checkConfig(YoudsFrameworkConfig::get('core.config_dir') . '/translation.xml'));
		$this->loadSupplementalData();
		$this->loadTimeZoneData();
		$this->loadAvailableLocales();
		if($this->defaultLocaleIdentifier === null) {
			throw new YoudsFrameworkException('Tried to use the translation system without a default locale and without a locale set');
		}
	...
	public function setLocale($identifier)
	{
		...
		$givenData = YoudsFrameworkLocale::parseLocaleIdentifier($identifier);
		$actualData = YoudsFrameworkLocale::parseLocaleIdentifier($this->currentLocaleIdentifier);
	 ...
	 *
	 * @return     YoudsFrameworkLocale The current locale.
	 *
	 * @author     Dominik del Bondio <ddb@bitxtender.com>
	 * @since      0.11.0
	 */
	 ...
	 *
	 * @return     YoudsFrameworkLocale The current default.
	 *
	 * @author     David Zülke <dz@bitxtender.com>
	 * @since      0.11.0
	 */
	 ...
	 * @param      YoudsFrameworkLocale The locale which should be used for formatting.
	 ...
	 * @param      string The type of the translator.
	 *
	 * @return     YoudsFrameworkITranslator The translator instance.
	 *
	 * @author     Dominik del Bondio <ddb@bitxtender.com>
	 * @since      0.11.0
	 */
	...
	{
		$this->supplementalData = include(YoudsFrameworkConfigCache::checkConfig(YoudsFrameworkConfig::get('core.cldr_dir') . '/supplementalData.xml'));
	}
	...
	{
		$this->timeZoneList = include(YoudsFrameworkConfig::get('core.cldr_dir') . '/timezones/zonelist.php');
	}
	...
	public function getMatchingLocaleIdentifiers($identifier)
	{
		...
		$idData = YoudsFrameworkLocale::parseLocaleIdentifier($identifier);
	...
	public function getLocaleIdentifier($identifier)
	{
		...
		switch(count($matchingLocaleIdentifiers)) {
			case 1:
				...
				break;
			case 0:
				throw new YoudsFrameworkException('Specified locale identifier ' . $identifier . ' which has no matching available locale defined');
			default:
				throw new YoudsFrameworkException('Specified ambiguous locale identifier ' . $identifier . ' which has matches: ' . implode(', ', $matchingLocaleIdentifiers));
		}
	...
	/**
	 * Returns a new YoudsFrameworkLocale object from the given identifier.
	 *
	 * @param      string The locale identifier
	 * @param      bool   Force a new instance even if an identical one exists.
	 *
	 * @return     YoudsFrameworkLocale The locale instance which matches the available
	 *                         locales most.
	 *
	 * @author     Dominik del Bondio <ddb@bitxtender.com>
	 * @author     David Zülke <dz@bitxtender.com>
	...
	public function getLocale($identifier, $forceNew = false)
	{
		...
		if($identifier[0] == '@' && $this->currentLocaleIdentifier) {
			$idData = YoudsFrameworkLocale::parseLocaleIdentifier($this->currentLocaleIdentifier);
			$identifier = $idData['locale_str'] . $identifier;

			$newIdData = YoudsFrameworkLocale::parseLocaleIdentifier($identifier);
		...
		if(!isset($this->localeDataCache[$idData['locale_str']])) {
			$lookupPath = YoudsFrameworkLocale::getLookupPath($availableLocale['identifierData']);
			$cldrDir = YoudsFrameworkConfig::get('core.cldr_dir');
			$data = null;

			foreach($lookupPath as $localeName) {
				$fileName = $cldrDir . '/locales/' . $localeName . '.xml';
				if(is_readable($fileName)) {
					$data = include(YoudsFrameworkConfigCache::checkConfig($fileName));
					break;
				}
			}
			if($data === null) {
				throw new YoudsFrameworkException('No data available for locale ' . $identifier);
			}
		...
		$locale = new YoudsFrameworkLocale();
	 ...
	 *
	 * @return     YoudsFrameworkTimeZone The current timezone instance.
	 *
	 * @author     Dominik del Bondio <ddb@bitxtender.com>
	 * @since      0.11.0
	 *
	 * @deprecated Superseded by YoudsFrameworkTranslationManager::getDefaultTimeZone()
	 */
	 ...
	 *
	 * @return     YoudsFrameworkTimeZone The default timezone instance.
	 *
	 ...
	 * hdd again.
	 *
	 * @return     YoudsFrameworkTimeZone The timezone instance for the given id.
	 *
	 * @author     Dominik del Bondio <ddb@bitxtender.com>
	 * @since      0.11.0
	 */
	public function createTimeZone($id, $cache = true)
	{
		if(!isset($this->timeZoneList[$id])) {
			try {
				return YoudsFrameworkTimeZone::createCustomTimeZone($this, $id);
			} catch(Exception $e) {
				return null;
			}
		}
		...
		if(!isset($this->timeZoneCache[$id]) || !$cache) {
			...
			$zoneData = include(YoudsFrameworkConfig::get('core.cldr_dir') . '/timezones/' . $this->timeZoneList[$currId]['filename']);

			$zone = new YoudsFrameworkOlsonTimeZone($this, $id, $zoneData);
	 ...
	 *
	 * @param      mixed This can be either an YoudsFrameworkLocale, an YoudsFrameworkTimeZone or
	 *                   a string specifying the calendar type.
	 *
	 * @return     YoudsFrameworkCalendar The current timezone instance.
	 *
	 * @author     Dominik del Bondio <ddb@bitxtender.com>
	 * @since      0.11.0
	 */
	public function createCalendar($type = null)
	{
		...
		$zone = null;
		$time = null;
		if($type instanceof YoudsFrameworkLocale) {
			$locale = $type;
		} elseif($type instanceof YoudsFrameworkTimeZone) {
			$zone = $type;
		} elseif($type instanceof DateTime) {
			$time = $type;
		} elseif(is_int($type)) {
			$time = $type * YoudsFrameworkDateDefinitions::MILLIS_PER_SECOND;
		} elseif($type !== null) {
			$calendarType = $type;
		}
		if($time === null) {
			$time = YoudsFrameworkCalendar::getNow();
		}
		...
		switch($calendarType) {
			case YoudsFrameworkCalendar::GREGORIAN:
				$c = new YoudsFrameworkGregorianCalendar($this /* $locale */);
				break;
			default:
				throw new YoudsFrameworkException('Calendar type ' . $calendarType . ' not supported');
		}
		...
		if($time instanceof DateTime) {
			...
			$c->set(YoudsFrameworkDateDefinitions::YEAR, $year);
			$c->set(YoudsFrameworkDateDefinitions::DAY_OF_YEAR, $doy + 1);
			$c->set(YoudsFrameworkDateDefinitions::HOUR_OF_DAY, $hour);
			$c->set(YoudsFrameworkDateDefinitions::MINUTE, $minute);
			$c->set(YoudsFrameworkDateDefinitions::SECOND, $second);
	 ...
	 *
	 * @return     YoudsFrameworkDateFormat The dateformat instance.
	 *
	 * @author     Dominik del Bondio <dominik.del.bondio@bitextender.com>
	 * @since      0.11.0
	 */
	...
	{
		$dateFormat = new YoudsFrameworkDateFormat($format);
