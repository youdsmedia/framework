...
/**
 * YoudsFrameworkRbacUser will handle roles and permissions for users
 *
 * @package    youds
 * @subpackage user
 *
 ...
 * @version    $Id$
 */
class YoudsFrameworkRbacSecurityUser extends YoudsFrameworkSecurityUser implements YoudsFrameworkISecurityUser
{
	/**
	 * The namespace under which roles will be stored.
	 */
	const ROLES_NAMESPACE = 'org.youds.user.RbacSecurityUser.roles';
	...
	{
		$cfg = $this->getParameter('definitions_file', YoudsFrameworkConfig::get('core.config_dir') . '/rbac_definitions.xml');
		
		if(is_readable($cfg)) {
			$this->definitions = include(YoudsFrameworkConfigCache::checkConfig($cfg, $this->context->getName()));
		}
	}
