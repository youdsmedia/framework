 ...
 * @version    $Id$
 */
final class YoudsFrameworkArrayPathDefinition
{
	/**
	 * constructor
	 * 
		...
		foreach($parts as $part) {
			if($part !== null) {
				...
				if(is_array($a) && (isset($a[$part]) || array_key_exists($part, $a))) {
					...
					//throw new YoudsFrameworkException('The part: ' . $part . ' does not exist in the given array');
					return $default;
				}
			}
		}
	...
	public static function getFlatKeyNames(array $array, $prefix = null)
	{
		...
		foreach($array as $key => $value) {
			...
			if(is_array($value)) {
				$names = array_merge($names, YoudsFrameworkArrayPathDefinition::getFlatKeyNames($value, $name));
			} else {
				$names[] = $name;
			}
		}
	...
	public static function flatten($array, $prefix = null)
	{
		...
		foreach($array as $key => $value) {
			...
			if(is_array($value)) {
				$flatArray += YoudsFrameworkArrayPathDefinition::flatten($value, $name);
			} else {
