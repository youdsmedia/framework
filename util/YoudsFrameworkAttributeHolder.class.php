...
/**
 * YoudsFrameworkAttributeHolder provides a base class for managing attributes with
 * namespaces. It contains all the functionality YoudsFrameworkParameterHolder provides.
 *
 * @package    youds
 * @subpackage util
 *
 ...
 * @version    $Id$
 */
abstract class YoudsFrameworkAttributeHolder extends YoudsFrameworkParameterHolder
{
	/**
	 ...
	 */
	protected $defaultNamespace = 'org.youds';
	...
	public function &getAttribute($name, $ns = null, $default = null)
	{
		...
		if(isset($this->attributes[$ns])) {
			...
			try {
				return YoudsFrameworkArrayPathDefinition::getValue($name, $this->attributes[$ns], $default);
	...
	public function getFlatAttributeNames($ns = null)
	{
		...
		if(isset($this->attributes[$ns])) {
			return YoudsFrameworkArrayPathDefinition::getFlatKeyNames($this->attributes[$ns]);
		}
	}
	...
	public function hasAttribute($name, $ns = null)
	{
		...
		if(isset($this->attributes[$ns])) {
			...
			try {
				return YoudsFrameworkArrayPathDefinition::hasValue($name, $this->attributes[$ns]);
			} catch(InvalidArgumentException $e) {
				return false;
			}
		}
	...
	public function &removeAttribute($name, $ns = null)
	{
		...
		if(isset($this->attributes[$ns])) {
			if(isset($this->attributes[$ns][$name]) || array_key_exists($name, $this->attributes[$ns])) {
				...
				try {
					$retval =& YoudsFrameworkArrayPathDefinition::unsetValue($name, $this->attributes[$ns]);
				} catch(InvalidArgumentException $e) {
				}
			}
		}
