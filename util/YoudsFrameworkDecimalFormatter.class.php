 ...
 * @version    $Id$
 */
class YoudsFrameworkDecimalFormatter
{
	/**
	 ...
	 */
	protected $roundingMode = YoudsFrameworkDecimalFormatter::ROUND_SCIENTIFIC;
...
//						$formatStr .= '%2$s';
				...
				case self::IN_NUMBER: {
					if(!in_array($c, $numberChars)) {
						...
						if($numberState == 'inInteger') {
							...
							if($c == ',' || $c == '.') {
								throw new YoudsFrameworkException($c. ' is not allowed in the fraction part of the number');
							} else {
								if($c == '#') {
									++$skippedFractionals;
								} elseif($c == '0') {
	...
	protected static function getDecimalParseRegex(YoudsFrameworkLocale $locale = null)
	{
	 ...
	 * @param      YoudsFrameworkLocale An optional locale to get the separators from.
