...
/**
 * YoudsFrameworkToolkit provides basic utility methods.
 *
 * @package    youds
 * @subpackage util
 *
 ...
 * @version    $Id$
 */
final class YoudsFrameworkToolkit
{
	/**
	...
	{
		if(!YoudsFrameworkConfig::get('core.cache_dir')) {
			throw new YoudsFrameworkException('Holy disk wipe, Batman! It seems that the value of "core.cache_dir" is empty, and because YoudsFramework considers you its most dearest of friends, it chose not to erase your entire file system. Skynet or other evil machines may not be so forgiving however, so please fix whatever code you wrote that caused this :)');
		}
		...
		$path = realpath(YoudsFrameworkConfig::get('core.cache_dir') . DIRECTORY_SEPARATOR . $path);
		if($path === false) {
			return false;
		}
		if(is_file($path)) {
	...
	public static function overloadHelper(array $definitions, array $parameters)
	{
		...
		if(!isset($countedDefs[$paramCount])) {
			throw new YoudsFrameworkException('overloadhelper couldn\'t find a matching method with the parameter count ' . $paramCount);
		}
		if(count($countedDefs[$paramCount]) > 1) {
			...
			if($matchCount == 0) {
				throw new YoudsFrameworkException('overloadhelper couldn\'t find a matching method');
			} elseif($matchCount > 1) {
				throw new YoudsFrameworkException('overloadhelper found ' . $matchCount . ' matching methods');
			}
	...
	public static function expandDirectives($value)
	{
		do {
			...
			$value = preg_replace_callback(
				'/\%([\w\.]+?)\%/',
				function($matches) { return YoudsFrameworkConfig::get($matches[1], '%' . $matches[1] . '%'); },
				$value
			);
	...
	public static function floorDivide($numerator, $denominator, &$remainder)
	{
		if((int)$denominator != $denominator) {
			throw new YoudsFrameworkException('YoudsFrameworkToolkit::floorDivive works only for int denominators');
		}
	...
	/**
	 * Evaluates a given YoudsFrameworkConfig per-module directive using the given info.
	 *
	...
	{
		return YoudsFrameworkToolkit::expandVariables(
			YoudsFrameworkToolkit::expandDirectives(
				YoudsFrameworkConfig::get(
					sprintf(
						'modules.%s.%s',
