...
/**
 * YoudsFrameworkBaseFileValidator is the base validator when validating files. 
 ...
 * @version    $Id$
 */
abstract class YoudsFrameworkBaseFileValidator extends YoudsFrameworkValidator
{
	/**
	 * @see        YoudsFrameworkValidator::initialize
	 *
	 * @author     Dominik del Bondio <ddb@bitxtender.com>
	 * @since      0.11.0
	 */
	public function initialize(YoudsFrameworkContext $context, array $parameters = array(), array $arguments = array(), array $errors = array())
	{
		if(!isset($parameters['source'])) {
			$parameters['source'] = YoudsFrameworkWebRequestDataHolder::SOURCE_FILES;
		}

		parent::initialize($context, $parameters, $arguments, $errors);
		
		if($this->hasParameter('mime_type') && !extension_loaded('fileinfo')) {
			throw new YoudsFrameworkValidatorException('MIME type checks in file validators require the "fileinfo" PHP extension to be loaded.');
		}
	}
