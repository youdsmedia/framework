...
/**
 * YoudsFrameworkDateTimeValidator verifies that a parameter is of a date and or time 
 * format.
 * 
 * Arguments: 
 *   This can be:
 ...
 *      (YoudsFrameworkDateDefinitions::MONTH, etc) as key and the argument field as 
 *      value.
 ...
 *                 'calendar' (will return the YoudsFrameworkCalendar object),
 ...
 * @version    $Id$
 */
class YoudsFrameworkDateTimeValidator extends YoudsFrameworkValidator
{
	/**
	 * Validates the input.
	 * 
	...
	protected function validate()
	{
		if(!YoudsFrameworkConfig::get('core.use_translation')) {
			throw new YoudsFrameworkConfigurationException('The datetime validator can only be used with use_translation on');
		}
		...
		if($this->hasMultipleArguments() && !$this->getParameter('arguments_format')) {
			...
			foreach($this->getArguments() as $calField => $field) {
				...
				if(defined($calField)) {
					...
					throw new YoudsFrameworkValidatorException('Unknown argument name "' . $calField . '" for argument "' . $field . '" supplied. This needs to be one of the constants defined in YoudsFrameworkDateDefinitions.');
				}
				if(!is_scalar($param)) {
				...
				if($calField == YoudsFrameworkDateDefinitions::MONTH) {
					$param -= 1;
				}
			...
			foreach((array)$this->getParameter('formats', array()) as $key => $item) {
				...
				if($type == 'format') {
					...
					$formatString = YoudsFrameworkDateFormatter::resolveFormat($format, $itemLocale, $type);
				...
				if(!$cal) {
					try {
						$format = new YoudsFrameworkDateFormat($formatString);
		...
		if($cast = $this->getParameter('cast_to')) {
			...
			if(is_array($cast)) {
				...
				if($type == 'format') {
					...
					$formatString = YoudsFrameworkDateFormatter::resolveFormat($format, $locale, $type);
				}
		...
		$defaultParseFormat = new YoudsFrameworkDateFormat('yyyy-MM-dd HH:mm:ss.S');
	 ...
	 * @param      YoudsFrameworkDateFormat The default format when parsing strings.
	 * @param      YoudsFrameworkLocale The locale to use.
	 *
	 * @return     YoudsFrameworkCalendar The calendar object storing the date.
	 * 
	 * @author     Dominik del Bondio <ddb@bitxtender.com>
	 * @since      0.11.0
	 */
	protected function getMinOrMaxValue($minMax, $defaultParseFormat, $locale)
	{
		...
		if(is_array($minMax)) {
			...
			if(!$minMaxValue instanceof YoudsFrameworkCalendar) {
				if(isset($minMax['format'])) {
					$format = new YoudsFrameworkDateFormat($minMax['format']);
				}
