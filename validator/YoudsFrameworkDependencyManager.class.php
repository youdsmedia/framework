...
/**
 * YoudsFrameworkDependencyManager handles the dependencies in the validation process
 *
 * @package    youds
 ...
 * @version    $Id$
 */
class YoudsFrameworkDependencyManager
{
	/**
	 ...
	 * @param      YoudsFrameworkVirtualArrayPath The base path to which all tokens are 
	 *                                   appended.
	 * 
	 ...
	 * @since      0.11.0
	 */
	public function checkDependencies(array $tokens, YoudsFrameworkVirtualArrayPath $base)
	{
		...
		foreach($tokens as $token) {
			...
			$path = new YoudsFrameworkVirtualArrayPath($token);
	 ...
	 * @since      0.11.0
	 */
	public function addDependTokens(array $tokens, YoudsFrameworkVirtualArrayPath $base)
	{
		$currentParts = $base->getParts();
		foreach($tokens as $token) {
			if($currentParts && strpos($token, '%') !== false) { 
