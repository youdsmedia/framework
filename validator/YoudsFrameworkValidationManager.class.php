...
/**
 * YoudsFrameworkValidationManager provides management for request parameters and their
 ...
 * @version    $Id$
 */
class YoudsFrameworkValidationManager extends YoudsFrameworkParameterHolder implements YoudsFrameworkIValidatorContainer
{
	/**
	 * @var        YoudsFrameworkDependencyManager The dependency manager.
	 */
	...
	/**
	 * @var        YoudsFrameworkValidationReport The report container storing the validation results.
	 */
	 ...
	 *
	 * @param      YoudsFrameworkContext The context instance.
	...
	public function initialize(YoudsFrameworkContext $context, array $parameters = array())
	{
		if(isset($parameters['mode'])) {
			if(!in_array($parameters['mode'], array(self::MODE_RELAXED, self::MODE_CONDITIONAL, self::MODE_STRICT))) {
				throw new YoudsFrameworkConfigurationException('Invalid validation mode "' . $parameters['mode'] . '" specified');
			}
		} else {
		...
		$this->dependencyManager = new YoudsFrameworkDependencyManager();
		$this->report = new YoudsFrameworkValidationReport();
		$this->children = array();
	}
	 ...
	 *
	 * @return     YoudsFrameworkValidationReport The result report container.
	 *
	 ...
	 * @param      YoudsFrameworkIValidatorContainer The parent (will use the validation 
	 *                                      manager if null is given)
	 * @return     YoudsFrameworkValidator
	 * 
	 * @author     Dominik del Bondio <ddb@bitxtender.com>
	 * @since      0.11.0
	 */
	public function createValidator($class, array $arguments, array $errors = array(), $parameters = array(), YoudsFrameworkIValidatorContainer $parent = null)
	{
		if($parent === null) {
			$parent = $this;
		}
	 ...
	 *
	 * @return     YoudsFrameworkDependencyManager The dependency manager instance.
	 *
	 * @author     Uwe Mesecke <uwe@mesecke.net>
	 * @since      0.11.0
	 */
	 ...
	 *
	 * @return     YoudsFrameworkVirtualArrayPath The base path.
	 *
	 * @author     Uwe Mesecke <uwe@mesecke.net>
	 * @since      0.11.0
	 */
	...
	{
		return new YoudsFrameworkVirtualArrayPath($this->getParameter('base', ''));
	}
	...
	public function execute(YoudsFrameworkRequestDataHolder $parameters)
	{
		$success = true;
		$this->report = new YoudsFrameworkValidationReport();
		$result = YoudsFrameworkValidator::SUCCESS;
		...
		foreach($this->children as $validator) {
			...
			switch($validatorResult) {
				case YoudsFrameworkValidator::SUCCESS:
					continue 2;
				case YoudsFrameworkValidator::INFO:
					continue 2;
				case YoudsFrameworkValidator::SILENT:
					continue 2;
				case YoudsFrameworkValidator::NOTICE:
					continue 2;
				case YoudsFrameworkValidator::ERROR:
					$success = false;
					continue 2;
				case YoudsFrameworkValidator::CRITICAL:
					$success = false;
					break 2;
			}
		}
		...
		if($mode == self::MODE_STRICT || ($executedValidators > 0 && $mode == self::MODE_CONDITIONAL)) {
			...
			foreach($parameters->getSourceNames() as $source) {
				...
				foreach(YoudsFrameworkArrayPathDefinition::getFlatKeyNames($sourceItems) as $name) {
					if(!isset($succeededArguments[$source . '/' . $name]) && (!$umap || ($source != YoudsFrameworkRequestDataHolder::SOURCE_PARAMETERS || ($name != $ma && $name != $aa)))) {
	 ...
	 * add errors either via YoudsFrameworkRequest::addError or by directly using this 
	 * method)
	 *
	...
	public function getResult()
	{
		...
		if(null === $result) {
			$result = YoudsFrameworkValidator::NOT_PROCESSED;
		}
	...
	{
		return $this->report->getAuthoritativeArgumentSeverity(new YoudsFrameworkValidationArgument($fieldname), $validatorName);
	}
	...
	{
		return $this->report->isArgumentFailed(new YoudsFrameworkValidationArgument($fieldname));
	}
	...
	{
		return $this->report->isArgumentValidated(new YoudsFrameworkValidationArgument($fieldname));
	}
	...
	{
		$incidents = $this->getFieldIncidents($name, YoudsFrameworkValidator::NOTICE);
	...
	public function getErrors($name = null)
	{
		...
		foreach($this->getIncidents(YoudsFrameworkValidator::NOTICE) as $incident) {
			$validator = $incident->getValidator();
	...
	public function getErrorMessages($name = null)
	{
		...
		if($name !== null) {
			...
			$incidents = $this->getIncidents(YoudsFrameworkValidator::NOTICE);
			$msgs = array();
			foreach($incidents as $incident) {
				foreach($incident->getErrors() as $error) {
	...
	public function hasError($name)
	{
		...
		return ($ec >= YoudsFrameworkValidator::NOTICE);
	}
	...
	public function hasErrors()
	{
		...
		return $this->getResult() > YoudsFrameworkValidator::NOTICE;
	}
	...
	{
		$name = new YoudsFrameworkValidationArgument($name);
		$incident = new YoudsFrameworkValidationIncident(null, YoudsFrameworkValidator::ERROR);
		$incident->addError(new YoudsFrameworkValidationError($message, null, array($name)));
	...
	public function setErrors(array $errors)
	{
		...
		foreach($errors as $name => $error) {
			$name = new YoudsFrameworkValidationArgument($name);
			$incident->addError(new YoudsFrameworkValidationError($error, null, array($name)));
		}
