...
/**
 * YoudsFrameworkValidationReport stores the result of a validation run.
 *
 * @package    youds
 ...
 * @version    $Id$
 */
class YoudsFrameworkValidationReport implements YoudsFrameworkIValidationReportQuery
{
	/**
	 ...
	 *                 you are comparing against YoudsFrameworkValidator::SUCCESS.
	 * 
	 ...
	 *
	 * @param      array The depend tokens of the YoudsFrameworkDependencyManager.
	 *
	 * @author     Steffen Gransow <youds@mivesto.de>
	 * @since      1.1.0
	 */
	 ...
	 * @param      YoudsFrameworkValidator The validator (if the error was cause inside 
	 *                            a validator).
	 *
	 * @author     Dominik del Bondio <dominik.del.bondio@bitextender.com>
	 * @since      1.0.0
	 */
	public function addArgumentResult(YoudsFrameworkValidationArgument $argument, $result, YoudsFrameworkValidator $validator = null)
	{
	 ...
	 * @since      1.0.0
	 */
	public function getAuthoritativeArgumentSeverity(YoudsFrameworkValidationArgument $argument, $validatorName = null)
	{
		...
		foreach($this->argumentResults[$argument->getHash()] as $result) {
			if($validatorName === null || ($result['validator'] instanceof YoudsFrameworkValidator && $result['validator']->getName() == $validatorName)) {
				if(null === $severity) {
					$severity = YoudsFrameworkValidator::NOT_PROCESSED;
				}
	 ...
	 * @since      1.0.0
	 */
	public function isArgumentValidated(YoudsFrameworkValidationArgument $argument)
	{
	 ...
	 * @since      1.0.0
	 */
	public function isArgumentFailed(YoudsFrameworkValidationArgument $argument)
	{
		$severity = $this->getAuthoritativeArgumentSeverity($argument);
		return ($severity > YoudsFrameworkValidator::SUCCESS);
	}
	...
	public function getSucceededArguments($source = null)
	{
		...
		foreach($this->argumentResults as $results) {
			...
			if($hasInSource && $severity >= YoudsFrameworkValidator::SUCCESS && $severity <= YoudsFrameworkValidator::INFO) {
	...
	public function getFailedArguments($source = null)
	{
		...
		if($this->getResult() <= YoudsFrameworkValidator::INFO) {
			return array();
		}
		...
		foreach($this->argumentResults as $results) {
			...
			if($hasInSource && $severity > YoudsFrameworkValidator::INFO) {
				$argument = $results[0]['argument'];
				$arguments[$argument->getHash()] = $argument;
			}
		}
	...
	/**
	 * Create a new YoudsFrameworkValidationReportQuery for this report.
	 *
	...
	{
		return new YoudsFrameworkValidationReportQuery($this);
	}
	...
	/**
	 * Retrieves all YoudsFrameworkValidationError objects in this report.
	 * 
	...
	/**
	 * Retrieves all YoudsFrameworkValidationArgument objects in this report.
	 * 
	 * @return     array An array of YoudsFrameworkValidationArgument objects.
	 * 
	 * @author     David Zülke <david.zuelke@bitextender.com>
