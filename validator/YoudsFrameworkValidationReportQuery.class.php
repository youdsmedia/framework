...
/**
 * YoudsFrameworkValidationReportQuery allows queries against the validation run report.
 *
 * @package    youds
 ...
 * @version    $Id$
 */
class YoudsFrameworkValidationReportQuery implements YoudsFrameworkIValidationReportQuery
{
	/**
	 * @var        YoudsFrameworkValidationReport
	 */
	protected $report;
	 ...
	 * Constructor.
	 * 
	 * @param      YoudsFrameworkValidationReport The validation report instance.
	 * 
	 * @author     Dominik del Bondio <dominik.del.bondio@bitextender.com>
	 * @since      1.0.0
	 */
	public function __construct(YoudsFrameworkValidationReport $report)
	{
		$this->report = $report;
	}
	...
	public function byArgument($argument)
	{
		if(is_array($argument)) {
			foreach($argument as &$arg) {
				if(!($arg instanceof YoudsFrameworkValidationArgument)) {
					$arg = new YoudsFrameworkValidationArgument($arg);
				}
			}
		} else {
