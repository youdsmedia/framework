...
/**
 * YoudsFrameworkValidator allows you to validate input
 *
 ...
 * @version    $Id$
 */
abstract class YoudsFrameworkValidator extends YoudsFrameworkParameterHolder
{
	/**
	 ...
	 */
	const SILENT = 200;
	const NONE = YoudsFrameworkValidator::SILENT;
	...
	/**
	 * @var        YoudsFrameworkIValidatorContainer parent validator container (in
	 *                                      most cases the validator manager)
	 */
	protected $parentContainer = null;

	/**
	 * @var        YoudsFrameworkVirtualArrayPath The current base for input names, 
	 *                                   dependencies etc.
	 */
	...
	/**
	 * @var        YoudsFrameworkRequestDataHolder The parameters which should be validated
	...
	/**
	 * @var        YoudsFrameworkValidationIncident The current incident.
	 */
	 ...
	 *
	 * @return     YoudsFrameworkVirtualArrayPath The basepath of this validator
	 *
	 * @author     Dominik del Bondio <ddb@bitxtender.com>
	 * @since      0.11.0
	 */
	 ...
	 * @since      0.11.0
	 */
	public function initialize(YoudsFrameworkContext $context, array $parameters = array(), array $arguments = array(), array $errors = array())
	{
		...
		if(!isset($parameters['source'])) {
			$parameters['source'] = YoudsFrameworkRequestDataHolder::SOURCE_PARAMETERS;
		}

		$this->setParameters($parameters);

		$this->name = $this->getParameter('name', YoudsFrameworkToolkit::uniqid());
	}
	 ...
	 *
	 * @return     YoudsFrameworkIValidatorContainer The parent container.
	 *
	 * @author     Dominik del Bondio <ddb@bitxtender.com>
	 * @since      0.11.0
	 */
	 ...
	 *
	 * @param      YoudsFrameworkIValidatorContainer The parent container.
	 *
	 * @author     Dominik del Bondio <ddb@bitxtender.com>
	 * @since      0.11.0
	 */
	public function setParentContainer(YoudsFrameworkIValidatorContainer $parent)
	{
	 ...
	 *
	 * @see        YoudsFrameworkValidationManager::shutdown()
	 *
	 * @author     Uwe Mesecke <uwe@mesecke.net>
	 * @since      0.11.0
	 */
	...
	protected function throwError($index = null, $affectedArgument = null, $argumentsRelative = false, $setAffected = false)
	{
		...
		if(!$this->incident) {
			$this->incident = new YoudsFrameworkValidationIncident($this, self::mapErrorCode($this->getParameter('severity', 'error')));
		}

		foreach($affectedArguments as &$argument) {
			$argument = new YoudsFrameworkValidationArgument($argument, $this->getParameter('source'));
		}
		
		if($error !== null || count($affectedArguments) != 0) {
			// don't throw empty error messages without affected fields
			$this->incident->addError(new YoudsFrameworkValidationError($error, $index, $affectedArguments));
		}
	}
	 ...
	 *                   YoudsFrameworkValidationArgument object if the value should be
	 *                   exported to a different source.
	 * @param      int   The result status code to use for the exported value.
	 *                   Defaults to YoudsFrameworkValidator::SUCCESS.
	 *
	 * @author     Dominik del Bondio <ddb@bitxtender.com>
	 * @author     David Zülke <david.zuelke@bitextender.com>
	 * @since      0.11.0
	...
	protected function export($value, $argument = null, $result = null)
	{
		...
		if($result === null) {
			$result = $this->getParameter('export_severity', YoudsFrameworkValidator::SUCCESS);
			if(!is_numeric($result) && defined($result)) {
				$result = constant($result);
			}
		}

		if(!($argument instanceof YoudsFrameworkValidationArgument) && (!is_string($argument) || $argument === '')) {
			return;
		}

		if($argument instanceof YoudsFrameworkValidationArgument) {
		...
		$cp = new YoudsFrameworkVirtualArrayPath($name);
		...
		if($this->parentContainer !== null) {
			...
			if(is_array($value)) {
				...
				foreach(YoudsFrameworkArrayPathDefinition::getFlatKeyNames($value) as $keyName) {
					$this->parentContainer->addArgumentResult(new YoudsFrameworkValidationArgument($cp->pushRetNew($keyName)->__toString(), $source), $result, $this);
				}
			}
			$this->parentContainer->addArgumentResult(new YoudsFrameworkValidationArgument($cp->__toString(), $source), $result, $this);
		}
	}
	 ...
	 *
	 * @param      YoudsFrameworkVirtualArrayPath The base in which the input should be 
	 *                                   validated.
	 *
	 * @return     int YoudsFrameworkValidator::SUCCESS if validation succeeded or given
	 *                 error severity.
	 *
	 * @author     Dominik del Bondio <ddb@bitxtender.com>
	 * @author     Uwe Mesecke <uwe@mesecke.net>
	 * @since      0.11.0
	 */
	protected function validateInBase(YoudsFrameworkVirtualArrayPath $base)
	{
		$base = clone $base;
		if($base->length() == 0) {
			...
			if($this->parentContainer !== null) {
				foreach($this->affectedArguments as $fieldname) {
					$this->parentContainer->addArgumentResult(new YoudsFrameworkValidationArgument($fieldname, $this->getParameter('source')), $result, $this);
				}
	 ...
	 *
	 * @param      YoudsFrameworkRequestDataHolder The data which should be validated.
	 *
	...
	{
		if($this->getParameter('source') != YoudsFrameworkRequestDataHolder::SOURCE_PARAMETERS && !in_array($this->getParameter('source'), $parameters->getSourceNames())) {
			throw new YoudsFrameworkConfigurationException('Unknown source "' . $this->getParameter('source') . '" specified in validator ' . $this->getName());
		}

		$this->validationParameters = $parameters;
		$base = new YoudsFrameworkVirtualArrayPath($this->getParameter('base'));
	 ...
	 *
	 * critical -> YoudsFrameworkValidator::CRITICAL
	 * error    -> YoudsFrameworkValidator::ERROR
	 * notice   -> YoudsFrameworkValidator::NOTICE
	 * none     -> YoudsFrameworkValidator::NONE
	 ...
	 *
	 * @throws     <b>YoudsFrameworkValidatorException</b> if the input was no known 
	 *                                           severity
	 *
	 * @author     Uwe Mesecke <uwe@mesecke.net>
	 * @since      0.11.0
	...
	public static function mapErrorCode($code)
	{
		switch(strtolower($code)) {
			...
			case 'info':
				return self::INFO;
			default:
				throw new YoudsFrameworkValidatorException('unknown error code: '.$code);
		}
	}
	...
	{
		if($this->parentContainer instanceof YoudsFrameworkIValidatorContainer) {
