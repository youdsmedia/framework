 ...
 * @version    $Id$
 */
class YoudsFrameworkStreamTemplateLayer extends YoudsFrameworkTemplateLayer
{
	/**
	 * Constructor.
	 *
	...
	public function getResourceStreamIdentifier()
	{
		...
		if(YoudsFrameworkConfig::get('core.use_translation')) {
			...
			foreach(YoudsFrameworkLocale::getLookupPath($this->context->getTranslationManager()->getCurrentLocaleIdentifier()) as $identifier) {
		...
		if($scheme != 'file' && !in_array($scheme, stream_get_wrappers())) {
			throw new YoudsFrameworkException('Unknown stream wrapper "' . $scheme . '", must be one of "' . implode('", "', stream_get_wrappers()) . '".');
		}
		...
		foreach((array)$this->getParameter('targets', array()) as $pattern) {
			...
			foreach($args as $arg) {
				$target = YoudsFrameworkToolkit::expandVariables($pattern, array_merge(array_filter($this->getParameters(), 'is_scalar'), array_filter($this->getParameters(), 'is_null'), $arg));
		...
		throw new YoudsFrameworkException('Template "' . $template . '" could not be found. Paths tried:' . "\n" . implode("\n", $attempts));
	}
}
